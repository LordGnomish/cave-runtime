...
# Manifest test count update — extract the test fn names from THIS cycle's
# block in qwen_drafted.rs and append [[tests]] entries to the crate's
# parity.manifest.toml so the parity calculator counts them. Without this
# step the qwen scaffolds raise tests/qwen_drafted.rs row count but the
# manifest stays frozen at the pre-cycle number — "fake parity progress."
MANIFEST="$WT_DIR/crates/$crate/parity.manifest.toml"
DRAFTED="$WT_DIR/crates/$crate/tests/qwen_drafted.rs"
MANIFEST_DELTA=0
if [ -f "$MANIFEST" ] && [ -f "$DRAFTED" ]; then
  # Pull every fn name annotated by this cycle's marker. The cycle marker
  # line is `// === cycle <CYCLE_TS> ...` and the block runs to EOF or the
  # next cycle marker.
  ...
  if [ -n "$CYCLE_FNS" ]; then
    {
      echo ""
      echo "# Qwen-pump scaffold cycle ${CYCLE_TS} (model=${WINNING_MODEL:-fallback-stub}, success_at_retry=${SUCCESS_AT_RETRY})"
      echo "# upstream_test mappings are pending manual review; local_test names match"
      echo "# the #[test] #[ignore = \"impl pending\"] functions in tests/qwen_drafted.rs."
      while IFS= read -r fn; do
        ...
        echo "local_test    = \"$fn\""
        MANIFEST_DELTA=$((MANIFEST_DELTA + 1))
      done <<< "$CYCLE_FNS"
    } >> "$MANIFEST"
    # MANIFEST_DELTA was incremented inside a subshell on some Bash versions
    # — re-count from the appended block to be sure.
    MANIFEST_DELTA=$(echo "$CYCLE_FNS" | grep -cE '^[a-zA-Z0-9_]+$')
    log "manifest-update: +$MANIFEST_DELTA [[tests]] entries appended to $crate/parity.manifest.toml"
  else
    log "manifest-update: no test fns extracted from cycle marker block, skip"
  fi
else
  log "manifest-update: skipped (manifest or drafted file missing)"
fi

# Commit on the feature branch (qwen_drafted.rs + manifest delta in one commit).
( cd "$WT_DIR" \
  && git add "crates/$crate/tests/qwen_drafted.rs" "crates/$crate/parity.manifest.toml" \
  && git commit -m "feat($crate): qwen-amele +$N red test scaffold (impl pending; manifest +$MANIFEST_DELTA)
