...
date: 2026-04-29
crate: cave-net
sprint: feat/cave-net-real-100
sprint_tip: 31a7854891e7393aa4830aaaa723f184cccbd220
upstream_pinned: cilium/cilium @ v1.19.3
fill_ratio: 1.0000
tests_passing: 1759
tests_baseline: 1556
tests_delta: +203
...
# cave-net — Cilium parity report (permanent record, 2026-04-29)

This file is the dated, permanent copy of `crates/cave-net/PARITY_REPORT.md` as
of sprint commit `31a7854`. The companion `parity.manifest.toml` proves
*coverage* (134 entries, 117 mapped + 17 skipped + 0 unmapped, fill_ratio = 1.0);
this report describes *fidelity*.

## Verification snapshot (run on `feat/cave-net-real-100` @ 31a7854)

```
cargo build --release -p cave-net   # clean
cargo test -p cave-net --release    # 1697 + 56 + 0 + 6 + 0 = 1759 passed, 0 failed, 5 ignored
```
...
|---|---|---|
| `cave-net` (lib + bins) | 1697 | 0 |
| `tests/cilium_parity_e2e` | 56 | 0 |
| `tests/qwen_drafted` | 0 | 5 (scaffold placeholders) |
| `tests/wire_faithful` | 6 | 0 |
| doc-tests | 0 | 0 |
| **total** | **1759** | **5** |
...
## What is **not** wire-faithful — explicit list
