...
impl KafkaTransport {
    ...
    pub fn pick_partition(&self, event: &CloudEvent) -> u32 {
        ...
        let key = event.extensions.get("partitionkey").map(String::as_str).unwrap_or(event.id.as_str());
...
impl Transport for KafkaTransport {
    ...
    fn publish(&mut self, destination: &str, event: &CloudEvent) -> DeliveryReceipt {
        ...
        DeliveryReceipt { destination: format!("{}#p{}", topic, part), attempt: 1, delivered: true }
    }
    fn address(&self, destination: &str) -> String {
...
impl RabbitMqTransport {
    pub fn new(uri: &str) -> Self {
        Self { broker_uri: uri.to_string(), queues: HashMap::new(), attempts: HashMap::new() }
    }
}
...
impl Transport for RabbitMqTransport {
    ...
    fn publish(&mut self, destination: &str, event: &CloudEvent) -> DeliveryReceipt {
        ...
        *entry += 1;
        let attempt = *entry;
        self.queues.entry(q.clone()).or_default().push(event.clone());
        DeliveryReceipt { destination: q, attempt, delivered: true }
    }
    fn address(&self, destination: &str) -> String {
        format!("{}/knative-{}", self.broker_uri.trim_end_matches('/'), destination)
    }
}
...
impl PulsarTransport {
    ...
    pub fn topic_of(&self, destination: &str) -> String {
        format!("persistent://{}/{}/knative-{}", self.tenant, self.namespace, destination)
    }
}

impl Transport for PulsarTransport {
    fn name(&self) -> &'static str { "pulsar" }
    fn publish(&mut self, destination: &str, event: &CloudEvent) -> DeliveryReceipt {
        let topic = self.topic_of(destination);
        self.log.push((topic.clone(), event.clone()));
        DeliveryReceipt { destination: topic, attempt: 1, delivered: true }
    }
    fn address(&self, destination: &str) -> String { self.topic_of(destination) }
}
...
impl Transport for NatsTransport {
    ...
    fn publish(&mut self, destination: &str, event: &CloudEvent) -> DeliveryReceipt {
        ...
        self.subjects.entry(subj.clone()).or_default().push(event.clone());
        DeliveryReceipt { destination: subj, attempt: 1, delivered: true }
    }
    fn address(&self, destination: &str) -> String {
        let base = self.urls.first().cloned().unwrap_or_else(|| "nats://localhost:4222".to_string());
...
impl GitHubSource {
    pub fn new(owner_and_repo: &str) -> Self {
        Self { owner_and_repo: owner_and_repo.to_string(), ..Default::default() }
    }
    ...
    pub fn accept(&self, github_event_header: &str, body: &str, signature: Option<&str>, id: &str) -> Option<CloudEvent> {
        if !self.event_types.is_empty() && !self.event_types.iter().any(|t| t == github_event_header) {
            return None;
        }
        if let Some(ref tok) = self.secret_token {
            let want = hmac_sha256_hex(tok.as_bytes(), body.as_bytes());
            if signature.map(|s| s.trim_start_matches("sha256=").eq_ignore_ascii_case(&want)) != Some(true) {
                return None;
            }
        }
        ...
        Some(CloudEvent {
            id: id.to_string(),
            source: format!("/apis/sources.knative.dev/v1/githubsources/{}", self.owner_and_repo),
...
mod tests {
    ...
    fn kafka_partition_uses_partitionkey_extension() {
        ...
        ev1.extensions.insert("partitionkey".to_string(), "tenant-x".to_string());
        ev2.extensions.insert("partitionkey".to_string(), "tenant-x".to_string());
    ...
    fn pulsar_uses_persistent_address_with_tenant_ns() {
        ...
        assert!(r.destination.starts_with("persistent://tenant-x/ns-y/knative-orders"));
    }
    ...
    fn github_hmac_signature_required_when_token_set() {
        ...
        assert!(g.accept("issues", body, Some("sha256=deadbeef"), "id-1").is_none());
    ...
    fn github_event_carries_type_prefix() {
        ...
        assert_eq!(ev.extensions.get("githuborg").map(|s| s.as_str()), Some("acme/cave"));
    }
    ...
    fn hmac_sha256_matches_rfc4231_test1() {
        ...
        assert_eq!(h, "b0344c61d8db38535ca8afceaf0bf12b881dc200c9833da726e9376c2e32cff7");
    }
}
