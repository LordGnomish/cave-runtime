...
//
// Charter v2 8-gate self-audit for cave-embed (michaelfeil/infinity 0.0.75
// serving-surface parity — OpenAI /v1/embeddings + reranker, model registry,
// dynamic batching, pooling, quantization, result cache, EmbeddingBackend trait).
//
// Gates:
//   1. SPDX coverage 100% of src/*.rs
//   2. source_sha pinned (0.0.75)
//   3. last_audit = 2026-xx
...
fn gate_2_source_sha_pinned() {
    ...
    assert!(
        ...
        "manifest must declare source_sha = infinity release tag"
    );
    assert!(
        m.contains("0.0.75"),
        "source_sha must pin infinity 0.0.75"
    );
}
...
fn gate_5_fill_ratio_floor() {
    ...
    assert!(
        ratio >= 0.95,
        "fill_ratio = {} (need >= 0.95 — cave-embed in-scope coverage)",
        ratio
    );
}
...
fn gate_7_no_stub_macros_in_src() {
    ...
    walk_rs(&src, &mut |p| {
        ...
        // cave-embed prompt-template strings.
