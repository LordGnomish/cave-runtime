...
impl PodIpPool {
    ...
    pub fn dual_stack(
        ...
        tenant: TenantId,
        cidr_v4: impl Into<String>,
        cidr_v6: impl Into<String>,
    ) -> Self {
        Self {
            name: name.into(),
            tenant,
...
impl PoolState {
    ...
    fn allocate_v4(&mut self, owner: &str) -> Result<IpAddr, IpamError> {
        ...
        let cidr = self
            ...
            .cidr_v4
            .as_deref()
            .ok_or_else(|| IpamError::PoolNotFound(self.pool.name.clone()))?;
    ...
    fn allocate_specific(&mut self, owner: &str, ip: IpAddr) -> Result<(), IpamError> {
        let (set, owners, cidr_str) = match ip {
            IpAddr::V4(_) => (
                &mut self.allocated_v4,
                &mut self.owners_v4,
                self.pool.cidr_v4.as_deref(),
            ),
            IpAddr::V6(_) => (
                &mut self.allocated_v6,
                &mut self.owners_v6,
                self.pool.cidr_v6.as_deref(),
            ),
        };
        let cidr =
            cidr_str.ok_or_else(|| IpamError::AddressOutOfPool(ip, self.pool.name.clone()))?;
        let net = IpNet::from_str(cidr).map_err(|_| IpamError::BadCidr(cidr.to_string()))?;
        if !net.contains(&ip) {
...
impl Ipam {
    ...
    pub fn pool_status(&self, name: &str) -> Result<PoolStatus, IpamError> {
        let s = self
            .pools
            .get(name)
            .ok_or_else(|| IpamError::PoolNotFound(name.to_string()))?;
    ...
    pub fn allocate_v4(&mut self, pool: &str, owner: &str) -> Result<IpAddr, IpamError> {
        let s = self
            .pools
            .get_mut(pool)
            .ok_or_else(|| IpamError::PoolNotFound(pool.to_string()))?;
        s.allocate_v4(owner)
    }
    ...
    pub fn allocate_dual_stack(
        &mut self,
        pool: &str,
        owner: &str,
    ) -> Result<(IpAddr, IpAddr), IpamError> {
    ...
    pub fn allocate_from_node(
        &mut self,
        tenant: TenantId,
        node: &str,
        owner: &str,
    ) -> Result<(Option<IpAddr>, Option<IpAddr>), IpamError> {
        if !matches!(self.mode, IpamMode::Kubernetes) {
            // For ClusterPool we require an explicit pool.
        }
        let (v4_cidr, v6_cidr) = self
            .node_cidrs
            .get(node)
            .cloned()
        ...
        let v4 = if v4_cidr.is_some() {
            Some(self.allocate_v4(&pool_name, owner)?)
        } else {
            None
        };
        let v6 = if v6_cidr.is_some() {
            Some(self.allocate_v6(&pool_name, owner)?)
        } else {
            None
        };
        Ok((v4, v6))
...
mod tests {
    ...
    fn ipam_cluster_pool_allocates_first_available_address() {
        let (_c, tenant) = cilium_test_ctx!(
            ...
            "Allocate",
            "tenant-ipam-cp-first"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/29"))
            .unwrap();
    ...
    fn ipam_cluster_pool_skips_gateway_address() {
        ...
        assert_eq!(
            ipam.allocate_v4("default", "ns/p").unwrap(),
            ip4(10, 0, 0, 2)
        );
    }
    ...
    fn ipam_cluster_pool_exhaustion_returns_error() {
        ...
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/30"))
            .unwrap();
    ...
    fn ipam_release_specific_unallocated_returns_false() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/clusterpool/clusterpool.go",
            "ReleaseIP.NotAllocated",
            "tenant-ipam-relip-na"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/29"))
            .unwrap();
    ...
    fn ipam_allocate_specific_address() {
        ...
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/24"))
            .unwrap();
        ipam.allocate_specific("default", "ns/p1", ip4(10, 0, 0, 100))
            .unwrap();
    ...
    fn ipam_allocate_specific_collision_returns_in_use() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/clusterpool/clusterpool.go",
            "AllocateIP.Collision",
            "tenant-ipam-allocip-coll"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/24"))
            .unwrap();
        ipam.allocate_specific("default", "ns/p1", ip4(10, 0, 0, 100))
            .unwrap();
        let err = ipam
            .allocate_specific("default", "ns/p2", ip4(10, 0, 0, 100))
            .unwrap_err();
    ...
    fn ipam_allocate_specific_out_of_pool_rejected() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/clusterpool/clusterpool.go",
            "AllocateIP.OutOfPool",
            "tenant-ipam-allocip-out"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/24"))
            .unwrap();
        let err = ipam
            .allocate_specific("default", "ns/p1", ip4(11, 0, 0, 1))
            .unwrap_err();
        assert_eq!(
            err,
            IpamError::AddressOutOfPool(ip4(11, 0, 0, 1), "default".into())
        );
    }
    ...
    fn ipam_pool_capacity_matches_cidr_size() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/ipam/types.go", "Pool.Capacity", "tenant-ipam-cap");
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/24"))
            .unwrap();
        let st = ipam.pool_status("default").unwrap();
    ...
    fn ipam_upsert_pool_with_bad_cidr_rejected() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/ipam/types.go", "Pool.Validate", "tenant-ipam-bad");
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        let err = ipam
            .upsert_pool(PodIpPool::ipv4("default", tenant, "not-a-cidr"))
            .unwrap_err();
    ...
    fn ipam_multi_pool_named_lookup_separately() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/multipool/manager.go",
            "Allocate",
            "tenant-ipam-mp-named"
        );
        let mut ipam = Ipam::new(IpamMode::MultiPool);
        ipam.upsert_pool(PodIpPool::ipv4(
            "workload-a",
            tenant.clone(),
            "10.10.0.0/24",
        ))
        .unwrap();
        ipam.upsert_pool(PodIpPool::ipv4("workload-b", tenant, "10.20.0.0/24"))
            .unwrap();
    ...
    fn ipam_multi_pool_upsert_replaces_existing_definition() {
        ...
        let mut ipam = Ipam::new(IpamMode::MultiPool);
        ipam.upsert_pool(PodIpPool::ipv4("p", tenant.clone(), "10.0.0.0/24"))
            .unwrap();
    ...
    fn ipam_dual_stack_allocates_v4_and_v6() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/types.go",
            "AllocateNext.DualStack",
            "tenant-ipam-ds-alloc"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::dual_stack(
            "default",
            tenant,
            "10.0.0.0/24",
            "fd00::/64",
    ...
    fn ipam_low_watermark_breach_when_free_below_threshold() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/ipam/types.go", "Watermark.Low", "tenant-ipam-wmlow");
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        let mut pool = PodIpPool::ipv4("default", tenant, "10.0.0.0/29");
    ...
    fn ipam_high_watermark_breach_when_free_above_threshold() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/ipam/types.go", "Watermark.High", "tenant-ipam-wmhi");
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
    ...
    fn ipam_pool_status_zero_when_empty_pool() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/ipam/types.go", "Status.Empty", "tenant-ipam-st-empty");
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        ipam.upsert_pool(PodIpPool::ipv4("default", tenant, "10.0.0.0/29"))
            .unwrap();
        let st = ipam.pool_status("default").unwrap();
    ...
    fn ipam_kubernetes_mode_allocates_from_node_pod_cidr() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/ipam/hostscope/hostscope.go",
            "Allocate",
            "tenant-ipam-k8s"
        );
    ...
    fn ipam_kubernetes_mode_unknown_node_returns_error() {
        ...
        let mut ipam = Ipam::new(IpamMode::Kubernetes);
        let err = ipam
            .allocate_from_node(tenant, "node-x", "ns/p1")
            .unwrap_err();
    ...
    fn ipam_kubernetes_mode_dual_stack_node_returns_both() {
        ...
        let mut ipam = Ipam::new(IpamMode::Kubernetes);
        ipam.set_node_pod_cidrs(
            "node-b",
            Some("10.244.2.0/24".into()),
            Some("fd00:2::/64".into()),
        );
    ...
    fn ipam_kubernetes_mode_isolates_per_node() {
        ...
        let (a, _) = ipam
            .allocate_from_node(tenant.clone(), "node-a", "ns/p1")
            .unwrap();
    ...
    fn ipam_release_specific_unknown_pool_returns_error() {
        let (_c, _t) = cilium_test_ctx!(
            "pkg/ipam/ipam.go",
            "ReleaseIP.PoolNotFound",
            "tenant-ipam-rel-pnf"
        );
        let mut ipam = Ipam::new(IpamMode::ClusterPool);
        let err = ipam
            .release_specific("missing", ip4(10, 0, 0, 1))
            .unwrap_err();
        assert_eq!(err, IpamError::PoolNotFound("missing".into()));
    }
}
