...
impl FqdnCache {
    ...
    pub fn release(&mut self, id: u32) -> bool {
        if let Some(entry) = self.by_id.remove(&id) {
            self.by_pattern_ip
                .remove(&(entry.pattern.clone(), entry.ip));
...
mod tests {
    ...
    fn fqdn_resolve_assigns_identity_in_fqdn_range() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/fqdn/fqdn.go", "FQDNCache.Resolve", "tenant-fqdn-rng");
        let mut c = FqdnCache::new(tenant);
        let id = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 4), 300, 100)
            .unwrap();
    ...
    fn fqdn_resolve_idempotent_for_same_pattern_and_ip() {
        ...
        let a = c
            ...
            .unwrap();
        let b = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 4), 300, 200)
            .unwrap();
        assert_eq!(a, b);
    }
    ...
    fn fqdn_distinct_ips_get_distinct_identities() {
        ...
        let a = c
            ...
            .unwrap();
        let b = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 5), 300, 100)
            .unwrap();
        assert_ne!(a, b);
        assert_eq!(b, a + 1);
    }
    ...
    fn fqdn_lookup_by_ip_returns_entry() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/fqdn/fqdn.go", "FQDNCache.LookupIP", "tenant-fqdn-lkip");
        let mut c = FqdnCache::new(tenant);
        let _ = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 4), 300, 100)
            .unwrap();
    ...
    fn fqdn_lookup_by_id_returns_entry() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/fqdn/fqdn.go", "FQDNCache.LookupID", "tenant-fqdn-lkid");
        let mut c = FqdnCache::new(tenant);
        let id = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 4), 300, 100)
            .unwrap();
    ...
    fn fqdn_release_drops_entry() {
        let (_c, tenant) =
            cilium_test_ctx!("pkg/fqdn/fqdn.go", "FQDNCache.Release", "tenant-fqdn-rel");
        let mut c = FqdnCache::new(tenant);
        let id = c
            .resolve("*.example.com", "api.example.com", ip(1, 2, 3, 4), 300, 100)
            .unwrap();
    ...
    fn fqdn_gc_removes_expired_entries() {
        ...
        let mut c = FqdnCache::new(tenant);
        c.resolve("*.example.com", "a.example.com", ip(1, 2, 3, 4), 60, 100)
            .unwrap();
        c.resolve("*.example.com", "b.example.com", ip(1, 2, 3, 5), 600, 100)
            .unwrap();
        let n = c.gc(200);
        assert_eq!(n, 1);
        assert!(c.lookup_by_ip(ip(1, 2, 3, 4)).is_none());
    ...
    fn fqdn_resolve_refreshes_ttl_on_hit() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/fqdn/fqdn.go",
            "FQDNCache.Resolve.RefreshTTL",
            "tenant-fqdn-ttl"
        );
        let mut c = FqdnCache::new(tenant);
        let id = c
            .resolve("*.example.com", "a.example.com", ip(1, 2, 3, 4), 60, 100)
            .unwrap();
        // Re-resolve at t=200 with TTL=600 → entry shouldn't be GC'd at t=300.
        c.resolve("*.example.com", "a.example.com", ip(1, 2, 3, 4), 600, 200)
            .unwrap();
        let n = c.gc(300);
        assert_eq!(n, 0);
