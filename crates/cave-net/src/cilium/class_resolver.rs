...
impl ClassResolver {
    pub fn new(tenant: TenantId, controller_name: impl Into<String>) -> Self {
        Self {
            tenant, controller_name: controller_name.into(),
    ...
    pub fn remove_class(&mut self, name: &str) -> Result<(), ResolverError> {
        self.classes.remove(name).ok_or_else(|| ResolverError::ClassNotFound(name.to_string()))?;
        Ok(())
    }
    ...
    pub fn resolve(&self, class_name: &str) -> Result<Option<&ClassParameters>, ResolverError> {
        let class = self.classes.get(class_name).ok_or_else(|| ResolverError::ClassNotFound(class_name.to_string()))?;
        if class.controller != self.controller_name {
            return Err(ResolverError::NotOwned(class_name.to_string(), class.controller.clone()));
        }
        ...
        let params = self.parameters.get(&key).ok_or_else(|| ResolverError::ParametersNotFound(pref.kind.clone(), pref.name.clone()))?;
        Ok(Some(params))
    }

    /// All Cilium-owned classes in the registry.
    pub fn owned_classes(&self) -> Vec<&ClassSpec> {
        self.classes.values().filter(|c| c.controller == self.controller_name).collect()
    }
}
...
mod tests {
    ...
    fn class(name: &str, controller: &str, params: Option<ParametersRef>) -> ClassSpec {
        ClassSpec { name: name.into(), controller: controller.into(), parameters: params }
    }

    fn params(pairs: &[(&str, &str)]) -> ClassParameters {
        ClassParameters {
            config: pairs.iter().map(|(k, v)| ((*k).into(), (*v).into())).collect(),
        }
    }

    fn ref_to(kind: &str, name: &str) -> ParametersRef {
        ParametersRef { api_group: "cilium.io".into(), kind: kind.into(), name: name.into(), namespace: None }
    }
    ...
    fn upsert_class_records_it() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Upsert", "tenant-cr-u");
    ...
    fn remove_class_drops_it() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Remove", "tenant-cr-rm");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller", None));
    ...
    fn remove_unknown_class_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Remove.NotFound", "tenant-cr-rmnf");
        let mut r = resolver(tenant);
    ...
    fn upsert_parameters_keyed_by_kind_name() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "UpsertParameters", "tenant-cr-up");
        let mut r = resolver(tenant);
        r.upsert_parameters("CiliumIngressClassParams", "default", params(&[("scope", "cluster")]));
        // Verify by resolution path.
        r.upsert_class(class("cilium", "cilium.io/ingress-controller",
            Some(ref_to("CiliumIngressClassParams", "default"))));
        let resolved = r.resolve("cilium").unwrap().unwrap();
        assert_eq!(resolved.config.get("scope").map(|s| s.as_str()), Some("cluster"));
    }

    #[test]
    fn remove_parameters_drops_entry() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "RemoveParameters", "tenant-cr-rmp");
        let mut r = resolver(tenant);
    ...
    fn resolve_unknown_class_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Resolve.ClassNotFound", "tenant-cr-rcnf");
    ...
    fn resolve_class_owned_by_other_controller_rejected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Resolve.NotOwned", "tenant-cr-rno");
        let mut r = resolver(tenant);
    ...
    fn resolve_class_without_parameters_returns_none() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Resolve.NoParams", "tenant-cr-rnp");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller", None));
    ...
    fn resolve_with_dangling_parameters_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Resolve.Dangling", "tenant-cr-rd");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller",
    ...
    fn resolve_returns_full_parameter_set() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Resolve.Full", "tenant-cr-rf");
        let mut r = resolver(tenant);
        r.upsert_parameters("Params", "default", params(&[
            ("lb_pool", "10.0.0.0/24"),
            ("tls_profile", "modern"),
        ]));
        r.upsert_class(class("cilium", "cilium.io/ingress-controller",
    ...
    fn owned_classes_filters_by_controller_name() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Owned", "tenant-cr-o");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller", None));
        r.upsert_class(class("nginx", "nginx.io/ingress-controller", None));
    ...
    fn owned_classes_empty_when_none_match() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Owned.None", "tenant-cr-on");
        let mut r = resolver(tenant);
        r.upsert_class(class("nginx", "nginx.io/ingress-controller", None));
    ...
    fn class_count_tracks_upserts() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Count", "tenant-cr-c");
        let mut r = resolver(tenant);
        for i in 0..5u8 {
            r.upsert_class(class(&format!("c-{i}"), "cilium.io/ingress-controller", None));
        }
    ...
    fn upsert_class_replaces_existing() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Upsert.Replace", "tenant-cr-rep");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller", None));
        r.upsert_class(class("cilium", "cilium.io/ingress-controller",
    ...
    fn resolver_works_for_gateway_class_controller_name() {
        let (_c, tenant) = cilium_test_ctx!("pkg/gateway-api/translation/gatewayclass.go", "Resolve", "tenant-cr-gw");
    ...
    fn resolve_handles_mixed_owners_correctly() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Mixed", "tenant-cr-mx");
        let mut r = resolver(tenant);
        r.upsert_class(class("cilium", "cilium.io/ingress-controller", None));
        r.upsert_class(class("nginx", "nginx.io/ingress-controller", None));
        // Cilium resolves; nginx errors NotOwned.
        assert!(r.resolve("cilium").is_ok());
        assert!(matches!(r.resolve("nginx").unwrap_err(), ResolverError::NotOwned(_, _)));
    }
    ...
    fn class_spec_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "ClassSpec.Serde", "tenant-cr-cserde");
        let c = class("cilium", "cilium.io/ingress-controller",
    ...
    fn class_parameters_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "Parameters.Serde", "tenant-cr-pserde");
    ...
    fn parameters_ref_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/ingress/class_resolver.go", "ParametersRef.Serde", "tenant-cr-prserde");
