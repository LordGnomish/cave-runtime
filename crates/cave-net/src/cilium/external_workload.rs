...
pub enum ExternalWorkloadError {
    ...
    BadTransition { from: WorkloadState, to: WorkloadState },
...
impl ExternalWorkloadManager {
    ...
    pub fn register(&mut self, spec: ExternalWorkloadSpec) -> Result<(), ExternalWorkloadError> {
        if spec.tenant != self.tenant {
            return Err(ExternalWorkloadError::TenantDenied { tenant: spec.tenant });
        }
        ...
        self.workloads.insert(spec.name.clone(), ExternalWorkload {
            spec, state: WorkloadState::Pending, identity: None, last_heartbeat_ns: 0,
        });
        Ok(())
    }

    pub fn deregister(&mut self, name: &str) -> Result<(), ExternalWorkloadError> {
        let w = self.workloads.remove(name).ok_or_else(|| ExternalWorkloadError::NotFound(name.to_string()))?;
    ...
    pub fn transition(&mut self, name: &str, to: WorkloadState) -> Result<(), ExternalWorkloadError> {
        let w = self.workloads.get_mut(name).ok_or_else(|| ExternalWorkloadError::NotFound(name.to_string()))?;
        let from = w.state;
        let ok = matches!(
            (from, to),
    ...
    pub fn assign_identity(&mut self, name: &str, identity: u32) -> Result<(), ExternalWorkloadError> {
        let w = self.workloads.get_mut(name).ok_or_else(|| ExternalWorkloadError::NotFound(name.to_string()))?;
    ...
    pub fn ready_count(&self) -> usize {
        self.workloads.values().filter(|w| matches!(w.state, WorkloadState::Ready)).count()
    }
}
...
mod tests {
    ...
    fn ew_register_succeeds_with_v4() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Register", "tenant-ew-reg");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        assert_eq!(m.count(), 1);
    }

    #[test]
    fn ew_register_duplicate_rejected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Register.Duplicate", "tenant-ew-dup");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant.clone(), Some(ip(192, 168, 1, 10)))).unwrap();
        let err = m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 11)))).unwrap_err();
        assert!(matches!(err, ExternalWorkloadError::Duplicate(_)));
    }

    #[test]
    fn ew_register_no_address_rejected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Register.NoAddr", "tenant-ew-noaddr");
        let mut m = mgr(tenant.clone());
    ...
    fn ew_lookup_by_name() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Lookup.Name", "tenant-ew-lkn");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    #[test]
    fn ew_lookup_by_ip() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Lookup.IP", "tenant-ew-lkip");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_lookup_unknown_returns_none() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Lookup.NotFound", "tenant-ew-lknf");
        let m = mgr(tenant);
    ...
    fn ew_initial_state_pending() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.Initial", "tenant-ew-pend");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_transition_pending_to_connected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.Connected", "tenant-ew-conn");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_transition_connected_to_ready() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.Ready", "tenant-ew-rdy");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        m.transition("vm-1", WorkloadState::Connected).unwrap();
    ...
    fn ew_transition_ready_to_stale_to_ready() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.Recovery", "tenant-ew-rec");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        m.transition("vm-1", WorkloadState::Connected).unwrap();
        m.transition("vm-1", WorkloadState::Ready).unwrap();
    ...
    fn ew_invalid_transition_rejected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.BadTransition", "tenant-ew-bt");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_assign_identity() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "AssignIdentity", "tenant-ew-aid");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_assign_identity_unknown_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "AssignIdentity.NotFound", "tenant-ew-aidnf");
        let mut m = mgr(tenant);
    ...
    fn ew_heartbeat_updates_timestamp() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Heartbeat", "tenant-ew-hb");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_heartbeat_recovers_stale_to_ready() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Heartbeat.Recovery", "tenant-ew-hbr");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        m.transition("vm-1", WorkloadState::Connected).unwrap();
        m.transition("vm-1", WorkloadState::Ready).unwrap();
    ...
    fn ew_sweep_marks_stale_after_threshold() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Sweep", "tenant-ew-sw");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        m.transition("vm-1", WorkloadState::Connected).unwrap();
        m.transition("vm-1", WorkloadState::Ready).unwrap();
    ...
    fn ew_sweep_keeps_recent_heartbeat() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Sweep.Fresh", "tenant-ew-swf");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
        m.transition("vm-1", WorkloadState::Connected).unwrap();
        m.transition("vm-1", WorkloadState::Ready).unwrap();
    ...
    fn ew_sweep_skips_non_ready_workloads() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Sweep.OnlyReady", "tenant-ew-swr");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_deregister_drops_workload_and_ip_index() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Deregister", "tenant-ew-rm");
        let mut m = mgr(tenant.clone());
        m.register(spec("vm-1", tenant, Some(ip(192, 168, 1, 10)))).unwrap();
    ...
    fn ew_deregister_unknown_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "Deregister.NotFound", "tenant-ew-rmnf");
        let mut m = mgr(tenant);
    ...
    fn ew_ready_count_only_counts_ready_state() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "ReadyCount", "tenant-ew-rdc");
        let mut m = mgr(tenant.clone());
        for i in 0..5u8 {
            m.register(spec(&format!("vm-{i}"), tenant.clone(), Some(ip(192, 168, 1, i)))).unwrap();
            if i < 3 {
                m.transition(&format!("vm-{i}"), WorkloadState::Connected).unwrap();
                m.transition(&format!("vm-{i}"), WorkloadState::Ready).unwrap();
            }
        }
    ...
    fn ew_dual_stack_workload_indexed_by_both_addrs() {
        let (_c, tenant) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "DualStack", "tenant-ew-ds");
        let mut m = mgr(tenant.clone());
    ...
    fn ew_workload_serde_round_trip() {
        let (_c, tenant) = cilium_test_ctx!("pkg/k8s/apis/cilium.io/v2/types.go", "ExternalWorkload.Serde", "tenant-ew-serde");
    ...
    fn ew_state_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/clustermesh/externalworkloads/manager.go", "State.Serde", "tenant-ew-stserde");
        for s in [WorkloadState::Pending, WorkloadState::Connected, WorkloadState::Ready, WorkloadState::Stale] {
