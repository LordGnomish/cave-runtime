...
pub enum IpcacheError {
    ...
    LowerPriority { existing: IpcacheSource, incoming: IpcacheSource },
...
impl Ipcache {
    pub fn new(tenant: TenantId) -> Self {
        Self { tenant, entries: HashMap::new() }
    }
...
mod tests {
    ...
    fn entry(identity: u32, source: IpcacheSource) -> IpcacheEntry {
        IpcacheEntry { identity, source, encryption_key: 0, tunnel_endpoint: None }
    }
    ...
    fn ipcache_source_priority_ordering() {
        let (_c, _t) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Source.Priority", "tenant-ipc-prio");
    ...
    fn ipcache_upsert_and_lookup_round_trip() {
        ...
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
        assert_eq!(c.lookup(ip(10, 0, 1, 5)).unwrap().identity, 256);
    }

    #[test]
    fn ipcache_lookup_unknown_returns_none() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Lookup.NotFound", "tenant-ipc-lknf");
    ...
    fn ipcache_identity_of_returns_identity() {
        let (_c, tenant) = cilium_test_ctx!("bpf/lib/ipcache.h", "lookup_ip4_remote_endpoint", "tenant-ipc-id");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(257, IpcacheSource::Local)).unwrap();
    ...
    fn ipcache_upsert_lower_priority_rejected() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Upsert.LowerPriority", "tenant-ipc-lp");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
        let err = c.upsert(ip(10, 0, 1, 5), entry(999, IpcacheSource::Kvstore)).unwrap_err();
    ...
    fn ipcache_upsert_higher_priority_overwrites() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Upsert.HigherPriority", "tenant-ipc-hp");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(999, IpcacheSource::Kvstore)).unwrap();
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
        assert_eq!(c.identity_of(ip(10, 0, 1, 5)), Some(256));
    }
    ...
    fn ipcache_upsert_same_priority_overwrites() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Upsert.SamePriority", "tenant-ipc-sp");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(100, IpcacheSource::Kvstore)).unwrap();
        c.upsert(ip(10, 0, 1, 5), entry(200, IpcacheSource::Kvstore)).unwrap();
    ...
    fn ipcache_remove_if_source_match_drops_entry() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Remove.SourceMatch", "tenant-ipc-rmm");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(100, IpcacheSource::Kvstore)).unwrap();
    ...
    fn ipcache_remove_if_source_mismatch_keeps_entry() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Remove.SourceMismatch", "tenant-ipc-rmmm");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
        assert!(!c.remove_if_source(ip(10, 0, 1, 5), IpcacheSource::Kvstore));
        // Entry still present.
    ...
    fn ipcache_remove_force_drops_any_source() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Remove.Force", "tenant-ipc-rmf");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
    ...
    fn ipcache_remove_force_unknown_returns_none() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Remove.Force.NotFound", "tenant-ipc-rmfnf");
        let mut c = Ipcache::new(tenant);
    ...
    fn ipcache_list_by_source_filters_correctly() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "ListBySource", "tenant-ipc-ls");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
        c.upsert(ip(10, 0, 1, 6), entry(257, IpcacheSource::Local)).unwrap();
        c.upsert(ip(10, 0, 1, 7), entry(258, IpcacheSource::Kvstore)).unwrap();
    ...
    fn ipcache_purge_source_drops_all_matching() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "PurgeSource", "tenant-ipc-pur");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Kvstore)).unwrap();
        c.upsert(ip(10, 0, 1, 6), entry(257, IpcacheSource::Kvstore)).unwrap();
        c.upsert(ip(10, 0, 1, 7), entry(258, IpcacheSource::Local)).unwrap();
    ...
    fn ipcache_purge_source_empty_returns_zero() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "PurgeSource.Empty", "tenant-ipc-pure");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Local)).unwrap();
    ...
    fn ipcache_entry_with_encryption_key() {
        let (_c, tenant) = cilium_test_ctx!("bpf/lib/ipcache.h", "remote_endpoint_info.key", "tenant-ipc-key");
        let mut c = Ipcache::new(tenant);
    ...
    fn ipcache_entry_with_tunnel_endpoint() {
        let (_c, tenant) = cilium_test_ctx!("bpf/lib/ipcache.h", "remote_endpoint_info.tunnel_endpoint", "tenant-ipc-tep");
        let mut c = Ipcache::new(tenant);
        let mut e = entry(256, IpcacheSource::Local);
        e.tunnel_endpoint = Some(ip(10, 0, 0, 1));
        c.upsert(ip(10, 244, 1, 5), e).unwrap();
        assert_eq!(c.lookup(ip(10, 244, 1, 5)).unwrap().tunnel_endpoint, Some(ip(10, 0, 0, 1)));
    }
    ...
    fn ipcache_len_tracks_inserts() {
        ...
        let mut c = Ipcache::new(tenant);
        for i in 0..10u8 {
            c.upsert(ip(10, 0, 1, i), entry(256 + i as u32, IpcacheSource::Local)).unwrap();
        }
        assert_eq!(c.len(), 10);
    }
    ...
    fn ipcache_clustermesh_loses_to_generated() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Source.GenVsMesh", "tenant-ipc-gvm");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Generated)).unwrap();
        let err = c.upsert(ip(10, 0, 1, 5), entry(999, IpcacheSource::ClusterMesh)).unwrap_err();
        assert!(matches!(err, IpcacheError::LowerPriority { .. }));
    }

    #[test]
    fn ipcache_kvstore_overrides_restored() {
        let (_c, tenant) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Source.KvOverRestored", "tenant-ipc-kor");
        let mut c = Ipcache::new(tenant);
        c.upsert(ip(10, 0, 1, 5), entry(256, IpcacheSource::Restored)).unwrap();
        c.upsert(ip(10, 0, 1, 5), entry(999, IpcacheSource::Kvstore)).unwrap();
        assert_eq!(c.identity_of(ip(10, 0, 1, 5)), Some(999));
    }
    ...
    fn ipcache_entry_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Entry.Serde", "tenant-ipc-eserde");
        let e = IpcacheEntry {
            identity: 256, source: IpcacheSource::Local,
            encryption_key: 7, tunnel_endpoint: Some(ip(10, 0, 0, 1)),
        };
    ...
    fn ipcache_source_serde_round_trip() {
        let (_c, _t) = cilium_test_ctx!("pkg/ipcache/ipcache.go", "Source.Serde", "tenant-ipc-sserde");
        for s in [
