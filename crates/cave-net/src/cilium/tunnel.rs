...
impl TunnelManager {
    pub fn new(
        tenant: TenantId,
        mode: TunnelMode,
        native_routing_cidr: Option<String>,
    ...
    pub fn remove_endpoint(&mut self, node: &str) -> Result<(), TunnelError> {
        self.endpoints
            .remove(node)
            .ok_or_else(|| TunnelError::NodeNotFound(node.to_string()))?;
        Ok(())
    }
    ...
    pub fn lookup_encap(&self, dst_ip: IpAddr) -> Result<EncapDecision, TunnelError> {
        ...
        for ep in self.endpoints.values() {
            let net = IpNet::from_str(&ep.pod_cidr)
                .map_err(|_| TunnelError::BadCidr(ep.pod_cidr.clone()))?;
            if net.contains(&dst_ip) {
                return Ok(EncapDecision::Encap {
                    node_ip: ep.node_ip,
                    vni: ep.vni,
                });
            }
...
mod tests {
    ...
    fn tunnel_disabled_mode_without_native_cidr_rejected() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.New.DisabledRequiresNative",
            "tenant-tun-disreq"
        );
    ...
    fn tunnel_disabled_mode_with_native_cidr_succeeds() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.New.Disabled",
            "tenant-tun-disok"
        );
        let m =
            TunnelManager::new(tenant, TunnelMode::Disabled, Some("10.0.0.0/16".into())).unwrap();
    ...
    fn tunnel_bad_native_cidr_rejected() {
        ...
        let err =
            TunnelManager::new(tenant, TunnelMode::Vxlan, Some("not-a-cidr".into())).unwrap_err();
    ...
    fn tunnel_upsert_endpoint_succeeds() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.UpsertEndpoint",
            "tenant-tun-up"
        );
        let mut m = vxlan_mgr(tenant);
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 0))
            .unwrap();
    ...
    fn tunnel_upsert_endpoint_with_bad_cidr_rejected() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.UpsertEndpoint.BadCidr",
            "tenant-tun-upbad"
        );
        let mut m = vxlan_mgr(tenant);
    ...
    fn tunnel_upsert_endpoint_replaces_in_place() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.UpsertEndpoint.Replace",
            "tenant-tun-uprep"
        );
        let mut m = vxlan_mgr(tenant);
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 0))
            .unwrap();
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 99), "10.244.1.0/24", 0))
            .unwrap();
        assert_eq!(m.endpoint_count(), 1);
    ...
    fn tunnel_remove_unknown_endpoint_returns_not_found() {
        let (_c, tenant) = cilium_test_ctx!(
            "pkg/datapath/tunnel/tunnel.go",
            "Manager.RemoveEndpoint.NotFound",
            "tenant-tun-rmnf"
        );
        let mut m = vxlan_mgr(tenant);
    ...
    fn tunnel_endpoint_count_tracks_upserts() {
        ...
        let mut m = vxlan_mgr(tenant);
        for i in 0..5u8 {
            m.upsert_endpoint(make_endpoint(
                &format!("node-{i}"),
                (10, 0, 0, i + 1),
                &format!("10.244.{i}.0/24"),
                0,
            ))
            .unwrap();
        }
    ...
    fn tunnel_lookup_encap_routes_to_owning_node() {
        ...
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 0))
            .unwrap();
        m.upsert_endpoint(make_endpoint("node-b", (10, 0, 0, 2), "10.244.2.0/24", 0))
            .unwrap();
    ...
    fn tunnel_native_routing_cidr_skips_encap() {
        let (_c, tenant) = cilium_test_ctx!(
            "bpf/lib/tunnel.h",
            "tunnel_lookup.Native",
            "tenant-tun-natv"
        );
        let mut m =
            TunnelManager::new(tenant, TunnelMode::Vxlan, Some("172.16.0.0/12".into())).unwrap();
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 0))
            .unwrap();
    ...
    fn tunnel_native_routing_cidr_overrides_endpoint_match() {
        let (_c, tenant) = cilium_test_ctx!(
            "bpf/lib/tunnel.h",
            "tunnel_lookup.NativeOverEndpoint",
            "tenant-tun-natov"
        );
        let mut m =
    ...
    fn tunnel_disabled_mode_unknown_destination_unknown() {
        let (_c, tenant) = cilium_test_ctx!(
            "bpf/lib/tunnel.h",
            "tunnel_lookup.DisabledUnknown",
            "tenant-tun-disunk"
        );
        let m =
            TunnelManager::new(tenant, TunnelMode::Disabled, Some("10.0.0.0/8".into())).unwrap();
        let d = m.lookup_encap(ip(8, 8, 8, 8)).unwrap();
        assert_eq!(d, EncapDecision::Unknown);
    }
    ...
    fn tunnel_geneve_mode_encaps_traffic() {
        ...
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 7))
            .unwrap();
        let d = m.lookup_encap(ip(10, 244, 1, 5)).unwrap();
        assert_eq!(
            d,
    ...
    fn tunnel_vni_carries_through_to_encap_decision() {
        let (_c, tenant) =
            cilium_test_ctx!("bpf/lib/tunnel.h", "tunnel_lookup.VNI", "tenant-tun-vni");
        let mut m = vxlan_mgr(tenant);
        m.upsert_endpoint(make_endpoint("node-a", (10, 0, 0, 1), "10.244.1.0/24", 42))
            .unwrap();
        let d = m.lookup_encap(ip(10, 244, 1, 5)).unwrap();
        match d {
