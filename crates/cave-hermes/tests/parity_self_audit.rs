...
//! Charter v2 self-audit for cave-hermes — pinned to hermes-agent v2026.5.16.
//!
...
#[test]
fn gate_1_upstream_version_pinned_v2026_5_16() {
    let m = manifest_text();
    let v = extract_after(&m, "\nversion ").or_else(|| extract_after(&m, "\nversion="));
    assert_eq!(
        v.as_deref(),
        Some("v2026.5.16"),
        "manifest [upstream] version must pin Hermes Agent v2026.5.16 (was {:?}). \
         Charter v2 always-latest gate.",
        v
    );
    assert_eq!(
        cave_hermes::UPSTREAM_VERSION,
        "v2026.5.16",
...
fn gate_6_mapped_partial_skipped_unmapped_sum_to_total_with_floor() {
    ...
    assert!(
        mapped >= 20,
        "cave-hermes MVP floor: >= 20 mapped subsystems (got {})",
        mapped
    );
}
...
fn gate_8_every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        total >= 10,
        "expected >= 10 .rs files in cave-hermes; got {}",
        total
    );
}

#[test]
fn gate_9_default_runtime_wires_four_named_builtins() {
    // Backend track minimum: the runtime ships with bash/file_read/
    // file_write/web_fetch registered under stable names, and the model
    // router has a Local/Mid/Top tier each populated.
    let rt = cave_hermes::default_runtime();
    ...
    for tier in [
        ...
        cave_hermes::router::ModelTier::Top,
    ] {
        assert!(
            !rt.router.tier_models(tier).is_empty(),
            "tier {tier:?} has no models in default_runtime"
        );
    }
}
