...
impl NamespaceState {
    ...
    pub fn upsert(&self, tenant: &str, ns: &str, phase: NamespacePhase) {
        self.inner
            .write()
            .unwrap()
            .insert((tenant.to_string(), ns.to_string()), phase);
    }
    ...
    pub fn delete(&self, tenant: &str, ns: &str) {
        self.inner
            .write()
            .unwrap()
            .remove(&(tenant.to_string(), ns.to_string()));
    }
    ...
    pub fn phase(&self, tenant: &str, ns: &str) -> Option<NamespacePhase> {
        self.inner
            .read()
            .unwrap()
            .get(&(tenant.to_string(), ns.to_string()))
            .copied()
    }
}
...
impl ValidatingWebhook for NamespaceLifecycleStrict {
    ...
    fn validate(&self, req: &AdmissionRequest) -> AdmissionResponse {
        ...
        match self.state.phase(&req.tenant_id, &req.namespace) {
            Some(NamespacePhase::Terminating) => AdmissionResponse::deny(
                req,
                403,
            ...
            None => {
                AdmissionResponse::deny(req, 404, format!("namespace {} not found", req.namespace))
            }
        }
    }
}
...
impl LimitRangeStore {
    ...
    pub fn put(&self, lr: LimitRange) {
        ...
        g.retain(|x| {
            !(x.tenant_id == lr.tenant_id && x.namespace == lr.namespace && x.name == lr.name)
        });
        g.push(lr);
    }
...
impl std::fmt::Display for LimitRangeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            LimitRangeError::Below {
                resource,
                value,
                min,
            } => write!(f, "{} value {} below min {}", resource, value, min),
            LimitRangeError::Above {
                resource,
                value,
                max,
            } => write!(f, "{} value {} above max {}", resource, value, max),
            LimitRangeError::RatioExceeded {
                resource,
                ratio_observed,
                ratio_max,
            } => write!(
                f,
                "{} ratio {} exceeds limit/request ratio {}",
                resource, ratio_observed, ratio_max
            ),
...
pub fn apply_container_defaults(container: &mut ContainerResources, items: &[LimitRangeItem]) {
    for item in items
        .iter()
        .filter(|i| i.kind == Some(LimitRangeItemType::Container))
    {
        ...
        .filter(|i| i.kind == Some(LimitRangeItemType::Container))
    {
        ...
        for (resource, ratio_max) in &item.max_limit_request_ratio {
            if let (Some(req), Some(lim)) = (
                container.requests.get(resource),
                container.limits.get(resource),
            ) {
                if *req > 0 {
...
impl ResourceQuotaStore {
    ...
    pub fn put(&self, q: ResourceQuota) {
        let mut g = self.inner.write().unwrap();
        g.retain(|x| {
            !(x.tenant_id == q.tenant_id && x.namespace == q.namespace && x.name == q.name)
        });
        g.push(q);
    }
    ...
    pub fn observe(&self, tenant: &str, namespace: &str, name: &str, resource: &str, delta: i64) {
        ...
        if let Some(q) = g
            .iter_mut()
            .find(|x| x.tenant_id == tenant && x.namespace == namespace && x.name == name)
        {
...
impl std::fmt::Display for QuotaError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        write!(
            f,
            "quota {} exceeded for {}: used {} + want {} > hard {}",
...
pub fn extract_quota_requests(r: &Resource) -> HashMap<String, i64> {
    ...
    match r {
        Resource::Pod(_) => {
            m.insert("pods".into(), 1);
        }
        Resource::ConfigMap(_) => {
            m.insert("configmaps".into(), 1);
        }
        Resource::Secret(_) => {
            m.insert("secrets".into(), 1);
        }
        Resource::Service(_) => {
            m.insert("services".into(), 1);
        }
...
impl LimitRangerPlugin {
    pub fn new(ranges: Arc<LimitRangeStore>) -> Self {
        Self { ranges }
    }
}
