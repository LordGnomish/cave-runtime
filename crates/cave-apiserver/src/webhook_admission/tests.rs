...
fn make_input<'a>(
    ns: &'a str,
    op: &'a Operation,
    obj: &'a HashMap<String, String>,
    ns_l: &'a HashMap<String, String>,
) -> MatchInput<'a> {
    MatchInput {
        group: "",
        version: "v1",
...
fn vwc(
    name: &str,
    tenant: &str,
    hooks: Vec<ValidatingWebhookSpec>,
...
fn client_config_requires_url_or_service() {
    ...
    assert_eq!(
        ...
        Err(WebhookValidationError::MissingClient)
    );
}
...
fn client_config_url_and_service_conflict() {
    ...
    assert_eq!(
        validate_client_config(&c),
        Err(WebhookValidationError::ConflictingClient)
    );
}
...
fn client_config_url_must_be_https() {
    ...
    assert_eq!(
        validate_client_config(&c),
        Err(WebhookValidationError::UrlNotHttps)
    );
}
...
fn client_config_service_port_out_of_range() {
    ...
    assert_eq!(
        validate_client_config(&c),
        Err(WebhookValidationError::BadPort)
    );
}
...
fn mutating_webhook_rejects_zero_timeout() {
    ...
    assert_eq!(
        validate_mutating_webhook(&h),
        Err(WebhookValidationError::BadTimeout)
    );
}
...
fn rule_filters_by_operation() {
    ...
    assert!(rule_matches(
        &rule,
        &make_input("default", &op_c, &empty, &empty)
    ));
    assert!(!rule_matches(
        &rule,
        &make_input("default", &op_d, &empty, &empty)
    ));
}
...
fn rule_scope_namespaced_filters_cluster_resource() {
    ...
    assert!(rule_matches(
        &rule,
        &make_input("default", &op, &empty, &empty)
    ));
...
#[test]
fn panic_client_returns_dial_failed() {
    ...
    let err = c
        ...
        .unwrap_err();
    assert_eq!(err, WebhookCallError::DialFailed);
}
...
fn mutating_dispatch_accumulates_patches() {
    ...
    d.upsert(mwc(
        "c",
        "acme",
        vec![mhook("h1", "https://x"), mhook("h2", "https://x")],
    ));
    let mut req = req(Operation::Create, "ns", "acme");
    let labs = HashMap::new();
    let op = Operation::Create;
...
fn validating_dispatch_denies_on_first_deny() {
    ...
    d.upsert(vwc(
        "c",
        "acme",
        vec![vhook("v1", "https://x"), vhook("v2", "https://x")],
    ));
    let r = req(Operation::Create, "ns", "acme");
    let labs = HashMap::new();
    let op = Operation::Create;
