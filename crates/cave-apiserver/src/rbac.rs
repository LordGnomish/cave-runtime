...
impl RbacAuthorizer {
    pub fn new() -> Self {
        Self {
            inner: Mutex::new(RbacInner::default()),
        }
    }
    ...
    pub fn upsert_role(&self, role: Role) {
        ...
        if role.namespace.is_empty() {
            inner
                .cluster_roles
                .insert((role.tenant_id.clone(), role.name.clone()), role);
        } else {
            inner.roles.insert(
                (
    ...
    pub fn review(&self, mut sar: SubjectAccessReview) -> SubjectAccessReview {
        ...
        if let Some(nra) = sar.spec.non_resource_attributes.clone() {
            if sar.spec.resource_attributes.is_some() {
                sar.status = SubjectAccessReviewStatus {
                    allowed: false,
                    denied: true,
                    reason: "exactly one of resource_attributes / non_resource_attributes required"
                        .into(),
        ...
        for b in inner.bindings.iter() {
            ...
            let role: Option<&Role> = match b.role_kind.as_str() {
                "ClusterRole" => inner
                    .cluster_roles
                    .get(&(b.tenant_id.clone(), b.role_name.clone())),
                "Role" => inner.roles.get(&(
                    b.tenant_id.clone(),
                    b.namespace.clone(),
                    b.role_name.clone(),
                )),
        ...
        for b in inner.bindings.iter() {
            ...
            if non_resource_rule_allows(&role.rules, &nra) {
                sar.status = SubjectAccessReviewStatus {
                    ...
                    denied: false,
                    reason: format!(
                        "RBAC: allowed (non-resource) by ClusterRoleBinding {}",
                        b.name
                    ),
                    evaluation_error: String::new(),
                };
...
fn rule_allows(rules: &[PolicyRule], attrs: &ResourceAttributes) -> bool {
    for r in rules {
        ...
        let res_ok = if attrs.subresource.is_empty() {
            ...
            r.resources
                .iter()
                .any(|x| x == "*" || x == &combined || x == &format!("{}/*", attrs.resource))
                || r.subresources
                    .iter()
                    .any(|s| s == "*" || s == &attrs.subresource)
        };
        let verb_ok = r.verbs.iter().any(|v| v == "*" || v == &attrs.verb);
        let name_ok =
            r.resource_names.is_empty() || r.resource_names.iter().any(|n| n == &attrs.name);
...
mod tests {
    ...
    fn test_resource_name_restricts_match_to_named_object() {
        ...
        auth.upsert_binding(Binding {
            tenant_id: "acme".into(),
            namespace: "default".into(),
            name: "named-bind".into(),
            subjects: vec![user_sub("alice")],
            role_kind: "Role".into(),
            role_name: "named".into(),
        });
        ...
        assert_eq!(
            r_no.spec.tenant_id, "acme",
            "tenant_id invariant on deny path"
        );
    }
    ...
    fn test_service_account_cannot_authorize_across_tenants() {
        ...
        assert_eq!(
            r.spec.tenant_id, "globex",
            "tenant_id invariant: SAR tenant retained"
        );
    }
    ...
    fn test_non_resource_url_grants_exact_path_via_cluster_role_binding() {
        ...
        auth.upsert_binding(Binding {
            tenant_id: "acme".into(),
            namespace: "".into(),
            name: "alice-metrics".into(),
            subjects: vec![user_sub("alice")],
            role_kind: "ClusterRole".into(),
            role_name: "metrics-reader".into(),
        });
        ...
        assert!(
            r.status.reason.contains("non-resource"),
    ...
    fn test_subresource_pods_exec_match_grants_create() {
        ...
        auth.upsert_binding(Binding {
            tenant_id: "acme".into(),
            namespace: "default".into(),
            name: "alice-exec".into(),
            subjects: vec![user_sub("alice")],
            role_kind: "Role".into(),
            role_name: "exec-er".into(),
        });
    ...
    fn test_subresource_wildcard_pods_star_grants_status_and_log() {
        ...
        auth.upsert_binding(Binding {
            tenant_id: "acme".into(),
            namespace: "default".into(),
            name: "alice-pods-sub".into(),
            subjects: vec![user_sub("alice")],
            role_kind: "Role".into(),
            role_name: "pods-all-sub".into(),
        });
        for sub in ["status", "log", "exec"] {
            ...
            assert!(
                r.status.allowed,
                "pods/{} should be allowed under pods/* rule",
                sub
            );
    ...
    fn test_sar_rejects_both_resource_and_non_resource_attributes() {
        ...
        let r = auth.review(sar);
        assert!(
            !r.status.allowed && r.status.denied,
