...
//! Charter v2 self-audit — cave-apiserver's own `parity.manifest.toml` must
//! carry an honest measured `fill_ratio` against kubernetes/kubernetes v1.36.0,
//! a pinned `source_sha` for reproducibility, the 2026-05-18 FINALIZE
//! audit date, and structurally consistent counts.
//!
//! The previous manifest already carried measured `fill_ratio = 0.8824`
//! (mapped 26 + partial 1 + skipped 18 + unmapped 6 = 51 packages, measured
//! audit 2026-05-12 + CEL evaluator MVP), but the `[upstream]` block was
//! missing `source_sha` and `last_audit` lagged. This file is the RED
...
//! field surfaces as a localised test failure.
...
const FLOOR_MAPPED: usize = 26;
const FLOOR_RS_FILES: usize = 50;
...
fn upstream_source_sha_is_present_and_matches_version() {
    ...
    assert!(
        ...
        "manifest [upstream] source_sha must be set for reproducibility \
         (got {:?}). Charter v2 close-out: pin to the {} tag tree-ish.",
        sha,
        UPSTREAM_VERSION
    );
    assert_eq!(
...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-apiserver measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn parity_honest_ratio_does_not_exceed_fill() {
    // Honest_ratio measures behavioural depth (counts partials at fractional
    // weight); fill_ratio counts mapped+skipped at full weight. Honest <= fill
    // is the structural invariant. Equality is the gold standard but not
    // required for close-out — partials honestly account for sub-package depth.
    let m = manifest_text();
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-apiserver IS a parity surface vs kubernetes/kubernetes — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-apiserver; got {}",
        FLOOR_RS_FILES,
        total
    );
}
