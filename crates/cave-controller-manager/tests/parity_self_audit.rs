...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-controller-manager measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-controller-manager IS a parity surface vs kubernetes/kubernetes — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-controller-manager; got {}",
        FLOOR_RS_FILES,
        total
    );
}
