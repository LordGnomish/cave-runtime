...
//
//! Charter v2 self-audit — cave-oncall must carry an honest, measured
//! `fill_ratio` against upstream grafana/oncall v1.10.0, a pinned
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "[parity] fill_ratio must be >= {} (Burak spec — cave-oncall MVP floor), got {}",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
