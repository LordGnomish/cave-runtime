...
use super::remote_write::{
    MatchType, ProtoLabel, ProtoSample, ProtoTimeSeries, QueryResult as ProtoQueryResult,
    ReadRequest, ReadResponse, decode_write_request, encode_write_request,
};
...
pub fn execute_read(req: ReadRequest, tsdb: &Tsdb) -> Result<ReadResponse> {
    ...
    for query in req.queries {
        ...
        let matchers: Vec<LabelMatcher> = query
            .matchers
            .into_iter()
            .filter_map(|m| match m.r#type {
        ...
        let timeseries: Vec<ProtoTimeSeries> = series
            .into_iter()
            .map(|(labels, samps)| ProtoTimeSeries {
                labels: labels
                    .iter()
                    .map(|(k, v)| ProtoLabel {
                        name: k.to_string(),
