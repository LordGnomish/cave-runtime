...
//
//! Charter v2 self-audit — cave-metrics must carry an honest, measured
//! `fill_ratio` against upstream prometheus/prometheus v3.3.0, a pinned
...
// ─── Assertion 3: fill_ratio is positive and meets MVP floor ─────────────────
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "[parity] fill_ratio must be >= {} (Burak spec — cave-metrics MVP floor), got {}",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
// ─── Assertion 5: cave-metrics is a workspace member ─────────────────────────
...
fn assertion_6_agpl_spdx_header_coverage() {
    ...
    assert!(
        missing.is_empty(),
        "{} of {} .rs files in cave-metrics missing AGPL SPDX header: {:?}",
        missing.len(),
        total,
        missing
    );
...
fn assertion_9_parity_index_json_consistency() {
    ...
    let i = idx
        .find("\"cave-metrics\":")
        .expect("docs/parity/parity-index.json must contain cave-metrics entry");
    let block_end = idx[i..]
        .find("\n    }")
        .expect("cave-metrics block must close with `\\n    }`");
    let block = &idx[i..i + block_end];

    let pr_line = block
        .lines()
        .find(|l| l.contains("\"parity_ratio\":"))
        .expect("cave-metrics block must contain parity_ratio");
    ...
    assert!(
        ...
        "parity-index.json cave-metrics entry must declare parity_ratio_source = \"manifest\""
    );
}
