...
//
//! Charter v2 self-audit — cave-dashboard must carry an honest, measured
//! `fill_ratio` against upstream grafana/grafana v11.5.0, a pinned
...
//! four missing manifest fields.
//!
//! Note: this crate ports grafana/grafana; in Burak's 2026-05-19 obs-stack
//! close-out it is referred to as "cave-grafana". cave-dashboard is the
//! existing workspace member (12 .rs files); no duplicate scaffold was
//! created.
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "[parity] fill_ratio must be >= {} (Burak spec — cave-dashboard MVP floor), got {}",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
