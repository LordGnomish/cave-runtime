...
fn jwk(x: &str) -> Jwk {
    Jwk::OKP { crv: "Ed25519".into(), x: x.into() }
}
...
fn new_account_dedupes_on_jwk_thumbprint() {
    ...
    let id1 = s.new_account(TENANT_A, jwk("aaaa"),
        vec![format!("mailto:ops@{}.test", TENANT_A)], true, None).unwrap();
    let id2 = s.new_account(TENANT_A, jwk("aaaa"),
        vec![format!("mailto:ops@{}.test", TENANT_A)], true, None).unwrap();
    assert_eq!(id1, id2);
    assert_eq!(s.account_count(), 1);

    // Same JWK under a DIFFERENT tenant ⇒ separate account.
    let id3 = s.new_account(TENANT_B, jwk("aaaa"),
        vec![format!("mailto:ops@{}.test", TENANT_B)], true, None).unwrap();
    assert_ne!(id1, id3);
...
fn orders_are_tenant_scoped_and_block_cross_tenant_lookup() {
    let mut s = AcmeServer::new();
    let acct_a = s.new_account(TENANT_A, jwk("aaaa"),
        vec![format!("mailto:ops@{}.test", TENANT_A)], true, None).unwrap();
    let order_id = s.new_order(TENANT_A, &acct_a,
        vec![Identifier::dns(format!("svc.{}.cave-runtime.test", TENANT_A))]).unwrap();
    ...
    let err = s.finalize_order(TENANT_B, &order_id, "https://x").unwrap_err();
    assert!(matches!(err, AcmeError::CrossTenantDenied { .. }));
}
