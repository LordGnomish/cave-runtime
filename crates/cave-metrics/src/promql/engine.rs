...
impl Engine {
    ...
    pub fn eval_instant(&self, expr: &Expr, ts_ms: i64) -> Result<QueryResult> {
        match expr {
            ...
            Expr::VectorSelector(vs) => {
                ...
                let pairs = self.tsdb.select_at(&vs.matchers, eval_ts, DEFAULT_LOOKBACK_MS);
                Ok(QueryResult::InstantVector(
                    pairs.into_iter().map(|(labels, s)| (labels, s.value)).collect()
                ))
            }
            ...
            Expr::Unary(u) => {
                match self.eval_instant(&u.expr, ts_ms)? {
                    ...
                    other => Err(MetricsError::Eval(format!("unary - cannot apply to {:?}", other))),
                }
            }
    ...
    fn eval_binary(&self, b: &BinaryExpr, ts_ms: i64) -> Result<QueryResult> {
            ...
            if let QueryResult::InstantVector(iv) = rhs {
                return Ok(QueryResult::InstantVector(
                    iv.into_iter().map(|(l, v)| {
                        ...
                        if b.op.is_comparison() && !b.return_bool {
                            if val != 0.0 { (l, *scalar) } else { (l, f64::NAN) }
                        } else {
                            (l, val)
                        }
                    }).filter(|(_, v)| !v.is_nan()).collect()
        ...
        Err(MetricsError::Eval(format!("binary op {:?} on incompatible types", b.op)))
    }
    ...
    fn binary_vector_op(
        ...
        let rhs_map: HashMap<Labels, f64> = rhs.into_iter()
            .map(|(labels, val)| {
    ...
    fn set_op(&self, op: BinaryOp, lhs: Vec<(Labels, f64)>, rhs: Vec<(Labels, f64)>) -> Result<QueryResult> {
        let rhs_set: std::collections::HashSet<Labels> = rhs.iter().map(|(l, _)| l.without_name()).collect();
        let out = match op {
            BinaryOp::And => lhs.into_iter()
                .filter(|(l, _)| rhs_set.contains(&l.without_name()))
                .collect(),
            BinaryOp::Or => {
                let mut res = lhs;
                let lhs_set: std::collections::HashSet<Labels> = res.iter().map(|(l, _)| l.without_name()).collect();
                for (l, v) in rhs {
                    if !lhs_set.contains(&l.without_name()) { res.push((l, v)); }
                }
                res
            }
            BinaryOp::Unless => lhs.into_iter()
                .filter(|(l, _)| !rhs_set.contains(&l.without_name()))
                .collect(),
            _ => unreachable!(),
        };
    ...
    fn eval_aggregate(&self, agg: &AggregateExpr, ts_ms: i64) -> Result<QueryResult> {
        ...
        let iv = match inner {
            ...
            _ => return Err(MetricsError::Eval("aggregation requires an instant vector".into())),
        };
            ...
            let result_val = match agg.op {
                ...
                AggregateOp::Quantile => {
                    let q = match agg.param.as_deref() {
                        ...
                        _ => return Err(MetricsError::Eval("quantile requires a numeric parameter".into())),
                    };
                ...
                AggregateOp::Topk => {
                    let k = match agg.param.as_deref() {
                        ...
                        _ => return Err(MetricsError::Eval("topk requires a numeric parameter".into())),
                    };
                    let top = fns::topk(k, members);
                    for (l, v) in top { out.push((l, v)); }
                    continue;
                }
                AggregateOp::Bottomk => {
                    let k = match agg.param.as_deref() {
                        Some(Expr::NumberLiteral(k)) => *k as usize,
                        _ => return Err(MetricsError::Eval("bottomk requires a numeric parameter".into())),
                    };
                    let bot = fns::bottomk(k, members);
                    for (l, v) in bot { out.push((l, v)); }
                    continue;
                }
    ...
    fn eval_call(&self, call: &CallExpr, ts_ms: i64) -> Result<QueryResult> {
        match call.func.as_str() {
            ...
            | "mad_over_time" => {
                ...
                let iv: Vec<(Labels, f64)> = rv.into_iter().filter_map(|(labels, samps)| {
                    let val = self.apply_range_fn(&call.func, &samps, self.range_ms_from_call(call))?;
                    Some((labels, val))
                }).collect();
            ...
            "quantile_over_time" => {
                let q = match call.args.first() {
                    Some(Expr::NumberLiteral(q)) => *q,
                    _ => return Err(MetricsError::Eval("quantile_over_time: first arg must be a number".into())),
                };
            ...
            "predict_linear" => {
                let t = match call.args.get(1) {
                    ...
                    _ => return Err(MetricsError::Eval("predict_linear: second arg must be a number".into())),
                };
            ...
            "histogram_quantile" => {
                let q = match call.args.first() {
                    Some(Expr::NumberLiteral(q)) => *q,
                    _ => return Err(MetricsError::Eval("histogram_quantile: first arg must be a number".into())),
                };
                // Evaluate the second arg (instant vector of _bucket series)
                let buckets_iv = match self.eval_instant(call.args.get(1).ok_or_else(|| MetricsError::Eval("histogram_quantile: missing arg".into()))?, ts_ms)? {
                    QueryResult::InstantVector(iv) => iv,
                    _ => return Err(MetricsError::Eval("histogram_quantile: arg must be an instant vector".into())),
                };

                // Group by all labels except "le"
                let mut groups: HashMap<Labels, Vec<(f64, f64)>> = HashMap::new();
                for (labels, val) in buckets_iv {
                    let le: f64 = labels.get("le").and_then(|v| {
                ...
                let iv: Vec<(Labels, f64)> = groups.into_iter().map(|(labels, buckets)| {
                    (labels, fns::histogram_quantile(q, buckets))
                }).collect();
                Ok(QueryResult::InstantVector(iv))
            }
            ...
            "sgn"   => self.map_iv(call, ts_ms, |v| if v > 0.0 { 1.0 } else if v < 0.0 { -1.0 } else { 0.0 }),
            ...
            "clamp" => {
                let min = match call.args.get(1) { Some(Expr::NumberLiteral(v)) => *v, _ => f64::NEG_INFINITY };
                let max = match call.args.get(2) { Some(Expr::NumberLiteral(v)) => *v, _ => f64::INFINITY };
            ...
            "clamp_max" => {
                let max = match call.args.get(1) { Some(Expr::NumberLiteral(v)) => *v, _ => f64::INFINITY };
            ...
            "label_replace" => {
                ...
                let out: Result<Vec<(Labels, f64)>> = iv.into_iter().map(|(labels, val)| {
            ...
            "label_join" => {
                ...
                let src_labels: Vec<String> = (3..call.args.len())
                    .filter_map(|i| self.string_arg(call, i).ok()).collect();
                let src_refs: Vec<&str> = src_labels.iter().map(|s| s.as_str()).collect();
                let out: Vec<(Labels, f64)> = iv.into_iter().map(|(labels, val)| {
                    (fns::label_join(&labels, &dst, &sep, &src_refs), val)
                }).collect();
                Ok(QueryResult::InstantVector(out))
            }
            ...
            // ── Scalar / time ──────────────────────────────────────────────
            "scalar" => {
                match self.eval_instant(call.args.first().ok_or_else(|| MetricsError::Eval("scalar: missing arg".into()))?, ts_ms)? {
                    QueryResult::InstantVector(iv) if iv.len() == 1 => Ok(QueryResult::Scalar(iv[0].1)),
            ...
            "vector" => {
                let val = match call.args.first() {
                    ...
                    _ => return Err(MetricsError::Eval("vector: first arg must be a number".into())),
                };
            ...
            "timestamp" => {
                let iv = self.eval_iv_first(call, ts_ms)?;
                Ok(QueryResult::InstantVector(iv.into_iter().map(|(l, _)| (l, ts_ms as f64 / 1000.0)).collect()))
            }
    ...
    fn eval_range_vector_arg(&self, call: &CallExpr, ts_ms: i64) -> Result<Vec<(Labels, Vec<Sample>)>> {
        self.eval_range_vector_arg_at(call, 0, ts_ms)
    }

    fn eval_range_vector_arg_at(&self, call: &CallExpr, idx: usize, ts_ms: i64) -> Result<Vec<(Labels, Vec<Sample>)>> {
        let arg = call.args.get(idx).ok_or_else(|| MetricsError::Eval(format!("{}: missing arg {}", call.func, idx)))?;
        match self.eval_instant(arg, ts_ms)? {
            QueryResult::RangeVector(rv) => Ok(rv),
            _ => Err(MetricsError::Eval(format!("{}: arg {} must be a range vector", call.func, idx))),
        }
    }
    ...
    fn eval_iv_first(&self, call: &CallExpr, ts_ms: i64) -> Result<Vec<(Labels, f64)>> {
        let arg = call.args.first().ok_or_else(|| MetricsError::Eval(format!("{}: missing arg", call.func)))?;
        match self.eval_instant(arg, ts_ms)? {
            QueryResult::InstantVector(iv) => Ok(iv),
            QueryResult::Scalar(v) => Ok(vec![(Labels::new(), v)]),
            _ => Err(MetricsError::Eval(format!("{}: arg must be an instant vector", call.func))),
        }
    }

    fn map_iv(&self, call: &CallExpr, ts_ms: i64, f: impl Fn(f64) -> f64) -> Result<QueryResult> {
        let iv = self.eval_iv_first(call, ts_ms)?;
        Ok(QueryResult::InstantVector(iv.into_iter().map(|(l, v)| (l, f(v))).collect()))
    }

    fn map_iv_first(&self, call: &CallExpr, ts_ms: i64, f: impl Fn(f64) -> f64) -> Result<QueryResult> {
    ...
    fn string_arg(&self, call: &CallExpr, idx: usize) -> Result<String> {
        match call.args.get(idx) {
            ...
            _ => Err(MetricsError::Eval(format!("{}: arg {} must be a string", call.func, idx))),
        }
    }
}
...
fn matching_key(labels: &Labels, matching: Option<&VectorMatching>) -> Labels {
    match matching {
        ...
        Some(vm) if vm.on => labels.with_only(&vm.labels.iter().map(|s| s.as_str()).collect::<Vec<_>>()),
