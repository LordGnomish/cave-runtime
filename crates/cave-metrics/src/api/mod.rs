...
pub fn create_router(state: Arc<MetricsState>) -> Router {
    Router::new()
        ...
        .route(
            "/api/v1/query",
            get(query::instant_query).post(query::instant_query),
        )
        .route(
            "/api/v1/query_range",
            get(query::range_query).post(query::range_query),
        )
        .route("/api/v1/query_exemplars", get(query::exemplars))
        // ── Metadata ──────────────────────────────────────────────────────
        .route(
            "/api/v1/labels",
            get(labels::list_labels).post(labels::list_labels),
        )
        .route("/api/v1/label/{name}/values", get(labels::label_values))
        .route(
            "/api/v1/series",
            get(series::list_series).post(series::list_series),
        )
