...
//! Charter v2 self-audit — cave-sbom's own `parity.manifest.toml`
//! must carry the canonical 2026-05-18 close-out fields:
//!
...
//!   6. infra_only == false
//!   7. at least 30 [[files]] mapping blocks
//!   8. mapped + partial + skipped + unmapped == total
//!   9. every .rs file under crates/cave-sbom carries the AGPL SPDX header
//!
...
#[test]
fn upstream_version_is_pinned_v4_11_6() {
    let m = manifest_text();
    let v = extract_after(&m, "\nversion ").or_else(|| extract_after(&m, "\nversion="));
    assert_eq!(
        v.as_deref(),
        Some("v4.11.6"),
        "manifest [upstream] version must pin DependencyTrack v4.11.6 (was {:?})",
        v
    );
}
...
fn upstream_source_sha_is_present() {
    ...
    assert!(
        ...
        "manifest [upstream] sha must be set for reproducibility (got {:?})",
        sha
    );
    // The v4.11.6 tag tree-ish; pinned during the Artifact + Security wave.
    assert_eq!(
        sha.as_deref(),
        Some("128fd0fa01bed9fcb57abffa3b30047c45941415"),
        "sha should match the v4.11.6 tag tree-ish (got {:?})",
        sha
    );
}
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-sbom IS a parity surface vs Dependency-Track — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        total >= 20,
        "expected >= 20 .rs files in cave-sbom; got {}",
        total
    );
}
