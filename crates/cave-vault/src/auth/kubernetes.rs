...
pub async fn login(
    ...
    let store = state.kubernetes_store.read().await;
    let role = store.roles.get(&body.role)
        .ok_or_else(|| VaultError::RoleNotFound(body.role.clone()))?.clone();

    // Decode JWT to extract service account info
    let claims = decode_jwt_payload(&body.jwt)
    ...
    let (sa_namespace, sa_name) = if parts.len() == 4 && parts[0] == "system" && parts[1] == "serviceaccount" {
        (parts[2], parts[3])
    } else {
        return Err(VaultError::Auth("invalid service account subject".into()));
    };

    let name_ok = role.bound_service_account_names.contains(&sa_name.to_string())
        || role.bound_service_account_names.contains(&"*".to_string());
    let ns_ok = role.bound_service_account_namespaces.contains(&sa_namespace.to_string())
        || role.bound_service_account_namespaces.contains(&"*".to_string());
    ...
    let params = CreateTokenParams {
        ...
        metadata: Some({
            ...
            m.insert("service_account_namespace".to_string(), sa_namespace.to_string());
            m
        }),
        ..Default::default()
    };
...
pub fn router(state: Arc<VaultState>) -> Router {
    Router::new()
        .route("/v1/auth/kubernetes/config", post(configure).get(read_config))
        .route("/v1/auth/kubernetes/role", get(list_roles))
        .route("/v1/auth/kubernetes/role/:role_name", post(create_role).get(read_role).delete(delete_role))
