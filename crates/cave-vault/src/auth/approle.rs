...
pub async fn create_role(
    ...
    let role = store
        ...
        .or_insert_with(|| ApproleRole {
            role_name: role_name.clone(),
            ..Default::default()
        });
    if let Some(b) = body.bind_secret_id {
        role.bind_secret_id = b;
    }
    if let Some(ttl) = body.secret_id_ttl.as_deref() {
        role.secret_id_ttl = crate::token::parse_duration(ttl);
    }
    if let Some(n) = body.secret_id_num_uses {
        role.secret_id_num_uses = n;
    }
    if let Some(ttl) = body.token_ttl.as_deref() {
        role.token_ttl = crate::token::parse_duration(ttl);
    }
    if let Some(ttl) = body.token_max_ttl.as_deref() {
        role.token_max_ttl = crate::token::parse_duration(ttl);
    }
    if let Some(p) = body.token_policies {
        role.token_policies = p;
    }
    if let Some(c) = body.token_bound_cidrs {
        role.token_bound_cidrs = c;
    }
    if let Some(period) = body.period.as_deref() {
        role.period = crate::token::parse_duration(period);
    }
...
pub async fn generate_secret_id(
    ...
    store
        .secret_id_by_id
        .insert(secret_id.clone(), (role_name.clone(), accessor.clone()));
    store
        .secret_ids
        .entry(role_name)
        .or_default()
        .insert(accessor.clone(), entry);
...
pub async fn login(
    ...
    if role.bind_secret_id {
        let secret_id = body
            .secret_id
            .ok_or_else(|| VaultError::Auth("secret_id required".into()))?;
        let (rn, accessor) = store
            .secret_id_by_id
            .get(&secret_id)
...
pub fn router(state: Arc<VaultState>) -> Router {
    Router::new()
        ...
        .route(
            ...
            get(read_role_id).post(update_role_id),
        )
        .route(
        ...
        .route(
            ...
            post(lookup_secret_id),
        )
        .route(
            "/v1/auth/approle/role/{role_name}/secret-id/destroy",
            post(destroy_secret_id),
        )
