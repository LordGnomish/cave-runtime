...
use crate::models::{
    ComplianceFramework, ComplianceReport, ControlException, Finding, FindingStatus,
};
use uuid::Uuid;
...
pub fn generate_report(
    ...
    let excepted_control_ids: std::collections::HashSet<Uuid> = exceptions
        .iter()
    ...
    let passed = relevant_findings
        .iter()
        .filter(|f| f.status == FindingStatus::Pass)
        .count();
    let failed = relevant_findings
        .iter()
        .filter(|f| {
            f.status == FindingStatus::Fail && !excepted_control_ids.contains(&f.control_id)
        })
        .count();
    let warned = relevant_findings
        .iter()
        .filter(|f| f.status == FindingStatus::Warn)
        .count();
    let na = relevant_findings
        .iter()
        .filter(|f| f.status == FindingStatus::NotApplicable)
        .count();
    let manual = relevant_findings
        .iter()
