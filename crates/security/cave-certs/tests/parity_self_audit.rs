...
//! Charter v2 self-audit gates for cave-certs.
//!
...
/// G7: upstream version is cert-manager v1.17+ (latest stable 2026).
#[test]
fn g7_upstream_version_recent() {
    let m = read_manifest();
    // cert-manager v1.17.x is the latest stable at time of writing (2026).
    let version_line = m
        .lines()
        .find(|l| l.trim_start().starts_with("version") && l.contains('"'))
        .expect("G7: version line missing");
    let has_recent = version_line.contains("v1.1")
        || version_line.contains("v1.2")
        || version_line.contains("v2.");
    assert!(has_recent, "G7: upstream version too old: {version_line}");
}

/// G8: four-track backend coverage — routes.rs wires cert endpoints.
#[test]
fn g8_routes_handler_exists() {
    ...
    assert!(
        contents.contains("certs") || contents.contains("cert") || contents.contains("health"),
        "G8: routes.rs must expose cave-certs endpoints"
    );
}
