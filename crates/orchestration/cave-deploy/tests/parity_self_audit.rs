...
//! `fill_ratio` against upstream argoproj/argo-cd v3.4.2 (+ Argo CD Image
//! Updater v0.16.0 module), a pinned `source_sha`, the 2026-05-24
...
//! sync + diff + health + rbac + rollout + notification + image_updater
//! public surface reachable through `cave_deploy`.
//!
...
fn assertion_9_deploy_surface_intact() {
    ...
    // 15. Image Updater surface (Argo CD Image Updater v0.16.0 parity)
    use cave_deploy::image_updater::{
        ImageRef, ImageUpdater, RegistryEndpoint, TagCandidate, TagSelector, UpdateStrategy,
    };
    let img = ImageRef::parse("api", "ghcr.io/cave/runtime:1.4.0").unwrap();
    assert_eq!(img.registry, "ghcr.io");
    let sel = TagSelector::Semver {
        constraint: "^1.4".into(),
    };
    let cands = [TagCandidate {
        tag: "1.4.7".into(),
        digest: None,
        pushed_at: None,
    }];
    ...
    up.add_registry(RegistryEndpoint {
        ...
        credential_ref: Some("kv/ghcr".into()),
        jittered: false,
    });
    up.set_policy("api", sel, UpdateStrategy::AnnotationWrite);
    assert!(up.policy("api").is_some());
}
