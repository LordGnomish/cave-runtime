...
//! Charter v2 self-audit — cave-etcd's `parity.manifest.toml` must
//! carry an honest measured `fill_ratio` against etcd-io/etcd v3.6.10,
//! a pinned `source_sha` for reproducibility, the 2026-05-18 FINALIZE
//! audit date, and structurally consistent counts.
//!
//! Previous manifest carried measured `fill_ratio = 0.9155` (mapped 28 +
//! partial 2 + skipped 35 + unmapped 6 = 71 packages, measured audit
//! 2026-05-12) but the `[upstream]` block was missing `source_sha` and
//! `last_audit` lagged. This file is the RED signal for those two gaps.
...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-etcd measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
#[test]
fn parity_last_audit_is_2026_05_30() {
    let m = manifest_text();
    let when = extract_after(&m, "\nlast_audit ").or_else(|| extract_after(&m, "\nlast_audit="));
    assert_eq!(
        when.as_deref(),
        Some("2026-05-30"),
        "[parity] last_audit must reflect the 2026-05-30 honest close-out (raft/tracker port)"
    );
}
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-etcd IS a parity surface vs etcd-io/etcd — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-etcd; got {}",
        FLOOR_RS_FILES,
        total
    );
}
