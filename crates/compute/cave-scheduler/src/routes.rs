...
pub fn create_router(state: Arc<SchedulerState>) -> Router {
    Router::new()
        ...
        .route(
            "/api/scheduler/nodes/{name}",
            get(get_node).delete(unregister_node),
        )
...
async fn register_node(
    State(s): State<Arc<SchedulerState>>,
    Json(node): Json<Node>,
...
async fn get_node(
    ...
    s.nodes
        ...
        .ok_or((StatusCode::NOT_FOUND, format!("node {} not found", name)))
}
