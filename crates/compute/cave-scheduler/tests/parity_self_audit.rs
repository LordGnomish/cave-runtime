...
//! Charter v2 self-audit — cave-scheduler's `parity.manifest.toml` must
...
//!
//! Previous manifest carried measured `fill_ratio = 0.8966` (mapped 19 +
//! partial 0 + skipped 7 + unmapped 3 = 29 packages, measured audit
//! 2026-05-13 batch2 imagelocality) but the `[upstream]` block was missing
...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-scheduler measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn parity_last_audit_is_2026_05_30() {
    ...
    assert_eq!(
        when.as_deref(),
        Some("2026-05-30"),
        "[parity] last_audit must reflect the 2026-05-30 honest-uplift ray (heap + config validation)"
    );
}
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-scheduler IS a parity surface vs kubernetes/kubernetes — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-scheduler; got {}",
        FLOOR_RS_FILES,
        total
    );
}
