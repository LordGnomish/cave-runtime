...
fn empty_rule_matches_resource_request() {
    ...
    assert!(rule_matches(
        &r,
        &resource_input("alice", "get", "", "pods", "default")
    ));
}
...
fn empty_rule_does_not_match_non_resource_request() {
    let r = PolicyRule::default();
    assert!(
        !rule_matches(&r, &nrurl_input("/healthz")),
...
fn user_filter() {
    ...
    assert!(!rule_matches(
        &r,
        &resource_input("bob", "get", "", "pods", "default")
    ));
}
...
fn user_groups_filter_intersection() {
    let r = PolicyRule {
        user_groups: vec!["devs".into(), "ops".into()],
        ..Default::default()
    };
...
fn verbs_filter() {
    ...
    assert!(rule_matches(
        &r,
        &resource_input("a", "list", "", "p", "ns")
    ));
    assert!(!rule_matches(
        &r,
        &resource_input("a", "delete", "", "p", "ns")
    ));
}
...
fn namespaces_filter() {
    ...
    assert!(rule_matches(
        &r,
        &resource_input("a", "get", "", "p", "default")
    ));
    assert!(!rule_matches(
        &r,
        &resource_input("a", "get", "", "p", "kube-system")
    ));
}
...
fn resources_filter_by_group() {
    ...
    assert!(rule_matches(
        &r,
        &resource_input("a", "get", "apps", "deployments", "default")
    ));
    assert!(!rule_matches(
        &r,
        &resource_input("a", "get", "", "pods", "default")
    ));
}
...
fn resources_filter_wildcard_resource() {
    ...
    assert!(rule_matches(
        &r,
        &resource_input("a", "get", "apps", "statefulsets", "default")
    ));
}
...
fn resources_filter_subresource_qualified_name() {
    ...
    input.subresource = "";
    assert!(
        !rule_matches(&r, &input),
        "main resource not in resource_names list"
    );
}
...
fn nrurl_single_level_wildcard() {
    ...
    assert!(
        !nrurl_matches("/api/*", "/api/v1/pods"),
        "second level not matched"
    );
...
fn stage_emitter_redacts_metadata_drops_request_response() {
    ...
    let ev = r.unwrap();
    assert!(
        ev.request_object.is_none(),
        "Metadata level drops request body"
    );
...
fn stage_emitter_request_response_keeps_both() {
    let mut e = StageEmitter::new(
        "aid".into(),
        dec(AuditLevel::RequestResponse),
        "acme".into(),
    );
    let r = e.build(
        AuditStage::ResponseComplete,
...
fn stage_emitter_omit_managed_fields_strips() {
    ...
    assert!(
        req["metadata"]["managedFields"].is_null(),
