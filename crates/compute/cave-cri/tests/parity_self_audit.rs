...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-cri measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn parity_last_audit_is_2026_05_30() {
    ...
    assert_eq!(
        when.as_deref(),
        Some("2026-05-30"),
        "[parity] last_audit must reflect the 2026-05-30 honest uplift (core/diff production path)"
    );
}
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-cri IS a parity surface vs containerd/runc — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-cri; got {}",
        FLOOR_RS_FILES,
        total
    );
}
