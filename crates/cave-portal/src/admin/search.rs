//! `/admin/search` view — search resource browser.
...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<SearchIndex>, SearchViewError> {
    ...
    Ok(scope(&state.search_indexes.read().unwrap(), &ctx.tenant, |r| &r.tenant)
        .into_iter().cloned().collect())
}

pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, SearchViewError> {
    let rows = list_records(state, ctx)?;
    let table_rows: Vec<Vec<String>> = rows.iter().map(|r| vec![r.name.clone(), r.doc_count.to_string(), r.size_bytes.to_string(), r.status.into()]).collect();
    let body = format!(
        r#"<section><h2 class="text-lg font-semibold mb-2">Search ({n})</h2>{tbl}</section>"#,
        n = rows.len(),
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!("plugins/search/src/components/IndexesList.tsx", "IndexesList", "acme");
    ...
    fn render_contains_owner_row() {
        let (_c, _t) = portal_test_ctx!("plugins/search/src/components/IndexesList.tsx", "RenderOwner", "acme");
    ...
    fn render_excludes_evil_row() {
        let (_c, _t) = portal_test_ctx!("plugins/search/src/components/IndexesList.tsx", "RenderEvil", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::SearchRead])).unwrap();
    ...
    fn render_shows_acme_count() {
        let (_c, _t) = portal_test_ctx!("plugins/search/src/components/IndexesList.tsx", "Count", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::SearchRead])).unwrap();
