...
use crate::admin::state::{AdminState, AiModelMetric, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<AiModelMetric>, AiObsViewError> {
    ctx.authorise(Permission::AiObsRead)?;
    Ok(
        scope(&state.ai_model_metrics.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, AiObsViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/ai-obs",
        &format!("ai-obs · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite =
    Cite::backstage("plugins/ai-obs/src/components/ModelsList.tsx", "ModelsList");
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/ai-obs/src/components/ModelsList.tsx",
            "ModelsList",
            "acme"
        );
