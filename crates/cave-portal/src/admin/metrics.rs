...
use crate::admin::state::{AdminState, MetricSeries, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<MetricSeries>, MetricsViewError> {
    ctx.authorise(Permission::MetricsRead)?;
    let mut rows: Vec<MetricSeries> =
        scope(&state.metric_series.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn group_by_scraper(rows: &[MetricSeries]) -> Vec<(String, usize)> {
    ...
    for r in rows {
        *acc.entry(r.scraper.clone()).or_insert(0) += 1;
    }
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, MetricsViewError> {
  ...
  {tbl}
        ...
        chips = chips,
        tbl = table(
            &["name", "scraper", "samples", "retention_days"],
            &table_rows
        ),
    );
    Ok(page_shell_full(
        ctx,
        "/admin/metrics",
        &format!("metrics · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/metrics/src/components/SeriesList.tsx",
    "SeriesList",
);
