...
use crate::admin::state::{AdminState, DnsZone, scope};
...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<DnsZone>, DnsViewError> {
    ...
    Ok(
        scope(&state.dns_zones.read().unwrap(), &ctx.tenant, |r| &r.tenant)
            .into_iter()
            .cloned()
            .collect(),
    )
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, DnsViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/dns",
        &format!("dns · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/dns/src/components/ZonesList.tsx",
            "ZonesList",
            "acme"
        );
    ...
    fn render_shows_acme_count() {
        let (_c, _t) =
            portal_test_ctx!("plugins/dns/src/components/ZonesList.tsx", "Count", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::DnsRead])).unwrap();
