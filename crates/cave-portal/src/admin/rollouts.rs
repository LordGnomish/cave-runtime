//! `/admin/rollouts` view — rollouts resource browser.
...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<RolloutStatus>, RolloutsViewError> {
    ...
    Ok(scope(&state.rollout_statuses.read().unwrap(), &ctx.tenant, |r| &r.tenant)
        .into_iter().cloned().collect())
}

pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, RolloutsViewError> {
    let rows = list_records(state, ctx)?;
    let table_rows: Vec<Vec<String>> = rows.iter().map(|r| vec![r.name.clone(), r.strategy.into(), r.traffic_pct.to_string(), r.state.into()]).collect();
    let body = format!(
        r#"<section><h2 class="text-lg font-semibold mb-2">Rollouts ({n})</h2>{tbl}</section>"#,
        n = rows.len(),
        tbl = table(&["name", "strategy", "traffic_pct", "state"], &table_rows),
    );
    Ok(page_shell(&format!("rollouts · {}", escape(ctx.tenant.as_str())), &body))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!("plugins/rollouts/src/components/StatusList.tsx", "StatusList", "acme");
        let s = AdminState::seeded();
        let r = list_records(&s, &ctx(&[Permission::RolloutsRead])).unwrap();
        assert_eq!(r.len(), 2);
    ...
    fn render_contains_owner_row() {
        let (_c, _t) = portal_test_ctx!("plugins/rollouts/src/components/StatusList.tsx", "RenderOwner", "acme");
    ...
    fn render_excludes_evil_row() {
        let (_c, _t) = portal_test_ctx!("plugins/rollouts/src/components/StatusList.tsx", "RenderEvil", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::RolloutsRead])).unwrap();
    ...
    fn render_shows_acme_count() {
        let (_c, _t) = portal_test_ctx!("plugins/rollouts/src/components/StatusList.tsx", "Count", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::RolloutsRead])).unwrap();
