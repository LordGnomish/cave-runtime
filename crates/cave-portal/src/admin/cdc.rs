...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<CdcPipeline>, CdcViewError> {
    ctx.authorise(Permission::CdcRead)?;
    Ok(scope(&state.cdc_pipelines.read().unwrap(), &ctx.tenant, |r| &r.tenant)
        .into_iter().cloned().collect())
}

pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, CdcViewError> {
    let rows = list_records(state, ctx)?;
    let table_rows: Vec<Vec<String>> = rows.iter().map(|r| vec![r.name.clone(), r.source.clone(), r.sink.clone(), r.state.into()]).collect();
    let body = format!(
...
#[allow(dead_code)]
const FILE_CITE: Cite = Cite::backstage("plugins/cdc/src/components/PipelinesList.tsx", "PipelinesList");
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!("plugins/cdc/src/components/PipelinesList.tsx", "PipelinesList", "acme");
    ...
    fn render_contains_owner_row() {
        let (_c, _t) = portal_test_ctx!("plugins/cdc/src/components/PipelinesList.tsx", "RenderOwner", "acme");
    ...
    fn render_excludes_evil_row() {
        let (_c, _t) = portal_test_ctx!("plugins/cdc/src/components/PipelinesList.tsx", "RenderEvil", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::CdcRead])).unwrap();
    ...
    fn render_shows_acme_count() {
        let (_c, _t) = portal_test_ctx!("plugins/cdc/src/components/PipelinesList.tsx", "Count", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::CdcRead])).unwrap();
