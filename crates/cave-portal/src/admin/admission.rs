...
use crate::admin::state::{AdminState, AdmissionDecision, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<AdmissionDecision>, AdmissionViewViewError> {
    ctx.authorise(Permission::AdmissionRead)?;
    Ok(scope(
        &state.admission_decisions.read().unwrap(),
        &ctx.tenant,
        |r| &r.tenant,
    )
    .into_iter()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, AdmissionViewViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/admission",
        &format!("admission · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/admission/src/components/DecisionsList.tsx",
    "DecisionsList",
);
