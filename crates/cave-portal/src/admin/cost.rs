...
use crate::admin::state::{AdminState, CostReport, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<CostReport>, CostViewError> {
    ctx.authorise(Permission::CostRead)?;
    Ok(
        scope(&state.cost_reports.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, CostViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/cost",
        &format!("cost · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite =
    Cite::backstage("plugins/cost/src/components/ReportsList.tsx", "ReportsList");
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/cost/src/components/ReportsList.tsx",
            "ReportsList",
            "acme"
        );
