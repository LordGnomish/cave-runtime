...
use crate::admin::state::{AdminState, DastScan, scope};
...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<DastScan>, DastViewError> {
    ...
    Ok(scope(&state.dast_scans.read().unwrap(), &ctx.tenant, |r| {
        &r.tenant
    })
    .into_iter()
    .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, DastViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/dast",
        &format!("dast · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/dast/src/components/ScansList.tsx",
            "ScansList",
            "acme"
        );
    ...
    fn render_shows_acme_count() {
        let (_c, _t) =
            portal_test_ctx!("plugins/dast/src/components/ScansList.tsx", "Count", "acme");
        let html = render(&AdminState::seeded(), &ctx(&[Permission::DastRead])).unwrap();
