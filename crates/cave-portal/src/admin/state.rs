...
pub struct AdminState {
    ...
    pub keda_scaled_object_details: RwLock<Vec<crate::admin::keda::types::KedaScaledObjectDetail>>,
...
impl AdminState {
    ...
    pub fn empty() -> Self {
        Self {
            ...
            cluster_live: std::sync::Arc::new(crate::admin::cluster_live::ClusterLiveState::new()),
    ...
    pub fn seeded() -> Self {
        ...
        s.etcd_leases.write().unwrap().extend([
            EtcdLease {
                ...
                lease_id: 1001,
                ttl_seconds: 30,
                keys: vec!["/state/leader".into()],
            },
            EtcdLease {
                tenant: evil.clone(),
                lease_id: 9999,
                ttl_seconds: 60,
                keys: vec!["/secret/x".into()],
            },
        ]);
        ...
        s.mesh_authz.write().unwrap().extend([
            MeshAuthzPolicy {
                ...
                name: "allow-web".into(),
                action: "Allow",
                principal_glob: "spiffe://*/ns/acme/sa/*".into(),
            },
            MeshAuthzPolicy {
                tenant: evil.clone(),
                name: "evil-allow".into(),
                action: "Allow",
                principal_glob: "*".into(),
            },
        ]);
        ...
        s.vault_secrets.write().unwrap().extend([
            ...
            VaultSecretMeta {
                ...
                path: "kv/api".into(),
                version: 1,
                created_unix: 1_000_500,
            },
            VaultSecretMeta {
                tenant: evil.clone(),
                path: "kv/secret".into(),
                version: 7,
                created_unix: 999_999,
            },
        ]);
        ...
        s.vault_engines.write().unwrap().extend([
            ...
            VaultSecretsEngine {
                ...
                version: 1,
                default_lease_ttl_s: 86400,
                enabled: true,
            },
            VaultSecretsEngine {
                tenant: acme.clone(),
                path: "database/".into(),
                engine_type: "database".into(),
                version: 1,
        ...
        s.vault_auth_methods.write().unwrap().extend([
            ...
            VaultAuthMethod {
                ...
                accessor: "auth_userpass_b2".into(),
                default_lease_ttl_s: 3600,
                enabled: true,
            },
            VaultAuthMethod {
                ...
                method_type: "kubernetes".into(),
                accessor: "auth_kubernetes_c3".into(),
            ...
            VaultAuthMethod {
                tenant: acme.clone(),
                path: "approle/".into(),
                method_type: "approle".into(),
                accessor: "auth_approle_d4".into(),
                default_lease_ttl_s: 1800,
                enabled: true,
            },
            VaultAuthMethod {
                tenant: acme.clone(),
                path: "oidc/".into(),
                method_type: "oidc".into(),
                accessor: "auth_oidc_e5".into(),
                default_lease_ttl_s: 3600,
                enabled: false,
            },
            VaultAuthMethod {
        ...
        s.recent_activity.write().unwrap().extend([
            ActivityEntry {
                ...
                when_unix: 1_000_100,
                kind: "deploy",
                summary: "deployed web v17".into(),
            },
            ActivityEntry {
                tenant: acme.clone(),
                when_unix: 1_000_200,
                kind: "policy",
                summary: "updated AuthorizationPolicy allow-web".into(),
            },
            ActivityEntry {
                tenant: evil.clone(),
                when_unix: 1_000_300,
                kind: "deploy",
                summary: "deployed evil-web v1".into(),
            },
        ]);
        ...
        s.scheduler_nodes.write().unwrap().extend([
            ...
            SchedulerNode {
                ...
                allocatable_mem_mib: 8192,
                taints: vec!["NoSchedule=cordoned".into()],
            },
            SchedulerNode {
                tenant: evil.clone(),
                name: "evil-node".into(),
        ...
        s.scheduler_policies.write().unwrap().extend([
            SchedulerPolicy {
                tenant: acme.clone(),
                name: "least-utilised".into(),
                predicate: "cpu<70".into(),
                weight: 5,
            },
            SchedulerPolicy {
                tenant: evil.clone(),
                name: "evil-pin".into(),
                predicate: "host==evil-node".into(),
                weight: 10,
            },
        ]);
        s.controller_leases.write().unwrap().extend([
            ControllerLease {
                tenant: acme.clone(),
                controller: "deployment".into(),
                leader_id: "ctl-1".into(),
                renewals: 17,
                expires_unix: 1_001_000,
            },
            ControllerLease {
                tenant: acme.clone(),
                controller: "replicaset".into(),
                leader_id: "ctl-1".into(),
                renewals: 18,
                expires_unix: 1_001_010,
            },
            ControllerLease {
                tenant: evil.clone(),
                controller: "evil-loop".into(),
                leader_id: "evil-1".into(),
                renewals: 1,
                expires_unix: 1_001_020,
            },
        ]);
        ...
        s.cloud_volumes.write().unwrap().extend([
            CloudVolume {
                ...
                size_gb: 50,
                attached_node: Some("node-a".into()),
            },
            CloudVolume {
                tenant: acme.clone(),
                id: "vol-2".into(),
        ...
        s.kamaji_tcps.write().unwrap().extend([
            KamajiTcp {
                tenant: acme.clone(),
                name: "tcp-prod".into(),
                k8s_version: "1.31.2".into(),
                ready_replicas: 3,
                desired_replicas: 3,
            },
            KamajiTcp {
                tenant: acme.clone(),
                name: "tcp-staging".into(),
                k8s_version: "1.31.0".into(),
                ready_replicas: 2,
                desired_replicas: 3,
            },
            KamajiTcp {
        ...
        s.rdbms_operator_clusters.write().unwrap().extend([
            RdbmsOperatorCluster {
                tenant: acme.clone(),
                name: "primary-prod".into(),
                upstream_kind: "CNPG",
                version: "1.24.0".into(),
                instances: 3,
                primary_pod: "primary-prod-1".into(),
                replication_lag_bytes: 8192,
                replication_state: "InSync",
            },
            RdbmsOperatorCluster {
                tenant: acme.clone(),
                name: "analytics".into(),
                ...
                version: "1.24.0".into(),
                instances: 2,
                primary_pod: "analytics-1".into(),
                replication_lag_bytes: 4_194_304,
                replication_state: "Catchup",
            },
            RdbmsOperatorCluster {
                tenant: evil.clone(),
                name: "evil-cluster".into(),
                upstream_kind: "CNPG",
                version: "1.10.0".into(),
                instances: 1,
                primary_pod: "evil-1".into(),
        ...
        s.rdbms_operator_backups.write().unwrap().extend([
            RdbmsOperatorBackup {
                tenant: acme.clone(),
                cluster: "primary-prod".into(),
                backup_id: "bk-2026-05-10-01".into(),
                started_unix: 1_001_500,
                finished_unix: Some(1_001_700),
                size_mib: 4096,
                state: "Completed",
            },
            RdbmsOperatorBackup {
                tenant: acme.clone(),
                cluster: "primary-prod".into(),
                backup_id: "bk-2026-05-10-02".into(),
                started_unix: 1_002_000,
                finished_unix: None,
                size_mib: 0,
                state: "Running",
            },
            RdbmsOperatorBackup {
                tenant: evil.clone(),
                cluster: "evil-cluster".into(),
                backup_id: "evil-bk-1".into(),
                started_unix: 1_000_000,
                finished_unix: Some(1_000_100),
                size_mib: 16,
                state: "Completed",
            },
        ]);
        ...
        s.streams_consumer_groups.write().unwrap().extend([
            StreamsConsumerGroup {
                tenant: acme.clone(),
                group_id: "orders-consumer".into(),
                topic: "orders".into(),
                members: 4,
                current_offset: 9_500,
                log_end_offset: 10_000,
                state: "Stable",
            },
            StreamsConsumerGroup {
                tenant: acme.clone(),
                group_id: "events-consumer".into(),
                topic: "events".into(),
                members: 2,
                current_offset: 5_000,
                log_end_offset: 50_000,
                state: "Rebalancing",
            },
            StreamsConsumerGroup {
                tenant: evil.clone(),
                group_id: "evil-consumer".into(),
                topic: "evil-topic".into(),
                members: 1,
                current_offset: 0,
                log_end_offset: 0,
                state: "Empty",
        ...
        s.streams_connectors.write().unwrap().extend([
            StreamsConnector {
                ...
                state: "Running",
                tasks_max: 2,
                class: "io.confluent.connect.jdbc.JdbcSourceConnector".into(),
                topics: "orders,refunds".into(),
            },
            StreamsConnector {
                tenant: acme.clone(),
                name: "hdfs-sink".into(),
                ...
                state: "Failed",
                tasks_max: 1,
                class: "io.confluent.connect.hdfs.HdfsSinkConnector".into(),
                topics: "events".into(),
            },
            StreamsConnector {
                tenant: evil.clone(),
                name: "evil-connector".into(),
                kind: "Source",
                state: "Running",
                tasks_max: 1,
                class: "evil.SourceConnector".into(),
                topics: "evil-topic".into(),
            },
        ]);
        s.streams_connect_workers.write().unwrap().extend([
            StreamsConnectWorker {
                tenant: acme.clone(),
                id: "worker-1".into(),
                state: "Running",
                host: "connect-1.acme.local:8083".into(),
                connectors_owned: 1,
                tasks_owned: 2,
            },
            StreamsConnectWorker {
                ...
                id: "worker-2".into(),
                state: "Running",
                host: "connect-2.acme.local:8083".into(),
                connectors_owned: 1,
                tasks_owned: 1,
            },
            StreamsConnectWorker {
        ...
        s.streams_connect_tasks.write().unwrap().extend([
            ...
            StreamsConnectTask {
                tenant: acme.clone(),
                connector: "hdfs-sink".into(),
                task: 0,
                state: "Failed",
                failure_trace: Some("java.io.IOException: connection refused".into()),
            },
            StreamsConnectTask {
                tenant: evil.clone(),
                connector: "evil-connector".into(),
                task: 0,
                state: "Running",
                failure_trace: None,
            },
        ...
        s.policy_rules.write().unwrap().extend([
            PolicyRule {
                tenant: acme.clone(),
                name: "deny-internet-prod".into(),
                action: "Deny",
                subject: "spiffe://*/ns/prod/sa/*".into(),
                resource: "egress:0.0.0.0/0".into(),
                enabled: true,
            },
            PolicyRule {
                tenant: acme.clone(),
                name: "allow-monitoring".into(),
                action: "Allow",
                subject: "spiffe://*/ns/monitoring/sa/prom".into(),
                resource: "/metrics".into(),
                enabled: true,
            },
            PolicyRule {
                tenant: evil.clone(),
        ...
        s.artifact_records.write().unwrap().extend([
            ArtifactRecord {
                tenant: acme.clone(),
                registry: "registry.acme/web".into(),
                name: "web:v17".into(),
                digest: "sha256:aaaa1111".into(),
                size_bytes: 67_108_864,
                pushed_unix: 1_001_000,
            },
            ArtifactRecord {
                tenant: acme.clone(),
                registry: "registry.acme/api".into(),
                name: "api:v3".into(),
                digest: "sha256:bbbb2222".into(),
                size_bytes: 134_217_728,
                pushed_unix: 1_001_500,
            },
            ArtifactRecord {
                tenant: evil.clone(),
                registry: "registry.evil/x".into(),
                name: "x:latest".into(),
                digest: "sha256:cccc3333".into(),
                size_bytes: 1024,
                pushed_unix: 999_000,
            },
        ]);
        s.alert_rules.write().unwrap().extend([
            AlertRule {
                ...
                severity: "critical",
                expr: "rate(errors[5m]) > 0.05".into(),
                for_seconds: 300,
            },
            AlertRule {
                tenant: acme.clone(),
                name: "DiskPressure".into(),
                severity: "warning",
                expr: "disk_free < 0.10".into(),
                for_seconds: 600,
            },
            AlertRule {
                tenant: evil.clone(),
                name: "EvilNoiseAlert".into(),
                severity: "info",
                expr: "evil > 0".into(),
                for_seconds: 30,
            },
        ...
        s.active_alerts.write().unwrap().extend([
            ActiveAlert {
                tenant: acme.clone(),
                rule: "HighErrorRate".into(),
                state: "firing",
                fired_unix: 1_002_100,
            },
            ActiveAlert {
                tenant: evil.clone(),
                rule: "EvilNoiseAlert".into(),
                state: "pending",
                fired_unix: 1_002_200,
            },
        ]);
        s.backup_jobs.write().unwrap().extend([
            BackupJob {
                tenant: acme.clone(),
                name: "pg-prod-daily".into(),
                source: "rdbms://pg-prod".into(),
                destination: "s3://backups/pg-prod/".into(),
                schedule_cron: "0 2 * * *".into(),
                last_run_unix: Some(1_002_000),
                state: "Completed",
            },
            BackupJob {
                tenant: acme.clone(),
                name: "etcd-hourly".into(),
                source: "etcd://cluster-a".into(),
                destination: "s3://backups/etcd/".into(),
                schedule_cron: "0 * * * *".into(),
                last_run_unix: Some(1_002_500),
                state: "Running",
            },
            BackupJob {
                tenant: evil.clone(),
                name: "evil-backup".into(),
                source: "evil".into(),
                destination: "evil".into(),
                schedule_cron: "* * * * *".into(),
                last_run_unix: None,
                state: "Scheduled",
            },
        ...
        s.incident_records.write().unwrap().extend([
            IncidentRecord {
                tenant: acme.clone(),
                id: "INC-2026-001".into(),
                title: "API latency spike".into(),
                severity: "SEV2",
                state: "Investigating",
                opened_unix: 1_002_300,
            },
            IncidentRecord {
                tenant: acme.clone(),
                id: "INC-2026-002".into(),
                title: "DB failover".into(),
                severity: "SEV1",
                state: "Resolved",
                opened_unix: 1_001_900,
            },
            IncidentRecord {
                tenant: evil.clone(),
                id: "EVIL-001".into(),
                title: "evil disruption".into(),
                severity: "SEV4",
                state: "Open",
                opened_unix: 999_000,
            },
        ...
        s.vuln_records.write().unwrap().extend([
            VulnRecord {
                tenant: acme.clone(),
                cve_id: "CVE-2025-0001".into(),
                package: "openssl".into(),
                installed_version: "3.0.10".into(),
                fixed_version: Some("3.0.14".into()),
                severity: "Critical",
            },
            VulnRecord {
                tenant: acme.clone(),
                cve_id: "CVE-2025-0042".into(),
                package: "tokio".into(),
                installed_version: "1.40.0".into(),
                fixed_version: None,
                severity: "Medium",
            },
            VulnRecord {
                tenant: evil.clone(),
                cve_id: "CVE-2025-9999".into(),
                package: "evil-lib".into(),
        ...
        s.workflow_runs.write().unwrap().extend([
            WorkflowRun {
                tenant: acme.clone(),
                name: "etl-orders".into(),
                run_id: "wf-1001".into(),
                status: "Succeeded",
                started_unix: 1_001_800,
                finished_unix: Some(1_001_900),
            },
            WorkflowRun {
                tenant: acme.clone(),
                name: "etl-orders".into(),
            ...
            WorkflowRun {
                ...
                run_id: "evil-1".into(),
                status: "Failed",
                started_unix: 999_000,
                finished_unix: Some(999_100),
            },
        ]);
        s.chaos_experiments.write().unwrap().extend([
            ChaosExperiment {
                tenant: acme.clone(),
                name: "kill-web-pod".into(),
                kind: "pod-kill".into(),
                target_selector: "app=web".into(),
                schedule: "Cron",
                last_run_unix: Some(1_002_000),
            },
            ChaosExperiment {
                tenant: acme.clone(),
                name: "delay-api-egress".into(),
                kind: "network-delay".into(),
                target_selector: "app=api".into(),
                schedule: "Once",
                last_run_unix: None,
            },
            ChaosExperiment {
                tenant: evil.clone(),
                name: "evil-chaos".into(),
                kind: "full-cluster".into(),
                target_selector: "*".into(),
                schedule: "Continuous",
                last_run_unix: Some(1_000_000),
            },
        ]);
        s.slos.write().unwrap().extend([
            Slo {
                tenant: acme.clone(),
                name: "web-availability".into(),
                service: "web".into(),
                objective_pct: 99.9,
                window_days: 30,
                current_pct: 99.94,
            ...
            Slo {
                tenant: acme.clone(),
                name: "api-latency-p99".into(),
                service: "api".into(),
                objective_pct: 99.0,
                window_days: 30,
                current_pct: 98.7,
                error_budget_remaining_pct: -30.0,
            },
            Slo {
                tenant: evil.clone(),
                name: "evil-slo".into(),
        ...
        s.chat_threads.write().unwrap().extend([
            ChatThread {
                tenant: acme.clone(),
                id: "thr-1".into(),
                topic: "deploy-coord".into(),
                members: 8,
                last_message_unix: 1001000,
            },
            ChatThread {
                tenant: acme.clone(),
                id: "thr-2".into(),
        ...
        s.dast_scans.write().unwrap().extend([
            DastScan {
                tenant: acme.clone(),
                target: "https://api.acme/".into(),
                scan_id: "dast-001".into(),
                findings: 3,
                severity: "medium",
            },
            DastScan {
                tenant: acme.clone(),
                target: "https://web.acme/".into(),
                scan_id: "dast-002".into(),
                findings: 12,
                severity: "high",
            },
            DastScan {
                tenant: evil.clone(),
                target: "https://evil/".into(),
                scan_id: "evil-1".into(),
                findings: 0,
                severity: "info",
            },
        ...
        s.devlake_metrics.write().unwrap().extend([
            DevlakeMetric {
                ...
                metric: "deploy_freq".into(),
                value_thousandths: 1500,
            },
            DevlakeMetric {
                tenant: acme.clone(),
                project: "acme-api".into(),
                metric: "lead_time_hours".into(),
                value_thousandths: 24000,
            },
            DevlakeMetric {
                tenant: evil.clone(),
                project: "evil".into(),
                metric: "evil_metric".into(),
                value_thousandths: 1,
            },
        ]);
        s.forensics_evidence.write().unwrap().extend([
            ForensicsEvidence {
                ...
                artifact: "memdump-pod-web-0".into(),
                collected_unix: 1001000,
                digest: "sha256:aaa1".into(),
            },
            ForensicsEvidence {
                tenant: acme.clone(),
                case_id: "CASE-002".into(),
                artifact: "pcap-2026-05-11".into(),
                collected_unix: 1001500,
                digest: "sha256:bbb2".into(),
            },
            ForensicsEvidence {
                tenant: evil.clone(),
                case_id: "EVIL-001".into(),
                artifact: "evil-evidence".into(),
                collected_unix: 999000,
                digest: "sha256:evil".into(),
            },
        ]);
        s.gateway_routes.write().unwrap().extend([
            GatewayRoute {
                tenant: acme.clone(),
                name: "web-route".into(),
                listener: "https-443".into(),
                hostname: "acme.com".into(),
                backend: "svc/web:80".into(),
            },
            GatewayRoute {
                tenant: acme.clone(),
                name: "api-route".into(),
                listener: "https-443".into(),
                hostname: "api.acme.com".into(),
                backend: "svc/api:8080".into(),
            },
            GatewayRoute {
                tenant: evil.clone(),
                name: "evil-route".into(),
                listener: "http-80".into(),
                hostname: "evil.com".into(),
                backend: "svc/evil:1".into(),
            },
        ]);
        ...
        s.pam_sessions.write().unwrap().extend([
            PamSession {
                tenant: acme.clone(),
                id: "sess-1".into(),
                principal: "alice@acme".into(),
                target: "pg-prod-1".into(),
                started_unix: 1001000,
                ended_unix: Some(1_001_300),
            },
            PamSession {
                tenant: acme.clone(),
                id: "sess-2".into(),
                principal: "bob@acme".into(),
                target: "etcd-cluster".into(),
                started_unix: 1001500,
                ended_unix: None,
            },
            PamSession {
                tenant: evil.clone(),
                id: "evil-sess".into(),
                principal: "mallory@evil".into(),
                target: "any".into(),
                started_unix: 999000,
                ended_unix: None,
            },
        ...
        s.scan_results.write().unwrap().extend([
            ...
            ScanResult {
                tenant: evil.clone(),
                scan_id: "evil-scan".into(),
                scanner: "evil-scanner".into(),
                findings: 99,
        ...
        s.secret_metadatas.write().unwrap().extend([
            SecretMetadata {
                ...
                backend: "vault-kv".into(),
                version: 3,
                created_unix: 1001000,
            },
            SecretMetadata {
                tenant: acme.clone(),
                path: "app/api-token".into(),
                backend: "vault-kv".into(),
                version: 1,
                created_unix: 1001500,
            },
            SecretMetadata {
                tenant: evil.clone(),
                path: "evil/secret".into(),
                backend: "evil-kv".into(),
                version: 1,
                created_unix: 999000,
            },
        ]);
        s.uptime_probes.write().unwrap().extend([
            UptimeProbe {
                tenant: acme.clone(),
                name: "web-prod".into(),
                url: "https://web.acme/health".into(),
                interval_seconds: 60,
                last_status: "Up",
            },
            UptimeProbe {
                tenant: acme.clone(),
                name: "api-prod".into(),
                url: "https://api.acme/health".into(),
                interval_seconds: 30,
                last_status: "Up",
            },
            UptimeProbe {
                tenant: evil.clone(),
                name: "evil-probe".into(),
                url: "https://evil/".into(),
                interval_seconds: 30,
                last_status: "Down",
            },
        ...
        s.metric_series.write().unwrap().extend([
            ...
            MetricSeries {
                tenant: evil.clone(),
                name: "evil_metric".into(),
                scraper: "evil-scraper".into(),
                sample_count: 1,
                retention_days: 1,
            },
        ]);
        s.trace_services.write().unwrap().extend([
            TraceService {
                tenant: acme.clone(),
                service: "web".into(),
                span_count_per_sec: 1500,
                error_rate_per_thousand: 5,
            },
            TraceService {
                tenant: acme.clone(),
                service: "api".into(),
                span_count_per_sec: 800,
                error_rate_per_thousand: 12,
            },
            TraceService {
                tenant: evil.clone(),
                service: "evil-svc".into(),
                span_count_per_sec: 1,
                error_rate_per_thousand: 999,
            },
        ]);
        ...
        s.log_streams.write().unwrap().extend([
            ...
            LogStream {
                ...
                sink: "loki".into(),
                ingest_rate_per_sec: 8000,
                retention_days: 14,
            },
            LogStream {
                tenant: evil.clone(),
                name: "evil-stream".into(),
                sink: "evil-sink".into(),
                ingest_rate_per_sec: 1,
                retention_days: 1,
            },
        ]);
        s.security_events.write().unwrap().extend([
            SecurityEvent {
                tenant: acme.clone(),
                id: "sec-1".into(),
                kind: "brute-force-detected".into(),
                severity: "high",
                at_unix: 1002000,
            },
            SecurityEvent {
        ...
        s.ha_failover_events.write().unwrap().extend([
            HaFailoverEvent {
                tenant: acme.clone(),
                id: "fo-1".into(),
                subject: "pg-prod".into(),
                old_primary: "pg-prod-1".into(),
                new_primary: "pg-prod-2".into(),
                at_unix: 1003000,
            },
            HaFailoverEvent {
                tenant: evil.clone(),
                id: "fo-evil".into(),
                subject: "evil".into(),
                old_primary: "e-1".into(),
                new_primary: "e-2".into(),
                at_unix: 999000,
            },
        ]);
        s.erp_invoices.write().unwrap().extend([
            ...
            ErpInvoice {
                ...
                customer: "ACME-CUST-2".into(),
                amount_cents: 1000000,
                status: "Open",
            },
            ErpInvoice {
                tenant: evil.clone(),
                invoice_id: "EVIL-INV".into(),
                customer: "EVIL-C".into(),
                amount_cents: 1,
                status: "Void",
            },
        ]);
        ...
        s.pipeline_runs.write().unwrap().extend([
            ...
            PipelineRun {
                tenant: evil.clone(),
                pipeline: "evil-pl".into(),
                run_id: "evil-run".into(),
                status: "Failed",
                duration_seconds: 1,
            },
        ]);
        ...
        s.knative_services.write().unwrap().extend([
            ...
            KnativeService {
                tenant: acme.clone(),
                name: "sentiment-svc".into(),
                image: "acme/nlp:v2".into(),
                replicas: 5,
                min_scale: 1,
                max_scale: 20,
            },
        ...
        s.local_llm_models.write().unwrap().extend([
            LocalLlmModel {
                tenant: acme.clone(),
                tag: "qwen3.6:35b-a3b-coding-mxfp8".into(),
        ...
        s.tracker_issues.write().unwrap().extend([
            TrackerIssue {
                tenant: acme.clone(),
                id: "ISS-100".into(),
                title: "slow query on orders".into(),
                state: "Open",
                assignee: Some("alice@acme".to_string()),
            },
            TrackerIssue {
                tenant: acme.clone(),
                id: "ISS-101".into(),
        ...
        s.upstream_projects.write().unwrap().extend([
            UpstreamProject {
                ...
                repo: "kubernetes/kubernetes".into(),
                pinned_version: "v1.31.2".into(),
                last_check_unix: 1003000,
            },
            UpstreamProject {
                tenant: acme.clone(),
                name: "istio".into(),
                repo: "istio/istio".into(),
                pinned_version: "1.23.0".into(),
                last_check_unix: 1003100,
            },
            UpstreamProject {
                tenant: evil.clone(),
                name: "evil-upstream".into(),
                repo: "evil/evil".into(),
                pinned_version: "0.0.1".into(),
                last_check_unix: 999000,
            },
        ]);
        ...
        s.cdc_pipelines.write().unwrap().extend([
            CdcPipeline {
                ...
                source: "pg:orders".into(),
                sink: "kafka:cdc-orders".into(),
                state: "Running",
            },
            CdcPipeline {
                tenant: acme.clone(),
                name: "users-cdc".into(),
                source: "pg:users".into(),
                sink: "kafka:cdc-users".into(),
                state: "Paused",
            },
            CdcPipeline {
                tenant: evil.clone(),
        ...
        s.cert_records.write().unwrap().extend([
            CertRecord {
                tenant: acme.clone(),
                subject: "acme.com".into(),
                issuer: "Let's Encrypt".into(),
                not_after_unix: 1700000000,
                serial: "01:23:45".into(),
            },
            CertRecord {
                tenant: acme.clone(),
                subject: "api.acme.com".into(),
                issuer: "Let's Encrypt".into(),
                not_after_unix: 1710000000,
                serial: "01:23:46".into(),
            },
            CertRecord {
                tenant: evil.clone(),
                subject: "evil.com".into(),
                issuer: "evil-ca".into(),
                not_after_unix: 999999,
                serial: "00".into(),
            },
        ]);
        ...
        s.crossplane_claims.write().unwrap().extend([
            CrossplaneClaim {
                tenant: acme.clone(),
                name: "db-1".into(),
                kind: "PostgresInstance".into(),
                composition: "composition-pg".into(),
                state: "Ready",
            },
            CrossplaneClaim {
                tenant: acme.clone(),
                name: "bucket-1".into(),
                kind: "S3Bucket".into(),
                composition: "composition-s3".into(),
                state: "Provisioning",
            },
            CrossplaneClaim {
                tenant: evil.clone(),
                name: "evil-claim".into(),
                kind: "evil".into(),
                composition: "evil-comp".into(),
                state: "Failed",
            },
        ]);
        s.gitops_apps.write().unwrap().extend([
            GitopsApp {
                tenant: acme.clone(),
                name: "web-app".into(),
                repo: "acme/k8s-config".into(),
                path: "apps/web".into(),
                synced_at_unix: 1003000,
            },
            GitopsApp {
                tenant: acme.clone(),
                name: "api-app".into(),
        ...
        s.oncall_shifts.write().unwrap().extend([
            OncallShift {
                tenant: acme.clone(),
                rotation: "sre-primary".into(),
                oncaller: "alice@acme".into(),
                start_unix: 1003000,
                end_unix: 1088400,
            },
            OncallShift {
                tenant: acme,
                rotation: "sre-secondary".into(),
                oncaller: "bob@acme".into(),
                start_unix: 1003000,
                end_unix: 1088400,
            },
            OncallShift {
                tenant: evil,
                rotation: "evil-rotation".into(),
                oncaller: "mallory@evil".into(),
                start_unix: 999000,
                end_unix: 1000000,
            },
        ]);
        ...
        s.search_indexes.write().unwrap().extend([
            ...
            SearchIndex {
                tenant: TenantId::new("acme").expect("test fixture"),
                name: "logs-index".into(),
                doc_count: 1000000000,
                size_bytes: 50000000000,
                status: "Healthy",
            },
            SearchIndex {
                tenant: TenantId::new("evil").expect("test fixture"),
                name: "evil-index".into(),
                doc_count: 1,
                size_bytes: 1,
                status: "Degraded",
            },
