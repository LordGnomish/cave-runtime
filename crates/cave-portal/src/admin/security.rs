...
use crate::admin::state::{AdminState, SecurityEvent, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<SecurityEvent>, SecurityViewError> {
    ctx.authorise(Permission::SecurityRead)?;
    Ok(
        scope(&state.security_events.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, SecurityViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/security",
        &format!("security · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/security/src/components/EventsList.tsx",
    "EventsList",
);
