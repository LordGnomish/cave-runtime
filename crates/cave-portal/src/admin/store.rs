...
use crate::admin::state::{AdminState, StoreBucket, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<StoreBucket>, StoreViewError> {
    ctx.authorise(Permission::StoreRead)?;
    let mut rows: Vec<StoreBucket> =
        scope(&state.store_buckets.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn group_by_backend(rows: &[StoreBucket]) -> Vec<(String, usize)> {
    ...
    for r in rows {
        *acc.entry(r.backend.clone()).or_insert(0) += 1;
    }
    acc.into_iter().collect()
}

pub fn total_objects(rows: &[StoreBucket]) -> u64 {
    rows.iter().map(|r| r.object_count).sum()
}
pub fn total_size_bytes(rows: &[StoreBucket]) -> u64 {
...
pub fn detail(
    ...
    ctx: &RequestCtx,
    name: &str,
) -> Result<Option<StoreBucket>, StoreViewError> {
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, StoreViewError> {
  ...
  {tbl}
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/store",
        &format!("store · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/store/src/components/BucketsList.tsx",
    "BucketsList",
);
...
mod tests {
    ...
    fn detail_returns_bucket_by_name() {
        ...
        if let Some(f) = r.first() {
            assert!(
                detail(&s, &ctx(&[Permission::StoreRead]), &f.name)
                    .unwrap()
                    .is_some()
            );
        }
        assert!(
            detail(&s, &ctx(&[Permission::StoreRead]), "no-such")
                .unwrap()
                .is_none()
        );
    }
