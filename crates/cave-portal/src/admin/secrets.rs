...
use crate::admin::state::{AdminState, SecretMetadata, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<SecretMetadata>, SecretsViewError> {
    ctx.authorise(Permission::SecretsBrowserRead)?;
    Ok(
        scope(&state.secret_metadatas.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, SecretsViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/secrets",
        &format!("secrets · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/secrets/src/components/SecretsList.tsx",
    "SecretsList",
);
...
mod tests {
    ...
    fn render_contains_owner_row() {
        ...
        let html = render(
            &AdminState::seeded(),
            &ctx(&[Permission::SecretsBrowserRead]),
        )
        .unwrap();
