...
use crate::admin::state::{AdminState, Slo, scope};
...
pub fn list_slos(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<Slo>, SloViewError> {
    ...
    Ok(
        scope(&state.slos.read().unwrap(), &ctx.tenant, |r| &r.tenant)
            .into_iter()
            .cloned()
            .collect(),
    )
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, SloViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/slo",
        &format!("slo · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) =
            portal_test_ctx!("plugins/slo/src/components/SloList.tsx", "SloList", "acme");
    ...
    fn breaching_slos_returns_negative_budget_only() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/slo/src/components/BurnBoard.tsx",
            "BurnBoard",
            "acme"
        );
        let s = AdminState::seeded();
    ...
    fn render_excludes_evil_slo() {
        let (_c, _t) =
            portal_test_ctx!("plugins/slo/src/components/SloPage.tsx", "SloPage", "acme");
