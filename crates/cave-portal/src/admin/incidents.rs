...
use crate::admin::state::{AdminState, IncidentRecord, scope};
...
pub fn list_incidents(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<IncidentRecord>, IncidentsViewError> {
    ctx.authorise(Permission::IncidentsRead)?;
    let mut rows: Vec<IncidentRecord> =
        scope(&state.incident_records.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn transition(
    ...
    let target = incs
        .iter_mut()
        .find(|i| i.tenant == ctx.tenant && i.id == id)
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, IncidentsViewError> {
  ...
  <h2 class="text-lg font-semibold mb-2">Incidents ({n})</h2>{tbl}
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/incidents",
        &format!("incidents · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/incidents/src/components/IncidentsList.tsx",
    "IncidentsList",
);
...
mod tests {
    ...
    fn transition_updates_state() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/incidents/src/components/StateSelect.tsx",
            "StateSelect",
            "acme"
        );
        let s = AdminState::seeded();
        ...
        assert_eq!(
            inc.iter().find(|x| x.id == "INC-2026-001").unwrap().state,
            "Resolved"
        );
    }
    ...
    fn transition_rejects_invalid_state_and_cross_tenant() {
        ...
        let c = ctx(&[Permission::IncidentsRead, Permission::IncidentsWrite]);
        assert!(matches!(
            transition(&s, &c, "INC-2026-001", "Pondering").unwrap_err(),
            IncidentsViewError::InvalidState(_)
        ));
        assert!(matches!(
            transition(&s, &c, "EVIL-001", "Resolved").unwrap_err(),
            IncidentsViewError::IncidentNotFound(_)
        ));
    }
    ...
    fn render_excludes_evil_incident() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/incidents/src/components/IncidentsPage.tsx",
            "IncidentsPage",
            "acme"
        );
        let html = render(&AdminState::seeded(), &ctx(&[Permission::IncidentsRead])).unwrap();
