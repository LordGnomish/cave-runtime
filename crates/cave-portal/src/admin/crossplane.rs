...
use crate::admin::state::{AdminState, CrossplaneClaim, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<CrossplaneClaim>, CrossplaneViewError> {
    ctx.authorise(Permission::CrossplaneRead)?;
    Ok(
        scope(&state.crossplane_claims.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, CrossplaneViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/crossplane",
        &format!("crossplane · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/crossplane/src/components/ClaimsList.tsx",
    "ClaimsList",
);
