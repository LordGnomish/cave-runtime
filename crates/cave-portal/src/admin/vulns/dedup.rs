...
pub const ALGORITHMS: &[(&str, &str)] = &[
    ...
    ("unique_id_from_tool", "stable scanner-generated fingerprint"),
    ("unique_id_from_tool_or_hash_code", "uid first, hash_code fallback"),
];
...
pub const SCANNERS: &[(&str, &str)] = &[
    ...
    ("Semgrep JSON Report", "title / cwe / line / file_path / description"),
...
pub fn render(_state: &AdminState, ctx: &RequestCtx) -> Result<String, VulnsViewError> {
    ...
    let algo_rows: Vec<Vec<String>> = ALGORITHMS.iter()
        .map(|(a, d)| vec![a.to_string(), d.to_string()]).collect();
    let scan_rows: Vec<Vec<String>> = SCANNERS.iter()
        .map(|(s, f)| vec![s.to_string(), f.to_string()]).collect();
    let body = format!(
        r#"<section>
