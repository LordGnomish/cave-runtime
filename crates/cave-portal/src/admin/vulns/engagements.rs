...
pub const ENGAGEMENT_STATUS: &[&str] = &[
    "NotStarted", "Blocked", "Cancelled", "Completed",
    "InProgress", "OnHold", "WaitingForResource",
];
...
pub fn render(_state: &AdminState, ctx: &RequestCtx) -> Result<String, VulnsViewError> {
  ...
  target window: 30 days from creation; default status: InProgress.</p>
        ...
        types = table(&["type"], &ENGAGEMENT_TYPES.iter().map(|s| vec![s.to_string()]).collect::<Vec<_>>()),
        status = table(&["status"], &ENGAGEMENT_STATUS.iter().map(|s| vec![s.to_string()]).collect::<Vec<_>>()),
    );
