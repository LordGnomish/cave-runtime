...
pub fn unfixed_count(state: &AdminState, ctx: &RequestCtx) -> Result<usize, VulnsViewError> {
    Ok(list_vulns(state, ctx)?
        .iter()
        .filter(|v| v.fixed_version.is_none())
        .count())
}
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, VulnsViewError> {
  ...
  <h2 class="text-lg font-semibold mb-2">CVEs ({n})</h2>{tbl}
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/vulns",
        &format!("vulns · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/vulns/src/components/CveList.tsx",
            "CveList",
            "acme"
        );
    ...
    fn list_by_severity_filters() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/vulns/src/components/SeverityFilter.tsx",
            "SeverityFilter",
            "acme"
        );
        let s = AdminState::seeded();
    ...
    fn unfixed_count_works() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/vulns/src/components/UnfixedBadge.tsx",
            "UnfixedBadge",
            "acme"
        );
        let s = AdminState::seeded();
        assert_eq!(
            unfixed_count(&s, &ctx(&[Permission::VulnsRead])).unwrap(),
            1
        );
    }
    ...
    fn group_by_severity_critical_first() {
        ...
        if let (Some(c), Some(h)) = (crit, high) {
            assert!(c < h);
        }
    }
    ...
    fn render_excludes_evil_cve() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/vulns/src/components/CvePage.tsx",
            "CvePage",
            "acme"
        );
        let html = render(&AdminState::seeded(), &ctx(&[Permission::VulnsRead])).unwrap();
