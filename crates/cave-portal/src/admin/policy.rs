...
use crate::admin::state::{AdminState, PolicyRule, scope};
...
pub fn list_rules(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<PolicyRule>, PolicyViewError> {
    ctx.authorise(Permission::PolicyRead)?;
    Ok(
        scope(&state.policy_rules.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn group_by_action(rows: &[PolicyRule]) -> Vec<(String, usize)> {
    ...
    for r in rows {
        *acc.entry(r.action.to_string()).or_insert(0) += 1;
    }
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, PolicyViewError> {
  ...
  <h2 class="text-lg font-semibold mb-2">Policy rules ({n})</h2>{tbl}
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/policy",
        &format!("policy · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
mod tests {
    ...
    fn list_filters_to_owner() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/policy/src/components/RulesList.tsx",
            "RulesList",
            "acme"
        );
    ...
    fn set_enabled_toggles_and_refuses_cross_tenant() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/policy/src/components/RuleToggle.tsx",
            "toggle",
            "acme"
        );
        let s = AdminState::seeded();
        ...
        assert!(
            !r.iter()
                .find(|x| x.name == "deny-internet-prod")
                .unwrap()
                .enabled
        );
        assert!(matches!(
            set_enabled(&s, &c, "evil-allow-all", false).unwrap_err(),
            PolicyViewError::RuleNotFound(_)
        ));
    }
    ...
    fn render_excludes_evil_rule() {
        let (_c, _t) = portal_test_ctx!(
            "plugins/policy/src/components/RulesPage.tsx",
            "RulesPage",
            "acme"
        );
        let s = AdminState::seeded();
