...
use crate::admin::state::{AdminState, TraceService, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<TraceService>, TraceViewError> {
    ctx.authorise(Permission::TraceRead)?;
    let mut rows: Vec<TraceService> =
        scope(&state.trace_services.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
        .collect();
    rows.sort_by(|a, b| {
        b.span_count_per_sec
            .cmp(&a.span_count_per_sec)
...
pub fn service_health(svc: &TraceService) -> &'static str {
    if svc.error_rate_per_thousand >= ERROR_RATE_FAILING_PER_K {
        "Failing"
    } else if svc.error_rate_per_thousand >= ERROR_RATE_DEGRADED_PER_K {
        "Degraded"
    } else {
        "Healthy"
    }
...
pub fn degraded_services<'a>(rows: &'a [TraceService]) -> Vec<&'a TraceService> {
    rows.iter()
        .filter(|s| service_health(s) != "Healthy")
        .collect()
}
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, TraceViewError> {
    ...
    let table_rows: Vec<Vec<String>> = rows
        ...
        .map(|r| {
            vec![
                escape(&r.service),
                r.span_count_per_sec.to_string(),
                r.error_rate_per_thousand.to_string(),
  ...
  {tbl}
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/trace",
        &format!("trace · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/trace/src/components/ServicesList.tsx",
    "ServicesList",
);
...
mod tests {
    ...
    fn list_filters_to_owner_sorted_by_spans_desc() {
        ...
        for w in r.windows(2) {
            assert!(w[0].span_count_per_sec >= w[1].span_count_per_sec);
        }
    }
    ...
    fn service_health_classifies_buckets() {
        ...
        let mk = |e: u32| TraceService {
            tenant: t.clone(),
            service: "s".into(),
            span_count_per_sec: 10,
            error_rate_per_thousand: e,
        };
