...
use crate::admin::state::{AdminState, PamSession, scope};
...
pub fn list_records(state: &AdminState, ctx: &RequestCtx) -> Result<Vec<PamSession>, PamViewError> {
    ...
    Ok(
        scope(&state.pam_sessions.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, PamViewError> {
    ...
    let body = format!(
        ...
        n = rows.len(),
        tbl = table(
            &["id", "principal", "target", "started", "ended"],
            &table_rows
        ),
    );
    Ok(page_shell_full(
        ctx,
        "/admin/pam",
        &format!("pam · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/pam/src/components/SessionsList.tsx",
    "SessionsList",
);
