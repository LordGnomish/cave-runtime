...
use crate::models::{
    DataFrame, DataFrameData, DataFrameSchema, DsQueryRequest, DsQueryResponse, FieldSchema,
    QueryResult,
};
...
fn transform_reduce(frames: Vec<DataFrame>, options: &serde_json::Value) -> Vec<DataFrame> {
    ...
    frames
        ...
        .map(|frame| {
            ...
            let mut new_fields: Vec<FieldSchema> = vec![FieldSchema {
                name: "Field".into(),
...
fn transform_merge(frames: Vec<DataFrame>) -> Vec<DataFrame> {
    ...
    for frame in &frames {
        let row_count = frame.data.values.first().map(|v| v.len()).unwrap_or(0);
        let field_idx: HashMap<&str, usize> = frame
            .schema
            .fields
            .iter()
            .enumerate()
...
fn transform_filter_fields(frames: Vec<DataFrame>, options: &serde_json::Value) -> Vec<DataFrame> {
    ...
    frames
        ...
        .map(|frame| {
            ...
            let new_fields: Vec<FieldSchema> = keep
                .iter()
                .map(|&i| frame.schema.fields[i].clone())
                .collect();
            let new_values: Vec<Vec<serde_json::Value>> = keep
                .iter()
...
fn transform_filter_by_value(
    ...
    frames
        ...
        .map(|frame| {
            ...
            let row_count = frame.data.values.first().map(|v| v.len()).unwrap_or(0);
            let keep_rows: Vec<usize> = (0..row_count)
                .filter(|&row| {
            ...
            let new_values: Vec<Vec<serde_json::Value>> = frame
                .data
                .values
                .iter()
                .map(|col| {
                    ...
                    (0..row_count)
                        .map(|row| {
                            let vals: Vec<f64> = frame
                                ...
                                .filter_map(|(i, _)| frame.data.values.get(i)?.get(row)?.as_f64())
                                .collect();
    ...
    frames
        ...
        .map(|frame| {
            let field_idx = frame
                ...
                .iter()
                .position(|f| f.name == field_name);
            let Some(sort_col) = field_idx else {
                return frame;
            };
...
/// Convert field type.
fn transform_convert_field_type(
    frames: Vec<DataFrame>,
    options: &serde_json::Value,
) -> Vec<DataFrame> {
    let conversions = options
        .get("conversions")
        .and_then(|v| v.as_array())
        .cloned()
        .unwrap_or_default();
        ...
        .map(|mut frame| {
            for conversion in &conversions {
                let field_name = conversion
                    ...
                    .and_then(|v| v.as_str())
                    .unwrap_or("");
                let dest_type = conversion
                    .get("destinationType")
                    .and_then(|v| v.as_str())
                    .unwrap_or("string");
                if let Some(field) = frame
                    .schema
                    .fields
                    .iter_mut()
...
fn transform_series_to_rows(frames: Vec<DataFrame>) -> Vec<DataFrame> {
    ...
    for frame in &frames {
        ...
        for row in 0..row_count {
            let ts = time_idx
                .and_then(|i| frame.data.values.get(i)?.get(row).cloned())
                .unwrap_or(serde_json::Value::Null);
            let val = val_idx
                .and_then(|i| frame.data.values.get(i)?.get(row).cloned())
                .unwrap_or(serde_json::Value::Null);
    ...
    vec![DataFrame {
        ...
        data: DataFrameData {
            values: vec![times, metrics, values],
            entities: None,
        },
    }]
}
