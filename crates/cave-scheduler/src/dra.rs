...
impl DraState {
    ...
    pub fn allocate(&mut self, claim_name: &str, node_name: &str, devices: Vec<String>) {
        ...
        if let Some(c) = self.claims.get_mut(claim_name) {
            c.allocation = Some(DeviceAllocation { node_name: node_name.into(), devices });
        }
        if let Some(s) = self.slices.get_mut(node_name) {
            for d in &mut s.devices {
                if chosen.contains(&d.name) { d.allocated_by = Some(claim_name.into()); }
            }
        }
    }
}
    ...
    fn filter(&self, pod: &Pod, node: &Node, _: &ClusterSnapshot) -> Status {
        ...
        for r in &pod.spec.resource_claims {
            let Some(claim) = self.state.claims.get(&r.claim_name) else {
                return Status::unresolvable("DynamicResources", format!("claim {} not found", r.claim_name));
            };
            ...
            if let Some(alloc) = &claim.allocation {
                if alloc.node_name != node.name {
                    return Status::unschedulable("DynamicResources",
            ...
            if self.state.try_allocate_on(&node.name, claim).is_none() {
                return Status::unschedulable("DynamicResources",
                    format!("node cannot satisfy claim {} ({}× {})", r.claim_name, claim.count, claim.device_class));
            }
        }
...
mod tests {
    ...
    fn ready(name: &str) -> Node {
        Node { name: name.into(), uid: Uuid::new_v4(), status: NodeStatus::Ready,
            capacity: ResourceCapacity::default(), allocatable: ResourceCapacity { cpu_millicores: 1000, memory_bytes: 1, pods: 10, ephemeral_storage_bytes: 0 },
    ...
    fn dev(name: &str, class: &str, vendor: &str) -> Device {
        let mut a = HashMap::new(); a.insert("vendor".into(), vendor.into());
    ...
    fn skip_when_no_claims() {
        ...
        assert_eq!(plug.filter(&pod, &ready("a"), &ClusterSnapshot::default()).code, Code::Skip);
    }

    #[test]
    fn allocates_when_devices_available() {
        let mut st = DraState::default();
        st.add_slice(ResourceSlice { node_name: "a".into(), devices: vec![
            dev("gpu0", "nvidia.com/gpu", "nvidia"),
            dev("gpu1", "nvidia.com/gpu", "nvidia"),
        ]});
        st.add_claim(ResourceClaim {
            name: "c1".into(), namespace: "ns".into(), tenant_id: "t".into(),
            device_class: "nvidia.com/gpu".into(), count: 1,
            selector: HashMap::from([("vendor".into(), "nvidia".into())]),
            allocation: None,
        });
        let plug = DynamicResources { state: &st };
        let mut pod = Pod::new("t", "ns", "p");
        pod.spec.resource_claims.push(ResourceClaimRef { name: "claim0".into(), claim_name: "c1".into() });
        assert!(plug.filter(&pod, &ready("a"), &ClusterSnapshot::default()).is_success());
    }

    #[test]
    fn rejects_when_class_mismatch() {
        let mut st = DraState::default();
        st.add_slice(ResourceSlice { node_name: "a".into(), devices: vec![ dev("dev0", "intel.com/fpga", "intel") ]});
        st.add_claim(ResourceClaim {
            name: "c1".into(), namespace: "ns".into(), tenant_id: "t".into(),
            device_class: "nvidia.com/gpu".into(), count: 1,
            selector: HashMap::new(), allocation: None,
        });
        let plug = DynamicResources { state: &st };
        let mut pod = Pod::new("t", "ns", "p");
        pod.spec.resource_claims.push(ResourceClaimRef { name: "claim0".into(), claim_name: "c1".into() });
        assert_eq!(plug.filter(&pod, &ready("a"), &ClusterSnapshot::default()).code, Code::Unschedulable);
    }

    #[test]
    fn rejects_cross_tenant_claim() {
        let mut st = DraState::default();
        st.add_slice(ResourceSlice { node_name: "a".into(), devices: vec![ dev("gpu0", "nvidia.com/gpu", "nvidia") ]});
        st.add_claim(ResourceClaim {
            name: "c1".into(), namespace: "ns".into(), tenant_id: "OTHER".into(),
            device_class: "nvidia.com/gpu".into(), count: 1,
            selector: HashMap::new(), allocation: None,
        });
        let plug = DynamicResources { state: &st };
    ...
    fn binds_node_after_allocation() {
        let mut st = DraState::default();
        st.add_slice(ResourceSlice { node_name: "a".into(), devices: vec![ dev("gpu0", "nvidia.com/gpu", "nvidia") ]});
        st.add_slice(ResourceSlice { node_name: "b".into(), devices: vec![ dev("gpu0", "nvidia.com/gpu", "nvidia") ]});
        st.add_claim(ResourceClaim {
            name: "c1".into(), namespace: "ns".into(), tenant_id: "t".into(),
            device_class: "nvidia.com/gpu".into(), count: 1,
            selector: HashMap::new(), allocation: None,
        ...
        assert_eq!(plug.filter(&pod, &ready("b"), &ClusterSnapshot::default()).code, Code::Unschedulable);
    }
}
