...
impl PodDisruptionBudget {
    ...
    fn matches(&self, pod: &Pod) -> bool {
        if pod.namespace != self.namespace || pod.tenant_id != self.tenant_id {
            return false;
        }
        self.selector
            .iter()
            .all(|(k, v)| pod.spec.node_selector.get(k) == Some(v))
    }
}
...
pub fn preempt(
    ...
    for node in nodes {
        ...
        let mut candidates: Vec<&Pod> = pods
            .iter()
            .filter(|p| {
                p.spec.priority < preemptor.spec.priority && p.tenant_id == preemptor.tenant_id
            })
            .collect();
        // Lowest priority first → if tie, larger CPU first (frees more per evict).
        candidates.sort_by(|a, b| {
            a.spec.priority.cmp(&b.spec.priority).then_with(|| {
                b.spec
                    .resources
                    .cpu_millicores
                    .cmp(&a.spec.resources.cpu_millicores)
            })
        });
...
mod tests {
    ...
    fn preempt_does_not_evict_higher_priority() {
        ...
        let snap = ClusterSnapshot {
            ...
            pods_by_node: HashMap::from([(
                "a".into(),
                vec![pod_at("t", "boss", 100, 4000, 8_000_000_000)],
            )]),
        };
    ...
    fn preempt_respects_tenant_isolation() {
        ...
        let snap = ClusterSnapshot {
            ...
            pods_by_node: HashMap::from([(
                "a".into(),
                vec![pod_at("other", "low", 0, 4000, 8_000_000_000)],
            )]),
        };
        let preemptor = pod_at("t", "new", 100, 1000, 0);
        assert!(
            preempt(&preemptor, &snap, &[]).is_none(),
    ...
    fn preempt_sets_nominated_node_name() {
        ...
        b.allocated.cpu_millicores = 4000;
        let snap = ClusterSnapshot {
            nodes: vec![a, b],
            pods_by_node: HashMap::from([
                (
