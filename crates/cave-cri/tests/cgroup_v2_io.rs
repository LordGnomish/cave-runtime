...
use cave_cri::cgroup::{
    apply_limits_in, attach_pid, create_cgroup_in, read_stats_in, remove_cgroup_in, set_freezer,
    CgroupHandle, FreezerState,
};
...
fn apply_limits_writes_real_bytes_to_canonical_files() {
    ...
    assert_eq!(
        fs::read_to_string(h.path.join("cpu.weight")).unwrap(),
        "512"
    );
    assert_eq!(
        fs::read_to_string(h.path.join("cpu.max")).unwrap(),
        "50000 100000"
    );
    assert_eq!(
        fs::read_to_string(h.path.join("memory.max")).unwrap(),
...
fn attach_pid_writes_to_cgroup_procs() {
    ...
    assert_eq!(
        fs::read_to_string(h.path.join("cgroup.procs")).unwrap(),
        "4242"
    );
}
...
fn freezer_state_toggles_cgroup_freeze_file() {
    ...
    assert_eq!(
        fs::read_to_string(h.path.join("cgroup.freeze")).unwrap(),
        "1"
    );
...
",
    ...
    fs::write(h.path.join("pids.events"), "max 2\nmax.imposed 0\n").unwrap();
    fs::write(
        h.path.join("io.stat"),
        "\
