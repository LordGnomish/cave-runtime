...
fn register_idempotent_per_tenant() {
    ...
    let client = AcmeClient::register(
        &mut server,
        TENANT,
        jwk("alice"),
...
fn new_order_starts_pending_with_one_authz_per_dns() {
    let mut server = AcmeServer::new();
    let mut client = AcmeClient::register(
        &mut server,
        TENANT,
        jwk("bob"),
        vec![format!("mailto:bob@{}.test", TENANT)],
    )
    .unwrap();
    let order_id = client
        .new_order(&[
        ...
        .unwrap();
    assert_eq!(
        client.order_status(&order_id).unwrap(),
        OrderStatus::Pending
    );
    assert_eq!(
        server.authorization_count(),
        2,
...
fn full_workflow_pending_to_ready_to_valid() {
    ...
    let acct_id = server
        .new_account(
            TENANT,
            jwk("carol"),
            vec![format!("mailto:carol@{}.test", TENANT)],
            true,
            None,
        )
        .unwrap();
    let order_id = server
        .new_order(
            TENANT,
            &acct_id,
            vec![Identifier::dns("svc.acme-prod.cave-runtime.test")],
        )
        .unwrap();
    assert_eq!(
        server.order(TENANT, &order_id).unwrap().status,
        OrderStatus::Pending
    );

    // Drive each authorization on the order to valid.
    let authz_ids = server
    ...
    let bad = server
        .finalize_order("tenant-other", &order_id, "https://x")
        .unwrap_err();
    assert!(matches!(
        bad,
        cave_acme::AcmeError::CrossTenantDenied { .. }
    ));
}
...
fn finalize_on_pending_order_is_rejected() {
    ...
    let acct_id = server
        .new_account(
            TENANT,
            jwk("dave"),
            vec![format!("mailto:dave@{}.test", TENANT)],
            true,
            None,
        )
        .unwrap();
    ...
    let err = server
        .finalize_order(TENANT, &order_id, "https://acme/cert/svc-001")
        .unwrap_err();
