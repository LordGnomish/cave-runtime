...
async fn main() -> anyhow::Result<()> {
    ...
    // Agent runtime (OpenJarvis primitives + self-improvement loop)
    let agent_state = Arc::new(cave_agent::AgentState::default());
    ...
    let app = Router::new()
        ...
        // Workspace parity-index passthrough — the on-disk artifact at
        // docs/parity/parity-index.json is regenerated by
        // scripts/build-parity-index.py and embedded by
        // cave-portal::admin::compliance. Surface it on a dedicated
        // /api/parity endpoint so portal-web (and external dashboards)
        // can fetch the live blob without going through the compliance
        // dashboard's HTML view.
        .route("/api/parity", get(api_parity))
        ...
        // Agent runtime (OpenJarvis primitives + self-improvement loop)
        .merge(cave_agent::router(agent_state))
        // Infrastructure & Networking
        ...
        // Middleware
        .layer(axum::middleware::from_fn(api_no_cache_layer))
    ...
    // `into_make_service_with_connect_info::<SocketAddr>()` is required by
    // every handler that extracts `ConnectInfo<SocketAddr>` (gateway proxy
    // fallback, DoH server, etc.). Without it, those handlers return
    // 500 "Missing request extension: ConnectInfo<SocketAddr>" — the
    // 2026-05-26 /api/parity bug that made the portal compliance page
    // un-loadable.
    axum::serve(
        listener,
...
/// `GET /api/parity` — serve the live `docs/parity/parity-index.json`
/// blob. Falls back to the build-time embedded snapshot when the file
/// is absent (e.g. binary running outside the source tree). 2026-05-27
/// portal-v3 ray.
async fn api_parity() -> axum::response::Response {
    use axum::http::{StatusCode, header};
    use axum::response::IntoResponse;

    const EMBEDDED: &str = include_str!("../../../../docs/parity/parity-index.json");
    let workspace_root =
        std::env::var("CAVE_WORKSPACE_ROOT").unwrap_or_else(|_| ".".into());
    let on_disk = std::path::Path::new(&workspace_root).join("docs/parity/parity-index.json");
    let body = std::fs::read_to_string(&on_disk).unwrap_or_else(|_| EMBEDDED.to_string());
    (
        StatusCode::OK,
