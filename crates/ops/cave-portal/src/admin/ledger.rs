...
use crate::admin::state::{AdminState, LedgerEntry, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<LedgerEntry>, LedgerViewError> {
    ctx.authorise(Permission::LedgerRead)?;
    Ok(
        scope(&state.ledger_entries.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
...
pub fn render(state: &AdminState, ctx: &RequestCtx) -> Result<String, LedgerViewError> {
    ...
    Ok(page_shell_full(
        ctx,
        "/admin/ledger",
        &format!("ledger · {}", escape(ctx.tenant.as_str())),
        &body,
    ))
}
...
const FILE_CITE: Cite = Cite::backstage(
    "plugins/ledger/src/components/EntriesList.tsx",
    "EntriesList",
);
