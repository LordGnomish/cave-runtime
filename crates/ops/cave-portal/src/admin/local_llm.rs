...
use crate::admin::state::{AdminState, LocalLlmModel, scope};
...
pub fn list_records(
    state: &AdminState,
    ctx: &RequestCtx,
) -> Result<Vec<LocalLlmModel>, LocalLlmViewError> {
    ctx.authorise(Permission::LocalLlmRead)?;
    Ok(
        scope(&state.local_llm_models.read().unwrap(), &ctx.tenant, |r| {
            &r.tenant
        })
        .into_iter()
        .cloned()
    ...
    let table_rows: Vec<Vec<String>> = rows
        ...
        .map(|r| {
            vec![
                ...
                r.quant.clone(),
                if r.loaded { "yes".into() } else { "no".into() },
            ]
        })
        .collect();
    let body = format!(
