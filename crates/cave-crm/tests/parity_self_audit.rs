...
//
//! Charter v2 self-audit — cave-crm must carry an honest, measured
//! `fill_ratio` against upstream Twenty (twentyhq/twenty) at the latest
//! stable release (v2.6.0), a pinned `source_sha` for reproducibility,
//! the 2026-05-19 close-out audit date, `parity_ratio_source = "manifest"`,
...
//! manifest counts and lands the MVP modules.
...
// ─── Assertion 3: fill_ratio is at least the MVP floor and ≤ 1.0 ────────────
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "cave-crm Charter v2 floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
// ─── Assertion 5: cave-crm is a workspace member ─────────────────────────────
...
// ─── Assertion 6: every .rs file carries AGPL SPDX ───────────────────────────
...
fn assertion_6_agpl_spdx_header_coverage() {
    ...
    assert!(
        missing.is_empty(),
        "{} of {} .rs files in cave-crm missing AGPL SPDX header: {:?}",
        missing.len(),
        total,
        missing
    );
...
// ─── Assertion 8: last_audit == 2026-05-19 ───────────────────────────────────
...
fn assertion_9_parity_index_json_consistency() {
    ...
    let i = idx
        .find("\"cave-crm\":")
        .expect("docs/parity/parity-index.json must contain cave-crm entry");
    let block_end = idx[i..]
        .find("\n    }")
        .expect("cave-crm block must close with `\\n    }`");
    let block = &idx[i..i + block_end];

    let pr_line = block
        .lines()
        .find(|l| l.contains("\"parity_ratio\":"))
        .expect("cave-crm block must contain parity_ratio");
    ...
    assert!(
        ...
        "parity-index.json cave-crm entry must declare parity_ratio_source = \"manifest\""
    );
}
