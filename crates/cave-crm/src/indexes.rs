...
impl IndexSet {
    ...
    pub fn default_specs() -> Vec<IndexSpec> {
        vec![
            IndexSpec {
                object_singular: "person".into(),
                columns: vec!["workspace_id".into()],
            ...
            IndexSpec {
                object_singular: "company".into(),
                columns: vec!["workspace_id".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "company".into(),
                columns: vec!["domain_name".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
            ...
            IndexSpec {
                object_singular: "opportunity".into(),
                columns: vec!["pipeline_step_id".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "opportunity".into(),
            ...
            IndexSpec {
                object_singular: "task".into(),
                columns: vec!["workspace_id".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "task".into(),
                columns: vec!["assignee_user_id".into(), "status".into()],
                kind: IndexKind::Composite,
            },
            IndexSpec {
                object_singular: "note".into(),
                columns: vec!["workspace_id".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "activity_target".into(),
                columns: vec!["activity_id".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "activity_target".into(),
            ...
            IndexSpec {
                object_singular: "calendar_event".into(),
                columns: vec!["starts_at".into()],
                kind: IndexKind::BTree,
            },
            IndexSpec {
                object_singular: "api_key".into(),
                columns: vec!["secret_hash".into()],
                kind: IndexKind::Unique,
            },
            IndexSpec {
                object_singular: "workspace_member".into(),
                columns: vec!["user_id".into(), "workspace_id".into()],
                kind: IndexKind::Unique,
            },
            IndexSpec {
                object_singular: "user".into(),
                columns: vec!["email".into()],
                kind: IndexKind::Unique,
            },
        ]
    ...
    pub fn seed_default_for_workspace(&mut self, workspace_id: Uuid) {
        self.by_workspace
            .insert(workspace_id, Self::default_specs());
    }
...
mod tests {
    ...
    fn default_specs_contains_critical_indexes() {
        ...
        assert!(
            s.iter()
                .any(|i| i.object_singular == "person" && i.columns == ["email"])
        );
        assert!(
            s.iter()
                .any(|i| i.object_singular == "user" && i.kind == IndexKind::Unique)
        );
        assert!(
            s.iter().any(
                |i| i.object_singular == "opportunity" && i.columns == ["status", "close_date"]
            )
        );
    }
