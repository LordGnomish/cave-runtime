...
//
// Charter v2 8-gate self-audit for cave-rdbms-operator (CloudNativePG v1.24.0
// + PgBouncer 1.21.0 parity).
//
// Gates:
//   1. SPDX coverage 100% of src/*.rs
//   2. source_sha pinned (CNPG 1.24.0 release tag)
...
fn gate_2_source_sha_pinned() {
    ...
    assert!(
        ...
        "manifest must declare source_sha pinning CloudNativePG release"
    );
    assert!(
        m.contains("1.24.0"),
        "source_sha must pin CloudNativePG 1.24.0"
    );
}
...
fn gate_5_fill_ratio_floor() {
    ...
    assert!(
        ratio >= 0.95,
        "fill_ratio = {} (need >= 0.95 — cave-rdbms-operator in-scope coverage)",
        ratio
    );
}
