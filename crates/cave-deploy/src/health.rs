...
impl HealthCheckRegistry {
    ...
    fn register_builtins(&mut self) {
        self.register(
            ResourceKey::new("apps", "v1", "Deployment"),
            check_deployment,
        );
        self.register(
            ResourceKey::new("apps", "v1", "StatefulSet"),
            check_statefulset,
        );
        self.register(ResourceKey::new("apps", "v1", "DaemonSet"), check_daemonset);
        self.register(
            ResourceKey::new("apps", "v1", "ReplicaSet"),
            check_replicaset,
        );
        ...
        self.register(
            ResourceKey::new("networking.k8s.io", "v1", "Ingress"),
            check_ingress,
        );
        self.register(
            ResourceKey::new("apiextensions.k8s.io", "v1", "CustomResourceDefinition"),
            check_crd,
        );
        self.register(
            ResourceKey::new("cert-manager.io", "v1", "Certificate"),
            check_certificate,
        );
        self.register(
            ResourceKey::new("argoproj.io", "v1alpha1", "Application"),
            check_argocd_app,
        );
    }
}
...
pub fn check_deployment(resource: &serde_json::Value) -> HealthCondition {
    ...
    if desired_replicas == 0 {
        return HealthCondition {
            ...
            message: Some("Deployment scaled to zero".to_string()),
        };
    }
...
pub fn check_statefulset(resource: &serde_json::Value) -> HealthCondition {
    ...
    if desired == 0 {
        return HealthCondition {
            status: HealthStatus::Suspended,
            message: Some("StatefulSet scaled to zero".to_string()),
        };
    }
...
pub fn check_daemonset(resource: &serde_json::Value) -> HealthCondition {
    ...
    if desired == 0 {
        return HealthCondition {
            status: HealthStatus::Healthy,
            message: Some("No nodes to schedule".to_string()),
        };
    }
...
pub fn check_replicaset(resource: &serde_json::Value) -> HealthCondition {
    ...
    if ready >= desired {
        ...
        HealthCondition {
            status: HealthStatus::Progressing,
            message: Some(format!("{}/{} ready", ready, desired)),
        }
    }
}
...
pub fn check_pod(resource: &serde_json::Value) -> HealthCondition {
    ...
    match phase {
        ...
        "Succeeded" => HealthCondition {
            status: HealthStatus::Healthy,
            message: Some("Pod completed".to_string()),
        },
        "Failed" => HealthCondition {
            status: HealthStatus::Degraded,
            message: Some("Pod failed".to_string()),
        },
        "Pending" => HealthCondition {
            status: HealthStatus::Progressing,
            message: Some("Pod pending".to_string()),
        },
        _ => HealthCondition {
            status: HealthStatus::Unknown,
            message: None,
...
pub fn check_pvc(resource: &serde_json::Value) -> HealthCondition {
    let phase = resource
        ...
        .unwrap_or("Unknown");
    match phase {
        "Bound" => HealthCondition {
            status: HealthStatus::Healthy,
            message: None,
        },
        "Pending" => HealthCondition {
            status: HealthStatus::Progressing,
            message: Some("PVC pending".to_string()),
        },
        "Lost" => HealthCondition {
            status: HealthStatus::Degraded,
            message: Some("PVC volume lost".to_string()),
        },
        _ => HealthCondition {
            status: HealthStatus::Unknown,
            message: None,
...
pub fn check_cronjob(resource: &serde_json::Value) -> HealthCondition {
    ...
    if suspended {
        HealthCondition {
            status: HealthStatus::Suspended,
            message: Some("CronJob suspended".to_string()),
        }
    } else {
        HealthCondition {
            status: HealthStatus::Healthy,
...
pub fn check_ingress(resource: &serde_json::Value) -> HealthCondition {
    ...
    if lb_ingress.map(|i| i.is_empty()).unwrap_or(true) {
        HealthCondition {
            status: HealthStatus::Progressing,
            message: Some("Waiting for ingress address".to_string()),
        }
    } else {
        HealthCondition {
            status: HealthStatus::Healthy,
...
pub fn check_crd(resource: &serde_json::Value) -> HealthCondition {
    ...
    HealthCondition {
        status: HealthStatus::Progressing,
        message: Some("CRD not yet established".to_string()),
    }
}
...
pub fn check_certificate(resource: &serde_json::Value) -> HealthCondition {
    ...
    HealthCondition {
        status: HealthStatus::Progressing,
        message: Some("Certificate provisioning".to_string()),
    }
}

pub fn check_argocd_app(resource: &serde_json::Value) -> HealthCondition {
    let health_status = resource
        .get("status")
