...
//! Charter v2 self-audit — cave-cloud-controller-manager's
//! `parity.manifest.toml` must pin kubernetes/kubernetes v1.36.0, carry
//! a `source_sha`, the 2026-05-19 FINALIZE audit date, structurally
//! consistent counts, an honest measured `fill_ratio`, and full AGPL
//! SPDX coverage across every `.rs` file in the crate.
//!
//! Until 2026-05-19 the manifest only carried a file-level [[files]]
//! list — no [parity] inventory, no `source_sha`, no `last_audit`. This
//! file is the RED signal that converted that legacy shape into a
//! subsystem-level inventory.
...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    assert!(
        ...
        "cave-cloud-controller-manager measured floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-cloud-controller-manager IS a parity surface vs kubernetes/kubernetes cloud-provider — infra_only must be false"
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        ...
        "expected >= {} .rs files in cave-cloud-controller-manager; got {}",
        FLOOR_RS_FILES,
        total
    );
}
