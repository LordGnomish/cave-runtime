...
use crate::models::{
    Confidence, Ecosystem, Finding, FindingCategory, ScanKind, ScanRequest, ScanTarget, Severity,
};
    ...
    async fn scan(&self, req: &ScanRequest) -> Result<Vec<Finding>, ScanError> {
        match &req.target {
            ScanTarget::PackageName { ecosystem, name } => {
                ...
                if self.is_typosquat(ecosystem, name) {
                    ...
                    f.remediation = Some(format!(
                        "Verify the package '{}' is from a trusted source before installing",
                        name
                    ));
            ...
            _ => Err(ScanError::InvalidRequest(
                "Expected PackageName target".to_string(),
            )),
        }
    }
}
...
mod tests {
    ...
    fn test_levenshtein_distance() {
        assert_eq!(
            NamespaceScanner::levenshtein_distance("requests", "requets"),
            1
        );
        assert_eq!(
            NamespaceScanner::levenshtein_distance("requests", "requests"),
            0
        );
