...
//
//! Charter v2 self-audit — cave-kamaji must carry an honest,
//! measured `fill_ratio` against upstream clastix-labs/kamaji
//! v1.0.0, a pinned `source_sha` for reproducibility, the
//! 2026-05-19 close-out audit date, `parity_ratio_source =
//! "manifest"`, 100% AGPL SPDX header coverage, no stub macros in
//! `src/`, and the TenantControlPlane CRD shape with its 5-phase
//! lifecycle + kubeconfig generator reachable through the public
//! API.
//!
...
const PINNED_VERSION: &str = "v1.0.0";
...
// ─── Assertion 1: upstream pinned to v1.0.0 ─────────────────────────────────
...
fn assertion_1_upstream_version_pinned() {
    ...
    assert_eq!(
        v.as_deref(),
        Some(PINNED_VERSION),
        "[upstream] version must pin Kamaji {} — Charter v2 always-latest gate (got {:?})",
        PINNED_VERSION,
        v
    );
}
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "cave-kamaji MVP floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
...
// ─── Assertion 6: counts sum to total + >= 5 mapped ─────────────────────────
...
fn assertion_6_counts_sum_to_total() {
    ...
    assert!(
        mapped >= 5,
        "cave-kamaji MVP floor: >= 5 mapped Kamaji subsystems (got {})",
        mapped
    );
}
...
fn assertion_7_agpl_spdx_header_coverage() {
    ...
    assert!(
        total >= 4,
        "expected >= 4 .rs files in cave-kamaji; got {}",
        total
    );
}
...
// ─── Assertion 9: TenantControlPlane lifecycle + kubeconfig surface ─────────

#[test]
fn assertion_9_tcp_lifecycle_and_kubeconfig_surface() {
    use cave_kamaji::lifecycle;
    use cave_kamaji::models::{
        CreateTenantRequest, TenantControlPlane, TenantPhase, TenantSpec, TenantStatus,
    };
    use cave_kamaji::KamajiState;
    use chrono::Utc;
    use uuid::Uuid;

    // 5-phase enum complete (Provisioning/Running/Upgrading/Deleting/Failed)
    let _phases = [
        TenantPhase::Provisioning,
        TenantPhase::Running,
        TenantPhase::Upgrading,
    ...
    // lifecycle transitions: provision → mark_running → health_check passes → kubeconfig generates
    lifecycle::provision(&mut tcp);
    assert_eq!(tcp.status.phase, TenantPhase::Provisioning);
    lifecycle::mark_running(&mut tcp, "https://1.2.3.4:6443".into());
    assert_eq!(tcp.status.phase, TenantPhase::Running);
    assert!(lifecycle::health_check(&tcp), "running + ready must health-check");

    let kubeconfig = lifecycle::generate_kubeconfig(&tcp);
    assert!(kubeconfig.is_some(), "kubeconfig must be generated once api_server_endpoint is set");
    let kc = kubeconfig.unwrap();
    assert_eq!(kc["apiVersion"], "v1");
    assert_eq!(kc["kind"], "Config");

    // CreateTenantRequest exists for the REST entry-point
    let _req = CreateTenantRequest {
        name: "x".into(),
        namespace: "y".into(),
        spec: TenantSpec {
            kubernetes_version: "v1.31.0".into(),
    ...
    // KamajiState constructible
    let _state = KamajiState::default();
}
