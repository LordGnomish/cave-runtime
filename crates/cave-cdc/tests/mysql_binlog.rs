...
use cave_cdc::mysql::{BinlogEventType, BinlogPosition};
...
fn include_schemas_filter_gates_downstream_emission() {
    let mut c = MySqlConnector::new(format!("{}-mysql", TENANT), TENANT, "shop", 1042);
    ...
    assert!(
        !c.should_emit("audit"),
        "audit not in allow-list ⇒ filtered out"
    );
}
...
fn record_position_persists_checkpoint_and_gtid() {
    ...
    assert_eq!(
        c.last_gtid(),
        Some("3E11FA47-71CA-11E1-9E33-C80AA9429562:23")
    );

    // Bad position is rejected — checkpoint not advanced.
    let bad = BinlogPosition {
        file: "no-dot".into(),
        pos: 0,
    };
    ...
    assert_eq!(
        c.last_committed_position(),
        &pos,
        "checkpoint unchanged on rejection"
    );
}
