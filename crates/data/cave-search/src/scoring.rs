...
//!
//! where:
//!   IDF(qi)  = ln((N - df + 0.5) / (df + 0.5) + 1)   [smooth Lucene variant]
//!   TF_norm  = tf × (k1+1) / (tf + k1 × (1−b + b×|D|/avgdl))
//!   k1 = 1.2, b = 0.75  (Manticore defaults)
//!
//! upstream: manticoresoftware/manticoresearch v25.8.2 — src/sphinxsearch.cpp BM25 ranker
...
impl Default for BM25Params {
    fn default() -> Self { BM25Params { k1: 1.2, b: 0.75 } }
}
...
/// Compute BM25 score for a single term occurrence (default parameters).
pub fn bm25_score(
    term_freq: u32,
    doc_len: u32,
    avg_doc_len: f64,
...
/// Tie-breaking: lower doc_id ranks first.
...
/// Multi-term BM25 search: tokenize `query`, accumulate per-doc scores, return ranked results.
