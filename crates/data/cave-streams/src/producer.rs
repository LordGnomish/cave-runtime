...
// ─────────────────────────────────────────────────────────────────────────
// Kafka DefaultPartitioner / Utils.murmur2 behavioural-parity tests
// (Apache Kafka 4.2.0 clients/.../common/utils/UtilsTest.testMurmur2 +
// clients/.../producer/internals/DefaultPartitioner). These live in-crate
// because `producer`/`models` are crate-private modules — an external
// integration test cannot import them.
// ─────────────────────────────────────────────────────────────────────────
#[cfg(test)]
mod murmur2_parity_tests {
    use super::{choose_partition, default_partition, kafka_murmur2, to_positive};
    use crate::models::PartitionerStrategy;
    ...
    fn murmur2_strategy_falls_back_to_round_robin_without_key() {
        // Null/empty key → sticky round-robin, exactly like DefaultPartitioner.
