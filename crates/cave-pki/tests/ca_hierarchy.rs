...
fn root_ca_can_only_be_generated_once() {
    let mut ca = Ca::new();
    let root1 = ca
        .generate_root("Cave Sovereign Root", KeyAlgorithm::EcdsaP384, 20)
        .unwrap();
    let err = ca
        .generate_root("Cave Sovereign Root", KeyAlgorithm::EcdsaP384, 20)
        .unwrap_err();
    ...
    assert!(
        handle.hardware_backed,
        "Root CA key MUST be HSM-backed (NIST SP 800-57)"
    );
...
fn platform_intermediate_requires_root_parent() {
    let mut ca = Ca::new();
    let err = ca
        .generate_platform_intermediate("Cave Platform CA", KeyAlgorithm::EcdsaP384)
        .unwrap_err();
    ...
    assert_eq!(
        handle.issuer_serial.as_deref(),
        Some(ca.root_serial().unwrap())
    );
    assert!(
        !handle.hardware_backed,
...
fn tenant_intermediate_is_one_per_tenant_idempotent() {
    ...
    let t_a = ca
        .generate_tenant_intermediate(TENANT_A, KeyAlgorithm::EcdsaP256)
        .unwrap();
    let t_a_dup = ca
        .generate_tenant_intermediate(TENANT_A, KeyAlgorithm::EcdsaP256)
        .unwrap();
    ...
    let t_b = ca
        .generate_tenant_intermediate(TENANT_B, KeyAlgorithm::Ed25519)
        .unwrap();
    assert_ne!(t_a, t_b);
...
fn key_algorithm_parsing_includes_pqc_hybrid() {
    ...
    assert!(
        EcdsaP256.requires_hsm_for_root(),
        "every root MUST be HSM-backed"
    );
}
