    ...
    async fn access(&self, ctx: &mut PluginCtx, config: &Value) -> PluginResult {
        ...
        if !deny.is_empty() && ip_matches(&ip, &deny) {
            return PluginResult::Halt(
                (StatusCode::FORBIDDEN, axum::Json(serde_json::json!({"message": "Your IP address is not allowed"}))).into_response(),
            );
        }
...
mod tests {
    ...
    fn ctx_with_ip(ip: &str) -> PluginCtx {
        PluginCtx::new("GET".into(), "/".into(), HashMap::new(), Bytes::new(), ip.into())
    }
