    ...
    async fn access(&self, ctx: &mut PluginCtx, config: &Value) -> PluginResult {
        ...
        match found_key {
            Some(key) => {
                ...
                ctx.ctx.insert("key_auth_key".to_string(), Value::String(key));
                PluginResult::Continue
            }
            None => {
                if let Some(_anon) = anonymous {
                    // Allow anonymous — consumer will be set to anonymous consumer
                    ctx.ctx.insert("key_auth_anonymous".to_string(), Value::Bool(true));
                    PluginResult::Continue
                } else {
                    PluginResult::Halt(
                        (
                            StatusCode::UNAUTHORIZED,
                            axum::Json(serde_json::json!({"message": "No API key found in request"})),
                        )
                            .into_response(),
                    )
                }
