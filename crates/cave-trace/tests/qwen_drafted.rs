...
mod tests {
    use cave_trace::propagation::{
        FLAG_SAMPLED, PropagationError, TRACEPARENT, TRACESTATE, VERSION, extract_or_new, inject,
    ...
    fn parse_traceparent_valid_sampled() {
        ...
        assert!(
            result.is_ok(),
            "Expected valid traceparent, got error: {:?}",
            result.err()
        );
    ...
    fn inject_parse_round_trip() {
        ...
        // Parse it back
        let parsed_tp = parse_traceparent(&injected_tp_str)
            .expect("Round-trip injection should yield valid traceparent");
