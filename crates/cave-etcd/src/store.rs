...
mod tests {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // sweep-001 — etcd parity push (+50 tests)
    ...
    // ── MVCC: revision monotonicity & header revision (TestKVRangeRev/TestKVPutOnHeader) ──

    #[test]
    fn etcd_parity_put_response_header_revision_is_post_put() {
        // upstream: etcd PutResponse.Header.Revision == revision after the put applied
        let store = KvStore::new();
    ...
    fn etcd_parity_delete_response_header_advances_revision() {
        // upstream: DeleteRange advances revision even if no keys deleted
        let store = KvStore::new();
        let r0 = store.current_revision();
        let resp = store.delete_range(&DeleteRangeRequest {
            key: "ghost".into(), range_end: None, prev_kv: false,
        });
        // etcd actually advances rev for delete attempts (matches our impl: next_revision called eagerly)
    ...
    fn etcd_parity_put_creates_then_updates_records_distinct_revs() {
        // upstream: TestKVPutOverwrite — create_revision stable, mod_revision advances
        let store = KvStore::new();
    ...
    // ── MVCC: range edge cases (TestKVRangeBadRev, TestKVRangeBadKey) ──

    #[test]
    fn etcd_parity_range_at_future_revision_returns_empty_for_unknown_key() {
        // upstream: querying future rev for a key that wasn't there returns no kvs
        let store = KvStore::new();
        let resp = store.range(&RangeRequest {
    ...
    fn etcd_parity_range_limit_zero_means_no_limit() {
        // upstream: limit=0 in etcd means unbounded; we model via Option<None>
        let store = KvStore::new();
    ...
    fn etcd_parity_range_keys_returned_sorted_lexicographically() {
        // upstream: etcd guarantees lex-ordered output for range queries
        let store = KvStore::new();
    ...
    // ── Txn: nested ops & atomicity (TestV3TxnAtomicity, TestTxnWithLimit) ──
    ...
    fn etcd_parity_txn_compare_version_zero_for_nonexistent_key() {
        // upstream behavior: nonexistent key has version 0
        let store = KvStore::new();
        let resp = store.txn(&TxnRequest {
            compare: vec![Compare {
    ...
    fn etcd_parity_txn_serialises_via_lock_under_concurrent_writers() {
        // upstream: Txn must serialise compare+execute so two CAS attempts cannot both succeed
        use std::sync::Arc;
        use std::thread;
        let store = Arc::new(KvStore::new());
        ...
        for _ in 0..8 {
            ...
            handles.push(thread::spawn(move || {
                // Attempt CAS: only succeeds when value==0; if success, write "1"
        ...
        // Exactly one CAS must succeed (race-free serialisation under txn_lock).
        assert_eq!(success_counter.load(std::sync::atomic::Ordering::SeqCst), 1);
    }

    // ── Watch: filter, replay, fan-out (TestV3WatchFromCurrentRevision, TestV3WatchCancelSynced) ──

    #[test]
    fn etcd_parity_watch_replays_events_at_or_after_start_revision() {
        let store = KvStore::new();
        pk_put(&store, "/n/a", "1"); // rev 2
        let start = store.current_revision(); // points at rev 2
        pk_put(&store, "/n/b", "2");
        pk_put(&store, "/n/c", "3");
        let resp = store.watch_create(&WatchCreateRequest {
        ...
        assert_eq!(resp.events.len(), 2); // only b and c (start+1 onward)
    }
    ...
    fn etcd_parity_watch_replay_skips_compacted_revisions() {
        // upstream: get_historical_events starts at max(from_rev, compacted+1)
        let store = KvStore::new();
        pk_put(&store, "k", "v1");
        pk_put(&store, "k", "v2");
        let mid = store.current_revision();
        pk_put(&store, "k", "v3");
        // compact mid+1 so revision <= mid is gone
        store.compact(mid + 1);
        let resp = store.watch_create(&WatchCreateRequest {
            key: "k".into(), range_end: None,
            start_revision: Some(1), progress_notify: false, prev_kv: false,
        });
        // Only v3 (after compaction boundary) should be replayed
        for e in &resp.events {
            // every replayed event must carry value "v3" (others were compacted away)
            assert_eq!(e.kv.value_str(), "v3");
        }
    }

    // ── Lease: TTL handling, expiry, key cleanup (TestV3LeaseGrant/Revoke/KeepAlive) ──
    ...
    fn etcd_parity_lease_grant_with_explicit_id_uses_id() {
        ...
        // Subsequent timetolive should find it
    ...
    fn etcd_parity_lease_put_overwrite_with_different_lease_releases_old() {
        // upstream: a put that changes lease must remove key from the old lease's keys
        let store = KvStore::new();
        ...
        assert!(l1_keys.is_empty(), "key must have been released from old lease");
    ...
    fn etcd_parity_lease_revoke_does_not_panic_on_missing_keys() {
        ...
        // External delete first
        store.delete_range(&DeleteRangeRequest { key: "k".into(), range_end: None, prev_kv: false });
        // Revoke must still succeed even though the lease key is already gone
    ...
    fn etcd_parity_lease_leases_lists_all_active_only() {
        ...
        let resp = store.lease_leases();
        let ids: Vec<i64> = resp.leases.iter().map(|l| l.id).collect();
        assert!(!ids.contains(&l1.id), "revoked lease must not appear");
        assert!(ids.contains(&l2.id));
    }

    // ── Auth: enable/disable, password, permission flow (TestAuthEnable/Disable/Permission) ──

    #[test]
    fn etcd_parity_auth_token_invalidated_after_disable_then_reenable() {
        // upstream: tokens issued under a previous auth-enabled session must be re-issued
        // after disable/re-enable. Our impl keeps tokens; test what we DO guarantee:
        // a fresh token is required to validate after re-enable when user changed password.
        let store = KvStore::new();
        ...
        // root user not present, but token validates against username; with no granted role,
        // any non-readwrite check should be denied even after re-enable.
        store.auth_disable().unwrap();
        store.auth_enable().unwrap();
    ...
    fn etcd_parity_auth_role_grant_permission_replaces_existing_for_same_key() {
        // upstream: granting again with same key replaces (no duplicate)
        let store = KvStore::new();
        store.role_add(&AuthRoleAddRequest { name: "r".into() }).unwrap();
        store.role_grant_permission(&AuthRoleGrantPermissionRequest {
    ...
    // ── Alarm: type & action coverage (TestV3Alarm) ──
    ...
    fn etcd_parity_alarm_activate_is_idempotent_per_member_type() {
        // upstream: activating twice does not double-insert
        let store = KvStore::new();
        store.alarm(&AlarmRequest { action: AlarmAction::Activate, member_id: 1, alarm: AlarmType::Nospace });
        store.alarm(&AlarmRequest { action: AlarmAction::Activate, member_id: 1, alarm: AlarmType::Nospace });
        let resp = store.alarm(&AlarmRequest { action: AlarmAction::Get, member_id: 0, alarm: AlarmType::None });
    ...
    // ── Compaction: revisions, idempotency (TestKVCompactReserveLastValue) ──
    ...
    fn etcd_parity_compact_idempotent_at_same_revision() {
        ...
        let rev = store.current_revision();
        store.compact(rev);
        // Calling again at same rev should not panic or change state
        store.compact(rev);
        assert_eq!(pk_get(&store, "k").unwrap().value_str(), "v");
    }
    ...
    fn etcd_parity_compact_preserves_latest_value_for_each_key() {
        // upstream: even after compaction, the most recent value for each live key remains accessible
        let store = KvStore::new();
        pk_put(&store, "k", "v1");
        pk_put(&store, "k", "v2");
        pk_put(&store, "k", "v3");
    ...
    fn etcd_parity_compact_then_read_at_higher_rev_succeeds() {
        ...
        let r2 = store.current_revision();
        store.compact(r1);
        // r2 is uncompacted, must still resolve
        let resp = store.range(&RangeRequest {
            key: "k".into(), range_end: None, limit: None,
    ...
    // ── Cluster: member operations (TestMemberAdd/Remove/Update) ──
    ...
    // ── Maintenance: hash & snapshot consistency (TestKVHashKV/Snapshot) ──
    ...
    fn etcd_parity_response_header_cluster_id_constant() {
        // upstream: header.cluster_id stable across ops in same cluster
        let store = KvStore::new();
    ...
    // ── Boundary conditions: empty keys & values ──

    #[test]
    fn etcd_parity_put_then_range_empty_key_handled() {
        // upstream allows empty range_end with empty key — represents "all keys"
        let store = KvStore::new();
        pk_put(&store, "z", "v");
        let resp = store.range(&RangeRequest {
    ...
    fn etcd_parity_put_large_value_handled() {
        let store = KvStore::new();
        let big_val: String = "a".repeat(64 * 1024); // 64KiB
    ...
    // ── MVCC: range_at_revision + delete events ──
    ...
    fn etcd_parity_range_at_revision_after_compaction_boundary_works() {
        ...
        let r1 = store.current_revision();
        pk_put(&store, "k", "v2");
        store.compact(r1); // compact up to r1 (exclusive)
        // Reading at the exact compacted boundary may or may not error;
        // reading at r1 is at boundary — implementation says < compacted errors.
        // r1 == compacted → not less than, so allowed.
        let resp = store.range(&RangeRequest {
            key: "k".into(), range_end: None, limit: None,
            revision: Some(r1), keys_only: false, count_only: false,
        });
        // Either ok with v1 or compacted error — but never v2.
        if let Ok(r) = resp {
    ...
    fn etcd_parity_watch_no_replay_when_no_start_revision() {
        // upstream: omitting start_revision → no historical replay; empty events
        let store = KvStore::new();
        pk_put(&store, "k", "v1");
        let resp = store.watch_create(&WatchCreateRequest {
            key: "k".into(), range_end: None,
