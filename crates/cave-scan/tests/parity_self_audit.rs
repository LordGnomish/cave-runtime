...
//! Charter v2 self-audit — cave-scan's own `parity.manifest.toml` must
//! carry an honest measured `fill_ratio` against SonarQube v10.4.1, a
//! pinned `source_sha` for reproducibility, the 2026-05-18 close-out
//! audit date, and structurally consistent bucket counts.
//!
//! Read-only audit on 2026-05-19 surfaced cave-scan as "kısmen": 48 real
//! tests + LCOV/Cobertura coverage parsing + keyword rule engine + severity
//! taxonomy, but `[parity].ratio = 0.0`, no `source_sha`, no
//! `parity_ratio_source`, no `[[mapped]]` / `[[partial]]` / `[[unmapped]]`
//! breakdown — i.e. the Charter v2 `parity_self_audit` test had never been
//! written. This file is the RED signal — every gate is asserted explicitly
//! so a future drift in any single field surfaces as a localised test failure
//! rather than a silent audit-doc regression.
...
fn upstream_source_sha_is_present_and_matches_version() {
    ...
    assert!(
        ...
        "manifest [upstream] source_sha must be set for reproducibility \
         (got {:?}). Charter v2 close-out: pin to the v10.4.1 tag tree-ish.",
        sha
    );
    assert_eq!(
        sha.as_deref(),
...
fn parity_fill_ratio_is_measured_and_at_least_0_65() {
    ...
    assert!(
        ratio >= 0.65,
        "cave-scan Charter v2 close-out floor: fill_ratio must be >= 0.65 \
         (got {}). cave-scan is honestly a narrow MVP port; lift only by \
         categorising newly-skipped subsystems or by porting unmapped ones, \
         never by inflating mapped.",
        ratio
    );
    assert!(
        ratio <= 1.0,
...
fn parity_honest_ratio_is_present_and_le_fill_ratio() {
    ...
    assert!(
        honest >= 0.5,
        "honest_ratio floor for cave-scan narrow MVP: >= 0.5 (got {})",
        honest
    );
}
...
#[test]
fn counts_sum_to_total_and_at_least_eight_mapped() {
    let m = manifest_text();
    ...
    assert!(
        mapped >= 8,
        "expected >= 8 mapped SonarQube subsystems (got {})",
        mapped
    );
}
        ...
        if p.extension().map(|e| e == "rs").unwrap_or(false) {
            if let Ok(s) = fs::read_to_string(p) {
                for (i, line) in s.lines().enumerate() {
                    // Strip line comments so the `// TODO:` style notes and
                    // any cave-scan keyword-rule pattern strings ("todo!\\s*\\(")
                    // do not register as real stub-macro call sites.
                    let code = line.split("//").next().unwrap_or("");
                    // Require ! immediately followed by ( so we don't catch
                    // cave-scan's *string literals* describing the macros.
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(total >= 6, "expected >= 6 .rs files in cave-scan; got {}", total);
}
