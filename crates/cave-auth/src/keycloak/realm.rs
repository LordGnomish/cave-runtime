...
impl RealmStore {
    ...
    pub async fn update(&self, id: &str, req: RealmUpdateRequest) -> Result<Realm, &'static str> {
        ...
        if let Some(v) = req.ssl_required { realm.ssl_required = v; }
        if let Some(v) = req.registration_allowed { realm.registration_allowed = v; }
        if let Some(v) = req.login_with_email_allowed { realm.login_with_email_allowed = v; }
        if let Some(v) = req.duplicate_emails_allowed { realm.duplicate_emails_allowed = v; }
        if let Some(v) = req.access_token_lifespan { realm.access_token_lifespan = v; }
        if let Some(v) = req.sso_session_idle_timeout { realm.sso_session_idle_timeout = v; }
        Ok(realm.clone())
    }
...
pub async fn create_realm(
    ...
    match store.create(req).await {
        Ok(realm) => (StatusCode::CREATED, Json(serde_json::to_value(realm).unwrap())).into_response(),
        Err("realm_exists") => (StatusCode::CONFLICT, Json(serde_json::json!({"error":"realm already exists"}))).into_response(),
...
pub async fn get_realm(
    ...
    match store.get(&realm).await {
        ...
        None => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"realm not found"}))).into_response(),
    }
}
...
pub async fn update_realm(
    ...
    match store.update(&realm, req).await {
        ...
        Err("not_found") => (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"realm not found"}))).into_response(),
        Err(_) => StatusCode::INTERNAL_SERVER_ERROR.into_response(),
    }
}
...
pub async fn delete_realm(
    ...
    if store.delete(&realm).await {
        ...
        (StatusCode::NOT_FOUND, Json(serde_json::json!({"error":"realm not found"}))).into_response()
    }
}
...
mod tests {
    ...
    async fn test_get_realm_success() {
        ...
        store.create(RealmRequest { id: "testrealm".to_string(), display_name: Some("Test".to_string()), enabled: None, ssl_required: None, registration_allowed: None, login_with_email_allowed: None, duplicate_emails_allowed: None, access_token_lifespan: None, sso_session_idle_timeout: None }).await.unwrap();
    ...
    async fn test_update_realm() {
        let store = RealmStore::new();
        store.create(RealmRequest { id: "updatable".to_string(), display_name: None, enabled: None, ssl_required: None, registration_allowed: None, login_with_email_allowed: None, duplicate_emails_allowed: None, access_token_lifespan: None, sso_session_idle_timeout: None }).await.unwrap();
        ...
        let put_resp = app
            .oneshot(
                Request::builder()
                    ...
                    .header("content-type", "application/json")
                    .body(Body::from(serde_json::json!({"display_name":"New Name"}).to_string()))
                    .unwrap(),
            )
            .await
            .unwrap();
    ...
    async fn test_delete_realm() {
        let store = RealmStore::new();
        store.create(RealmRequest { id: "todelete".to_string(), display_name: None, enabled: None, ssl_required: None, registration_allowed: None, login_with_email_allowed: None, duplicate_emails_allowed: None, access_token_lifespan: None, sso_session_idle_timeout: None }).await.unwrap();
        let app = router(store.clone());
    ...
    async fn test_list_realms() {
        ...
        for name in &["realm-alpha", "realm-beta"] {
            store.create(RealmRequest { id: name.to_string(), display_name: None, enabled: None, ssl_required: None, registration_allowed: None, login_with_email_allowed: None, duplicate_emails_allowed: None, access_token_lifespan: None, sso_session_idle_timeout: None }).await.unwrap();
        }
