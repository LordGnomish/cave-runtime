...
use super::{
    authz_request::{validate, AuthzRequest, ResponseKind, AuthzError},
...
pub async fn authorize_get(
            ...
            match state.par.take(&uri).await {
                Some(rec) => {
                    if Utc::now().timestamp() > rec.exp_unix {
                        return error_redirect("invalid_request_uri", "request_uri expired", &req).into_response();
                    }
                ...
                None => return error_redirect("invalid_request_uri", "unknown request_uri", &req).into_response(),
            }
        }
    }
    ...
    if validated.raw.prompt.as_deref().map(|p| p.split_whitespace().any(|t| t == "none")).unwrap_or(false) {
    ...
    let client = match state.clients.get_by_client_id(&realm, &validated.raw.client_id).await {
        Some(c) => c,
...
pub async fn authorize_post(
    ...
    if let Some(v) = form.get("client_id") { if !v.is_empty() { q.client_id = v.clone(); } }
    if let Some(v) = form.get("redirect_uri") { if !v.is_empty() { q.redirect_uri = v.clone(); } }
    if let Some(v) = form.get("response_type") { if !v.is_empty() { q.response_type = v.clone(); } }
    if let Some(v) = form.get("scope") { q.scope = Some(v.clone()); }
    if let Some(v) = form.get("state") { q.state = Some(v.clone()); }
    if let Some(v) = form.get("nonce") { q.nonce = Some(v.clone()); }
    if let Some(v) = form.get("code_challenge") { q.code_challenge = Some(v.clone()); }
    if let Some(v) = form.get("code_challenge_method") { q.code_challenge_method = Some(v.clone()); }
    ...
    if !state.users.verify_password(&realm, user.id, &password).await {
    ...
    state.codes.put(AuthorizationCode {
        ...
        scope: validated.raw.scope.clone().unwrap_or_else(|| "openid".into()),
    ...
    let redirect = format!("{}{}{}", validated.raw.redirect_uri, separator, params.join("&"));
...
pub fn router(state: OAuthEndpointsState) -> Router {
    Router::new()
        .route("/realms/{realm}/protocol/openid-connect/auth", get(authorize_get).post(authorize_post))
        .with_state(state)
}
...
mod tests {
    ...
    async fn setup() -> (Router, OAuthEndpointsState) {
        ...
        realms.create(RealmRequest {
            ...
            registration_allowed: None, login_with_email_allowed: None, duplicate_emails_allowed: None,
            access_token_lifespan: None, sso_session_idle_timeout: None,
        }).await.unwrap();

        let users = UserStore::new();
        users.create("myrealm", CreateUserRequest {
            username: "alice".into(), email: Some("a@x".into()), email_verified: Some(true),
        ...
        clients.create("myrealm", CreateClientRequest {
            client_id: "app".into(), name: None, description: None, enabled: Some(true),
    ...
    async fn authorize_get_renders_login_form() {
        ...
        let resp = app.oneshot(Request::builder().method("GET").uri(uri).body(Body::empty()).unwrap()).await.unwrap();
    ...
    async fn authorize_post_emits_code_redirect() {
        ...
        let resp = app.oneshot(Request::builder().method("POST").uri(uri)
    ...
    async fn par_request_uri_is_resolved() {
        ...
        let url = format!("/realms/myrealm/protocol/openid-connect/auth?request_uri={}", urlencoding::encode(&par_uri));
        let resp = app.oneshot(Request::builder().method("GET").uri(url).body(Body::empty()).unwrap()).await.unwrap();
        // PAR is consumed.
