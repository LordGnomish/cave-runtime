...
use crate::s3::types::{StorageClass, VersioningState};
...
async fn test_bucket_create_list_delete() {
    ...
    store
        .create_bucket("my-bucket", "us-east-1", "cave")
        .await
        .unwrap();
...
async fn test_bucket_name_validation() {
    ...
    assert!(
        store
            .create_bucket("AB", "us-east-1", "cave")
            .await
            .is_err()
    ); // too short + uppercase
    ...
    assert!(
        store
            .create_bucket("-bad", "us-east-1", "cave")
            .await
            .is_err()
    ); // starts with -
    assert!(
        store
            .create_bucket("good-bucket", "us-east-1", "cave")
            .await
            .is_ok()
    );
}
...
async fn test_put_get_delete_object() {
    ...
    store
        .create_bucket("test", "us-east-1", "cave")
        .await
        .unwrap();
    ...
    // Delete
    store
        .delete_object("test", "hello.txt", None)
        .await
        .unwrap();
    assert!(
        store
...
async fn test_object_metadata() {
    ...
    let obj = store
        .get_object("test", "obj", None, None, None)
        .await
        .unwrap();
    assert_eq!(
        obj.metadata.get("author").map(|s| s.as_str()),
        Some("alice")
    );
    assert_eq!(
        obj.metadata.get("project").map(|s| s.as_str()),
        Some("cave")
    );
}
...
async fn test_object_tagging() {
    ...
    store
        .put_object_tagging("test", "obj", None, tags.clone())
        .await
        .unwrap();
...
async fn test_versioning() {
    ...
    store
        ...
        .unwrap();
    store
        .set_versioning("test", VersioningState::Enabled)
        .await
        .unwrap();
...
async fn test_list_objects_v2() {
    ...
    // List all
    let all = store
        .list_objects_v2("test", "", None, 100, None)
        .await
        .unwrap();
    ...
    // Prefix filter
    let a_prefix = store
        .list_objects_v2("test", "a/", None, 100, None)
        .await
        .unwrap();
    ...
    let with_delim = store
        .list_objects_v2("test", "", Some("/"), 100, None)
        .await
        .unwrap();
    ...
    // Max keys
    let limited = store
        .list_objects_v2("test", "", None, 2, None)
        .await
        .unwrap();
...
async fn test_copy_object() {
    ...
    store
        .create_bucket("src", "us-east-1", "cave")
        .await
        .unwrap();
    store
        .create_bucket("dst", "us-east-1", "cave")
        .await
        .unwrap();
    ...
    let got = store
        .get_object("dst", "copy.txt", None, None, None)
        .await
        .unwrap();
...
async fn test_multipart_upload() {
    ...
    let etag1 = store
        .upload_part(&upload_id, 1, part1_data.clone())
        .await
        .unwrap();
    let etag2 = store
        .upload_part(&upload_id, 2, part2_data.clone())
        .await
        .unwrap();
    let etag3 = store
        .upload_part(&upload_id, 3, part3_data.clone())
        .await
        .unwrap();
...
async fn test_abort_multipart() {
    ...
    store
        .upload_part(&upload_id, 1, vec![b'A'; 1024])
        .await
        .unwrap();
...
async fn test_sse_s3_encryption() {
    ...
    let obj = store
        .get_object("test", "secret.txt", None, None, None)
        .await
        .unwrap();
...
async fn test_delete_objects_batch() {
    ...
    let entries = vec![
        crate::s3::store::DeleteObjectEntry {
            key: "a".to_string(),
            version_id: None,
        },
        crate::s3::store::DeleteObjectEntry {
