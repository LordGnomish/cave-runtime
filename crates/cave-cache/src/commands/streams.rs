...
use crate::resp::Resp;
use crate::types::{
    bytes_to_i64, Consumer, ConsumerGroup, Entry, PendingEntry, Stream, StreamEntry, StreamId,
    Value,
};
...
pub fn cmd_xadd(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 5 { return Err(CacheError::wrong_arity("xadd")); }
    ...
    // Parse options
    loop {
        if i >= args.len() { return Err(CacheError::Syntax); }
        match args[i].to_ascii_uppercase().as_slice() {
            b"MAXLEN" => {
                i += 1;
                let approx = if i < args.len() && args[i] == b"~" { i += 1; true } else { false };
            ...
            b"MINID" => {
                i += 1;
                let _approx = if i < args.len() && args[i] == b"~" { i += 1; true } else { false };
                minid = Some(StreamId::parse(&args[i]).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))?);
                i += 1;
            }
            b"NOMKSTREAM" => { i += 1; }
            _ => break,
    ...
    let ms = now_ms();
    let id = if id_raw == b"*" {
        let seq = if stream.last_id.ms == ms { stream.last_id.seq + 1 } else { 0 };
        StreamId { ms, seq }
    } else {
        let parsed = StreamId::parse(id_raw).ok_or_else(|| CacheError::generic("ERR Invalid stream ID specified as stream command argument"))?;
        // Handle partial ID like "123-*"
        if id_raw.ends_with(b"-*") || id_raw.ends_with(b"*") && !id_raw.starts_with(b"*") {
            // Use existing ms, increment seq
            if parsed.ms < stream.last_id.ms || (parsed.ms == stream.last_id.ms && parsed.seq <= stream.last_id.seq) {
...
pub fn cmd_xread(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xread")); }
    ...
    while i < args.len() {
        match args[i].to_ascii_uppercase().as_slice() {
            b"COUNT" => { i += 1; count = Some(bytes_to_i64(&args[i]).ok_or(CacheError::NotInteger)? as usize); i += 1; }
            b"BLOCK" => { i += 1; block = Some(bytes_to_i64(&args[i]).ok_or(CacheError::NotInteger)? as u64); i += 1; }
            b"STREAMS" => { i += 1; break; }
            _ => { i += 1; }
        }
    }
    ...
    if num_streams == 0 { return Err(CacheError::Syntax); }
...
fn xrange_impl(args: &[Vec<u8>], db: &mut Db, rev: bool) -> CacheResult<Resp> {
    ...
    let (start_raw, end_raw) = if rev { (&args[3], &args[2]) } else { (&args[2], &args[3]) };
    ...
    let start = if start_raw == b"-" { StreamId::zero() }
        else { StreamId::parse(start_raw).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))? };
    let end = if end_raw == b"+" { StreamId { ms: u64::MAX, seq: u64::MAX } }
        else { StreamId::parse(end_raw).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))? };

    match db.get_typed(&args[1], "stream")? {
        Some(e) => match &e.value {
            Value::Stream(s) => {
                let take = count.unwrap_or(usize::MAX);
                let mut entries: Vec<&StreamEntry> = s.entries.iter()
                    .filter(|e| e.id >= start && e.id <= end)
                    .collect();
                if rev { entries.reverse(); }
                let result: Vec<Resp> = entries.into_iter().take(take).map(stream_entry_to_resp).collect();
...
pub fn cmd_xlen(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() != 2 { return Err(CacheError::wrong_arity("xlen")); }
    match db.get_typed(&args[1], "stream")? {
        Some(e) => match &e.value {
...
pub fn cmd_xtrim(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xtrim")); }
    let key = &args[1];
    let strategy = &args[2];
    let mut i = 3;
    let approx = if i < args.len() && args[i] == b"~" { i += 1; true } else { false };
        ...
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                ...
                match strategy.to_ascii_uppercase().as_slice() {
                    ...
                    b"MINID" => {
                        let min_id = StreamId::parse(&args[i]).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))?;
...
pub fn cmd_xdel(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 3 { return Err(CacheError::wrong_arity("xdel")); }
    let key = &args[1];
    let ids: Vec<StreamId> = args[2..].iter()
...
pub fn cmd_xinfo(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 3 { return Err(CacheError::wrong_arity("xinfo")); }
    match args[2].to_ascii_uppercase().as_slice() {
        b"STREAM" => {
            if args.len() < 4 { return Err(CacheError::wrong_arity("xinfo stream")); }
            match db.get_typed(&args[3], "stream")? {
                Some(e) => match &e.value {
                    Value::Stream(s) => {
                        let first_id = s.entries.first().map(|e| e.id.to_string()).unwrap_or_default();
                        let last_id = s.entries.last().map(|e| e.id.to_string()).unwrap_or_default();
        ...
        b"GROUPS" => {
            if args.len() < 4 { return Err(CacheError::wrong_arity("xinfo groups")); }
            match db.get_typed(&args[3], "stream")? {
                Some(e) => match &e.value {
                    Value::Stream(s) => {
                        let groups: Vec<Resp> = s.groups.values().map(|g| {
                            Resp::Array(Some(vec![
                                ...
                                Resp::BulkString(Some(g.last_delivered_id.to_string().into_bytes())),
                            ]))
                        }).collect();
        ...
        b"CONSUMERS" => {
            if args.len() < 5 { return Err(CacheError::wrong_arity("xinfo consumers")); }
            match db.get_typed(&args[3], "stream")? {
                Some(e) => match &e.value {
                    Value::Stream(s) => {
                        let group = s.groups.get(args[4].as_slice())
                            .ok_or(CacheError::NoGroup)?;
                        let consumers: Vec<Resp> = group.consumers.values().map(|c| {
                            Resp::Array(Some(vec![
                                Resp::BulkString(Some(b"name".to_vec())),
...
pub fn cmd_xgroup(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 3 { return Err(CacheError::wrong_arity("xgroup")); }
    match args[2].to_ascii_uppercase().as_slice() {
        ...
        _ => Err(CacheError::generic(format!("ERR Unknown subcommand: {:?}", std::str::from_utf8(&args[2])))),
    }
}

fn xgroup_create(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xgroup create")); }
    let key = &args[1];
    ...
    if !db.exists(key) {
        if mkstream {
            ...
            return Err(CacheError::generic("ERR The XGROUP subcommand requires the key to exist"));
        }
    }
        ...
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                if s.groups.contains_key(group_name.as_slice()) {
                    return Err(CacheError::generic("BUSYGROUP Consumer Group name already exists"));
                }
                s.groups.insert(group_name.clone(), ConsumerGroup {
                    name: group_name,
...
fn xgroup_destroy(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 3 { return Err(CacheError::wrong_arity("xgroup destroy")); }
...
fn xgroup_createconsumer(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xgroup createconsumer")); }
    let ms = now_ms();
    match db.get_typed_mut(&args[1], "stream")? {
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                let group = s.groups.get_mut(args[2].as_slice()).ok_or(CacheError::NoGroup)?;
                if group.consumers.contains_key(args[3].as_slice()) {
                    return Ok(Resp::Integer(0));
                }
                group.consumers.insert(args[3].clone(), Consumer {
                    name: args[3].clone(),
                    seen_time: ms,
                    active_time: ms,
                    pel: vec![],
...
fn xgroup_delconsumer(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xgroup delconsumer")); }
    match db.get_typed_mut(&args[1], "stream")? {
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                let group = s.groups.get_mut(args[2].as_slice()).ok_or(CacheError::NoGroup)?;
...
fn xgroup_setid(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xgroup setid")); }
...
pub fn cmd_xreadgroup(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 7 { return Err(CacheError::wrong_arity("xreadgroup")); }
    ...
    for (key, id_raw) in keys.iter().zip(ids.iter()) {
        ...
        match db.get_typed_mut(key, "stream")? {
            Some(entry) => match &mut entry.value {
                Value::Stream(s) => {
                    let group_data = s.groups.get_mut(group.as_slice()).ok_or(CacheError::NoGroup)?;

                    let consumer = group_data.consumers.entry(consumer_name.clone())
                        .or_insert(Consumer {
                    ...
                    if pending_only {
                        ...
                        if !noack {
                            let cons = group_data.consumers.get_mut(consumer_name.as_slice()).unwrap();
                            for e in &delivered {
                                cons.pel.push(e.id);
                                group_data.pel.insert(e.id, PendingEntry {
                                    id: e.id,
                        ...
                        let from = StreamId::parse(id_raw).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))?;
                        let cons = group_data.consumers.get(consumer_name.as_slice());
                        if let Some(cons) = cons {
                            let pending_ids: Vec<StreamId> = cons.pel.iter()
                                .filter(|&&id| id >= from)
                                .take(count)
                                .copied()
                                .collect();
                    ...
                    let resp_entries: Vec<Resp> = delivered.iter().map(stream_entry_to_resp).collect();
...
pub fn cmd_xack(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 4 { return Err(CacheError::wrong_arity("xack")); }
    let key = &args[1];
    let group = &args[2];
    let ids: Vec<StreamId> = args[3..].iter()
        .map(|id| StreamId::parse(id).ok_or_else(|| CacheError::generic("ERR Invalid stream ID")))
        .collect::<CacheResult<_>>()?;
...
pub fn cmd_xpending(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 3 { return Err(CacheError::wrong_arity("xpending")); }
    let key = &args[1];
    let group = &args[2];
        ...
        Some(e) => match &e.value {
            Value::Stream(s) => {
                ...
                if args.len() == 3 {
                    ...
                    let min = group_data.pel.keys().min().map(|id| id.to_string()).unwrap_or_default();
                    let max = group_data.pel.keys().max().map(|id| id.to_string()).unwrap_or_default();
                    ...
                    let consumers: Vec<Resp> = consumer_counts.iter()
                        .map(|(name, cnt)| Resp::Array(Some(vec![
                    ...
                    if i < args.len() && args[i].to_ascii_uppercase() == b"IDLE" {
                        idle_filter = Some(bytes_to_i64(&args[i + 1]).ok_or(CacheError::NotInteger)? as u64);
                        i += 2;
                    }
                    let start = StreamId::parse(&args[i]).ok_or_else(|| CacheError::generic("ERR"))?;
                    let end = StreamId::parse(&args[i + 1]).ok_or_else(|| CacheError::generic("ERR"))?;
                    let count = bytes_to_i64(&args[i + 2]).ok_or(CacheError::NotInteger)? as usize;
                    let consumer_filter = args.get(i + 3);

                    let now = now_ms();
                    let entries: Vec<Resp> = group_data.pel.values()
                        .filter(|pe| pe.id >= start && pe.id <= end)
                        .filter(|pe| consumer_filter.map(|c| c.as_slice() == pe.consumer.as_slice()).unwrap_or(true))
                        .filter(|pe| idle_filter.map(|idle| now - pe.delivery_time >= idle).unwrap_or(true))
                        .take(count)
                        .map(|pe| {
...
pub fn cmd_xclaim(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 6 { return Err(CacheError::wrong_arity("xclaim")); }
    let key = &args[1];
    let group = &args[2];
    let consumer_name = args[3].clone();
    let min_idle = bytes_to_i64(&args[4]).ok_or(CacheError::NotInteger)? as u64;
    let ids: Vec<StreamId> = args[5..].iter()
        ...
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                ...
                let result: Vec<Resp> = if justid {
                    claimed.iter().map(|id| Resp::BulkString(Some(id.to_string().into_bytes()))).collect()
                } else {
                    claimed.iter()
...
pub fn cmd_xautoclaim(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() < 7 { return Err(CacheError::wrong_arity("xautoclaim")); }
    let key = &args[1];
    let group = &args[2];
    let consumer_name = args[3].clone();
    let min_idle = bytes_to_i64(&args[4]).ok_or(CacheError::NotInteger)? as u64;
    let start = StreamId::parse(&args[5]).ok_or_else(|| CacheError::generic("ERR Invalid stream ID"))?;
    ...
    match db.get_typed_mut(key, "stream")? {
        Some(entry) => match &mut entry.value {
            Value::Stream(s) => {
                ...
                for pe in group_data.pel.values_mut()
                ...
                let next_start = claimed_ids.last().map(|id| {
                    StreamId { ms: id.ms, seq: id.seq + 1 }
                }).unwrap_or(StreamId::zero());

                let result: Vec<Resp> = if justid {
                    claimed_ids.iter().map(|id| Resp::BulkString(Some(id.to_string().into_bytes()))).collect()
                } else {
                    claimed_ids.iter()
                        .filter_map(|id| s.entries.iter().find(|e| &e.id == id))
                        .map(stream_entry_to_resp)
