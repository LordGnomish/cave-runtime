...
fn expire_impl(args: &[Vec<u8>], db: &mut Db, ms: bool, abs: bool) -> CacheResult<Resp> {
    ...
    if let Some(entry) = db.get_mut(key) {
        ...
        if nx && current_expiry.is_some() { return Ok(Resp::Integer(0)); }
        // XX: only set if has expiry
        if xx && current_expiry.is_none() { return Ok(Resp::Integer(0)); }
        // GT: only set if new expiry > current
        if gt && current_expiry.map(|t| new_expiry <= t).unwrap_or(false) { return Ok(Resp::Integer(0)); }
        // LT: only set if new expiry < current
        if lt && current_expiry.map(|t| new_expiry >= t).unwrap_or(false) { return Ok(Resp::Integer(0)); }
...
fn ttl_impl(args: &[Vec<u8>], db: &mut Db, ms: bool) -> CacheResult<Resp> {
    ...
    if args.len() != 2 { return Err(CacheError::wrong_arity(cmd)); }
...
pub fn cmd_persist(args: &[Vec<u8>], db: &mut Db) -> CacheResult<Resp> {
    if args.len() != 2 { return Err(CacheError::wrong_arity("persist")); }
...
fn expiretime_impl(args: &[Vec<u8>], db: &mut Db, ms: bool) -> CacheResult<Resp> {
    ...
    match db.get(&args[1]) {
        ...
        Some(e) => match e.expires_at {
            ...
            Some(t) => {
                let remaining = t.checked_duration_since(Instant::now()).unwrap_or(Duration::ZERO);
                let unix_now = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default();
