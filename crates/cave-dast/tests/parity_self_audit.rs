...
//! Charter v2 self-audit — cave-dast's own `parity.manifest.toml` must
//! carry an honest measured `fill_ratio` against zaproxy/zaproxy
//! v2.14.0, a pinned `source_sha` for reproducibility, the 2026-05-18
//! FINALIZE audit date, and structurally consistent counts.
//!
//! Parses the manifest as plain text (no new dependency) and asserts
//! every Charter v2 close-out gate explicitly so any drift surfaces as
//! a localised test failure.
...
#[test]
fn upstream_version_is_pinned_v2_14_0() {
    let m = manifest_text();
    ...
    assert_eq!(
        v.as_deref(),
        Some("v2.14.0"),
        "manifest [upstream] version must pin zaproxy/zaproxy v2.14.0 (was {:?})",
        v
    );
}
...
fn upstream_source_sha_is_present_and_matches_version() {
    ...
    assert!(
        ...
        "manifest [upstream] source_sha must be set for reproducibility \
         (got {:?}). Charter v2 close-out: pin to the v2.14.0 tag tree-ish.",
        sha
    );
    assert_eq!(
        sha.as_deref(),
...
fn parity_fill_ratio_is_measured_and_at_least_floor() {
    ...
    // cave-dast Charter v2 floor: 0.65 — port covers active+passive scan
...
fn parity_ratio_source_is_manifest() {
    ...
    assert_eq!(
        src.as_deref(),
        Some("manifest"),
        "[parity] parity_ratio_source must be \"manifest\" so parity-index reads \
         fill_ratio directly from this file (got {:?})",
        src
    );
}
...
fn parity_infra_only_is_false() {
    ...
    assert_eq!(
        v.as_deref(),
        Some("false"),
        "cave-dast IS a parity surface vs OWASP ZAP — infra_only must be false"
    );
}

#[test]
fn at_least_ten_mapped_file_blocks() {
    let m = manifest_text();
    // Each [[files]] block with `status = "ported"` is a real port.
    let blocks = m.matches("\n[[files]]").count();
    assert!(
        blocks >= 10,
        "expected >= 10 [[files]] blocks (ZAP module ports); got {}",
        blocks
    );
}
...
fn every_rs_file_carries_agpl_spdx() {
    ...
    assert!(
        total >= 15,
        "expected >= 15 .rs files in cave-dast post-port; got {}",
        total
    );
}
