...
pub fn cis_kubernetes_framework() -> ComplianceFramework {
    ...
    let controls = vec![
        make_control(
            fw_id,
            "CIS-1.1.1",
            "Ensure API server config file permissions",
            "Master Node Security Configuration",
            ControlSeverity::Critical,
            true,
            "check_apiserver_file_perms",
            "Set permissions to 600 on kube-apiserver.yaml",
        ),
        make_control(
            fw_id,
            "CIS-1.2.1",
        ...
        make_control(
            fw_id,
            "CIS-1.2.2",
            "Ensure basic auth is not used",
            "API Server",
            ControlSeverity::High,
            true,
            "check_basic_auth",
            "Remove --basic-auth-file from kube-apiserver",
        ),
        make_control(
            fw_id,
            "CIS-1.2.6",
            "Ensure AlwaysAdmit admission plugin is not set",
            "API Server",
            ControlSeverity::High,
            true,
            "check_always_admit",
        ...
        make_control(
            fw_id,
            "CIS-1.2.9",
            "Ensure EventRateLimit admission plugin is set",
            "API Server",
            ControlSeverity::Medium,
            true,
            "check_event_rate_limit",
            "Add EventRateLimit to --enable-admission-plugins",
        ),
        make_control(
            fw_id,
            "CIS-2.1",
            "Ensure etcd is configured with TLS",
            "etcd",
            ControlSeverity::Critical,
            true,
            "check_etcd_tls",
            "Configure --cert-file and --key-file for etcd",
        ),
        make_control(
            fw_id,
            "CIS-3.1.1",
            "Ensure client cert auth not used for users",
            "Authentication and Authorization",
            ControlSeverity::High,
            false,
            "",
            "Use OIDC or similar for user authentication",
        ),
        make_control(
            fw_id,
            "CIS-4.1.1",
            "Ensure worker service account permissions",
            "Worker Node Configuration",
            ControlSeverity::High,
            true,
            "check_worker_sa",
            "Ensure service accounts are not over-privileged",
        ),
        make_control(
            fw_id,
            "CIS-5.1.1",
            ...
            true,
            "check_rbac_enabled",
            "Set --authorization-mode=RBAC on kube-apiserver",
        ),
        make_control(
            fw_id,
            "CIS-5.2.1",
            ...
            "Use Pod Security Admission to restrict privileged pods",
        ),
        make_control(
            fw_id,
            "CIS-5.2.2",
            "Minimize hostPID containers",
            "Pod Security Standards",
            ControlSeverity::High,
            true,
            "check_host_pid",
            "Restrict hostPID via admission policy",
        ),
        make_control(
            fw_id,
            "CIS-5.3.1",
        ...
        make_control(
            fw_id,
            "CIS-5.4.1",
            "Prefer Secrets as files over env vars",
            "Secrets Management",
            ControlSeverity::Medium,
            false,
            "",
        ...
        make_control(
            fw_id,
            "CIS-5.7.1",
            "Create administrative boundaries with namespaces",
            "General Policies",
            ControlSeverity::Low,
            false,
            "",
            "Use namespaces to segment workloads by team/environment",
        ),
    ];
    ComplianceFramework {
        id: fw_id,
...
pub fn soc2_framework() -> ComplianceFramework {
    ...
    let controls = vec![
        make_control(
            ...
            "Implement RBAC and MFA for all privileged access",
        ),
        make_control(
            fw_id,
            "SOC2-CC6.2",
            "Prior to issuing access credentials",
            "Common Criteria",
            ControlSeverity::High,
            false,
            "",
        ...
        make_control(
            ...
            true,
            "check_vuln_scanning",
            "Run regular vulnerability scans on all containers",
        ),
        make_control(
            fw_id,
            "SOC2-CC8.1",
            ...
            false,
            "",
            "Document all infrastructure changes with approval",
        ),
        make_control(
            fw_id,
            "SOC2-A1.2",
            ...
            true,
            "check_monitoring",
            "Monitor system availability and set SLOs",
        ),
    ];
    ComplianceFramework {
        id: fw_id,
...
pub fn pci_dss_framework() -> ComplianceFramework {
    ...
    let controls = vec![
        ...
        make_control(
            ...
            false,
            "",
            "Implement need-to-know access policy for cardholder data",
        ),
        make_control(
            fw_id,
            "PCI-8.2",
        ...
        make_control(
            ...
            true,
            "check_audit_logging",
            "Enable audit logging for all access to cardholder data",
        ),
        make_control(
            fw_id,
            "PCI-11.3",
...
pub fn hipaa_framework() -> ComplianceFramework {
    ...
    let controls = vec![
        ...
        make_control(
            ...
            true,
            "check_hipaa_audit",
            "Implement audit controls for ePHI access",
        ),
        make_control(
            fw_id,
            "HIPAA-164.312(e)(1)",
            ...
            "Encrypt all ePHI in transit using TLS 1.2+",
        ),
        make_control(
            fw_id,
...
fn make_control(
    ...
    Control {
        ...
        severity,
        automated,
        check_fn: if check_fn.is_empty() {
            None
        } else {
            Some(check_fn.to_string())
        },
    ...
    fn test_all_controls_have_remediation() {
        for fw in builtin_frameworks() {
            for ctrl in &fw.controls {
                assert!(
                    !ctrl.remediation.is_empty(),
                    "Control {} has no remediation",
                    ctrl.control_id
                );
            }
        }
