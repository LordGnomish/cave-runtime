...
//
//! Charter v2 self-audit — cave-iceberg must carry an honest, measured
//! `fill_ratio` against upstream apache/iceberg-rust at the latest
//! stable release (v0.9.1), a pinned `source_sha` for reproducibility,
...
fn assertion_3_fill_ratio_meets_floor() {
    ...
    assert!(
        ...
        "cave-iceberg Charter v2 floor: fill_ratio must be >= {} (got {})",
        FLOOR_FILL_RATIO,
        ratio
    );
    assert!(
...
fn assertion_6_agpl_spdx_header_coverage() {
    ...
    assert!(
        missing.is_empty(),
        "{} of {} .rs files in cave-iceberg missing AGPL SPDX header: {:?}",
        missing.len(),
        total,
        missing
    );
...
fn assertion_9_parity_index_json_consistency() {
    ...
    let i = idx
        .find("\"cave-iceberg\":")
        .expect("docs/parity/parity-index.json must contain cave-iceberg entry");
    let block_end = idx[i..]
        .find("\n    }")
        .expect("cave-iceberg block must close with `\\n    }`");
    let block = &idx[i..i + block_end];

    let pr_line = block
        .lines()
        .find(|l| l.contains("\"parity_ratio\":"))
        .expect("cave-iceberg block must contain parity_ratio");
    ...
    assert!(
        ...
        "parity-index.json cave-iceberg entry must declare parity_ratio_source = \"manifest\""
    );
}
