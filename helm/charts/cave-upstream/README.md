...
Helm chart for **cave-upstream** — port of Upstream tracker portal API.
...
```bash
helm install my-upstream ./helm/charts/cave-upstream \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-upstream` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-upstream ./helm/charts/cave-upstream \
...
```bash
helm test my-upstream --namespace cave-system
```
