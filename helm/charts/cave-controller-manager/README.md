...
Helm chart for **cave-controller-manager** — port of K8s controller manager.

Workload kind: `deployment`. Default service port: `10257`.

## Install

```bash
helm install my-controller-manager ./helm/charts/cave-controller-manager \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-controller-manager` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10257` | upstream-aligned default |
...
```bash
helm upgrade my-controller-manager ./helm/charts/cave-controller-manager \
...
```bash
helm test my-controller-manager --namespace cave-system
```
