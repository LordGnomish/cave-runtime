...
Helm chart for **cave-net** — port of Hubble + Cilium CNI.

Workload kind: `daemonset`. Default service port: `4244`.

## Install

```bash
helm install my-net ./helm/charts/cave-net \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-net` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `4244` | upstream-aligned default |
...
```bash
helm upgrade my-net ./helm/charts/cave-net \
...
```bash
helm test my-net --namespace cave-system
```
