...
Helm chart for **cave-oncall** — port of Oncall scheduler.
...
```bash
helm install my-oncall ./helm/charts/cave-oncall \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-oncall` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-oncall ./helm/charts/cave-oncall \
...
```bash
helm test my-oncall --namespace cave-system
```
