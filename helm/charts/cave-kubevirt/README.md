...
Helm chart for **cave-kubevirt** — port of KubeVirt VM operator.
...
```bash
helm install my-kubevirt ./helm/charts/cave-kubevirt \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-kubevirt` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-kubevirt ./helm/charts/cave-kubevirt \
...
```bash
helm test my-kubevirt --namespace cave-system
```
