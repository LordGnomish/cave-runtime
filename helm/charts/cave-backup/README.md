...
Helm chart for **cave-backup** — port of Velero backup controller.

Workload kind: `deployment`. Default service port: `8085`.

## Install

```bash
helm install my-backup ./helm/charts/cave-backup \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-backup` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8085` | upstream-aligned default |
...
```bash
helm upgrade my-backup ./helm/charts/cave-backup \
...
```bash
helm test my-backup --namespace cave-system
```
