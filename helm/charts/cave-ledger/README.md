...
Helm chart for **cave-ledger** — port of Append-only ledger service.

Workload kind: `statefulset`. Default service port: `8080`.

## Install

```bash
helm install my-ledger ./helm/charts/cave-ledger \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-ledger` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-ledger ./helm/charts/cave-ledger \
...
```bash
helm test my-ledger --namespace cave-system
```
