...
Helm chart for **cave-vulns** — port of DefectDojo v2.58.2.
...
```bash
helm install my-vulns ./helm/charts/cave-vulns \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-vulns` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-vulns ./helm/charts/cave-vulns \
...
```bash
helm test my-vulns --namespace cave-system
```
