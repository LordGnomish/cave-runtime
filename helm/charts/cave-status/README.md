...
Helm chart for **cave-status** — port of Statuspage service.
...
```bash
helm install my-status ./helm/charts/cave-status \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-status` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-status ./helm/charts/cave-status \
...
```bash
helm test my-status --namespace cave-system
```
