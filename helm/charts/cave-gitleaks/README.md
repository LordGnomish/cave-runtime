...
Helm chart for **cave-gitleaks** — port of Gitleaks secret scanner.
...
```bash
helm install my-gitleaks ./helm/charts/cave-gitleaks \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-gitleaks` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-gitleaks ./helm/charts/cave-gitleaks \
...
```bash
helm test my-gitleaks --namespace cave-system
```
