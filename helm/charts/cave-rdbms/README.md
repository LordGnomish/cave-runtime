...
Helm chart for **cave-rdbms** — port of PostgreSQL 16.
...
```bash
helm install my-rdbms ./helm/charts/cave-rdbms \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-rdbms` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-rdbms ./helm/charts/cave-rdbms \
...
```bash
helm test my-rdbms --namespace cave-system
```
