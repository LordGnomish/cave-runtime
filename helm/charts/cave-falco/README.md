...
Helm chart for **cave-falco** — port of Falco runtime detection.

Workload kind: `daemonset`. Default service port: `8765`.

## Install

```bash
helm install my-falco ./helm/charts/cave-falco \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-falco` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8765` | upstream-aligned default |
...
```bash
helm upgrade my-falco ./helm/charts/cave-falco \
...
```bash
helm test my-falco --namespace cave-system
```
