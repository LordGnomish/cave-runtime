...
Helm chart for **cave-portal-web** — port of Cave portal web UI.

Workload kind: `deployment`. Default service port: `3000`.

## Install

```bash
helm install my-portal-web ./helm/charts/cave-portal-web \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-portal-web` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `3000` | upstream-aligned default |
...
```bash
helm upgrade my-portal-web ./helm/charts/cave-portal-web \
...
```bash
helm test my-portal-web --namespace cave-system
```
