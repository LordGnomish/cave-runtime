...
Helm chart for **cave-portal-api** — port of Cave portal API.
...
```bash
helm install my-portal-api ./helm/charts/cave-portal-api \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-portal-api` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-portal-api ./helm/charts/cave-portal-api \
...
```bash
helm test my-portal-api --namespace cave-system
```
