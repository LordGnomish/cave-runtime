...
Helm chart for **cave-local-llm** — port of Local Ollama LLM server.

Workload kind: `deployment`. Default service port: `11434`.

## Install

```bash
helm install my-local-llm ./helm/charts/cave-local-llm \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-local-llm` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `11434` | upstream-aligned default |
...
```bash
helm upgrade my-local-llm ./helm/charts/cave-local-llm \
...
```bash
helm test my-local-llm --namespace cave-system
```
