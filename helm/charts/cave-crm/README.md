...
Helm chart for **cave-crm** — port of CRM integration service.
...
```bash
helm install my-crm ./helm/charts/cave-crm \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-crm` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-crm ./helm/charts/cave-crm \
...
```bash
helm test my-crm --namespace cave-system
```
