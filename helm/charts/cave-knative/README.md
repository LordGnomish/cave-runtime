...
Helm chart for **cave-knative** — port of Knative Serving + Argo Events.
...
```bash
helm install my-knative ./helm/charts/cave-knative \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-knative` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-knative ./helm/charts/cave-knative \
...
```bash
helm test my-knative --namespace cave-system
```
