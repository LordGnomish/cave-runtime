...
Helm chart for **cave-gateway** — port of Kong 3.9.1 + Gravitee 4.11.7.
...
```bash
helm install my-gateway ./helm/charts/cave-gateway \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-gateway` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-gateway ./helm/charts/cave-gateway \
...
```bash
helm test my-gateway --namespace cave-system
```
