...
Helm chart for **cave-runbook** — port of Runbook execution service.
...
```bash
helm install my-runbook ./helm/charts/cave-runbook \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-runbook` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-runbook ./helm/charts/cave-runbook \
...
```bash
helm test my-runbook --namespace cave-system
```
