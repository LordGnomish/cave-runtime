...
Helm chart for **cave-rollouts** — port of Argo Rollouts v1.9.0.

Workload kind: `deployment`. Default service port: `8090`.

## Install

```bash
helm install my-rollouts ./helm/charts/cave-rollouts \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-rollouts` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8090` | upstream-aligned default |
...
```bash
helm upgrade my-rollouts ./helm/charts/cave-rollouts \
...
```bash
helm test my-rollouts --namespace cave-system
```
