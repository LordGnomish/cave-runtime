...
Helm chart for **cave-scheduler** — port of K8s scheduler.

Workload kind: `deployment`. Default service port: `10259`.

## Install

```bash
helm install my-scheduler ./helm/charts/cave-scheduler \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-scheduler` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10259` | upstream-aligned default |
...
```bash
helm upgrade my-scheduler ./helm/charts/cave-scheduler \
...
```bash
helm test my-scheduler --namespace cave-system
```
