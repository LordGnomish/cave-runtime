...
Helm chart for **cave-streams** — port of Apache Kafka 4.2.0 + Pulsar v4.2.0.

Workload kind: `statefulset`. Default service port: `9092`.

## Install

```bash
helm install my-streams ./helm/charts/cave-streams \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-streams` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9092` | upstream-aligned default |
...
```bash
helm upgrade my-streams ./helm/charts/cave-streams \
...
```bash
helm test my-streams --namespace cave-system
```
