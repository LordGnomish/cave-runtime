...
Helm chart for **cave-dast** — port of OWASP ZAP DAST scanner.
...
```bash
helm install my-dast ./helm/charts/cave-dast \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-dast` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-dast ./helm/charts/cave-dast \
...
```bash
helm test my-dast --namespace cave-system
```
