...
Helm chart for **cave-datafusion** — port of Apache DataFusion query engine.
...
```bash
helm install my-datafusion ./helm/charts/cave-datafusion \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-datafusion` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-datafusion ./helm/charts/cave-datafusion \
...
```bash
helm test my-datafusion --namespace cave-system
```
