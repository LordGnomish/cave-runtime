...
Helm chart for **cave-hermes** — port of Hermes LLM dispatcher.
...
```bash
helm install my-hermes ./helm/charts/cave-hermes \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-hermes` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-hermes ./helm/charts/cave-hermes \
...
```bash
helm test my-hermes --namespace cave-system
```
