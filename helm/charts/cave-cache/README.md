...
Helm chart for **cave-cache** — port of Redis/KeyDB cache.

Workload kind: `statefulset`. Default service port: `6379`.

## Install

```bash
helm install my-cache ./helm/charts/cave-cache \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cache` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `6379` | upstream-aligned default |
...
```bash
helm upgrade my-cache ./helm/charts/cave-cache \
...
```bash
helm test my-cache --namespace cave-system
```
