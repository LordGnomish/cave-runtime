...
Helm chart for **cave-sign** — port of Sigstore Cosign v3.0.6.
...
```bash
helm install my-sign ./helm/charts/cave-sign \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-sign` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-sign ./helm/charts/cave-sign \
...
```bash
helm test my-sign --namespace cave-system
```
