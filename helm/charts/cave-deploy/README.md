...
Helm chart for **cave-deploy** — port of ArgoCD v3.4.2.
...
```bash
helm install my-deploy ./helm/charts/cave-deploy \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-deploy` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-deploy ./helm/charts/cave-deploy \
...
```bash
helm test my-deploy --namespace cave-system
```
