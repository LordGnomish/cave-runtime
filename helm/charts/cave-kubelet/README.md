...
Helm chart for **cave-kubelet** — port of kubelet.

Workload kind: `daemonset`. Default service port: `10250`.

## Install

```bash
helm install my-kubelet ./helm/charts/cave-kubelet \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-kubelet` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10250` | upstream-aligned default |
...
```bash
helm upgrade my-kubelet ./helm/charts/cave-kubelet \
...
```bash
helm test my-kubelet --namespace cave-system
```
