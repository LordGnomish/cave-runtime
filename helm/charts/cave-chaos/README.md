...
Helm chart for **cave-chaos** — port of Chaos Mesh controller.

Workload kind: `deployment`. Default service port: `10080`.

## Install

```bash
helm install my-chaos ./helm/charts/cave-chaos \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-chaos` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10080` | upstream-aligned default |
...
```bash
helm upgrade my-chaos ./helm/charts/cave-chaos \
...
```bash
helm test my-chaos --namespace cave-system
```
