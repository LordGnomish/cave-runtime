...
Helm chart for **cave-incidents** — port of Incident response service.
...
```bash
helm install my-incidents ./helm/charts/cave-incidents \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-incidents` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-incidents ./helm/charts/cave-incidents \
...
```bash
helm test my-incidents --namespace cave-system
```
