...
Helm chart for **cave-cost** — port of Kubecost cost analyzer.
...
```bash
helm install my-cost ./helm/charts/cave-cost \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cost` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-cost ./helm/charts/cave-cost \
...
```bash
helm test my-cost --namespace cave-system
```
