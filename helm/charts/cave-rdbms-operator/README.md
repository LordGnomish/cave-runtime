...
Helm chart for **cave-rdbms-operator** — port of CloudNativePG operator.

Workload kind: `deployment`. Default service port: `9443`.

## Install

```bash
helm install my-rdbms-operator ./helm/charts/cave-rdbms-operator \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-rdbms-operator` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9443` | upstream-aligned default |
...
```bash
helm upgrade my-rdbms-operator ./helm/charts/cave-rdbms-operator \
...
```bash
helm test my-rdbms-operator --namespace cave-system
```
