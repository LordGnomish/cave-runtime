...
Helm chart for **cave-tracker** — port of Issue tracker integration.
...
```bash
helm install my-tracker ./helm/charts/cave-tracker \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-tracker` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-tracker ./helm/charts/cave-tracker \
...
```bash
helm test my-tracker --namespace cave-system
```
