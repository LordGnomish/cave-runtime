...
Helm chart for **cave-upstream-watchd** — port of Upstream watcher daemon.
...
```bash
helm install my-upstream-watchd ./helm/charts/cave-upstream-watchd \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-upstream-watchd` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-upstream-watchd ./helm/charts/cave-upstream-watchd \
...
```bash
helm test my-upstream-watchd --namespace cave-system
```
