...
Helm chart for **cave-cost-alloc** — port of OpenCost allocation.

Workload kind: `deployment`. Default service port: `9003`.

## Install

```bash
helm install my-cost-alloc ./helm/charts/cave-cost-alloc \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cost-alloc` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9003` | upstream-aligned default |
...
```bash
helm upgrade my-cost-alloc ./helm/charts/cave-cost-alloc \
...
```bash
helm test my-cost-alloc --namespace cave-system
```
