...
Helm chart for **cave-acme** — port of cert-manager.

Workload kind: `deployment`. Default service port: `9402`.

## Install

```bash
helm install my-acme ./helm/charts/cave-acme \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-acme` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9402` | upstream-aligned default |
...
```bash
helm upgrade my-acme ./helm/charts/cave-acme \
...
```bash
helm test my-acme --namespace cave-system
```
