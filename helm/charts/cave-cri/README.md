...
Helm chart for **cave-cri** — port of Container Runtime Interface.

Workload kind: `daemonset`. Default service port: `10010`.

## Install

```bash
helm install my-cri ./helm/charts/cave-cri \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cri` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10010` | upstream-aligned default |
...
```bash
helm upgrade my-cri ./helm/charts/cave-cri \
...
```bash
helm test my-cri --namespace cave-system
```
