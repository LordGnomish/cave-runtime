...
Helm chart for **cave-search** — port of OpenSearch cluster.

Workload kind: `statefulset`. Default service port: `9200`.

## Install

```bash
helm install my-search ./helm/charts/cave-search \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-search` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9200` | upstream-aligned default |
...
```bash
helm upgrade my-search ./helm/charts/cave-search \
...
```bash
helm test my-search --namespace cave-system
```
