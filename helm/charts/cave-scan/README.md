...
Helm chart for **cave-scan** — port of Multi-engine vulnerability scan.
...
```bash
helm install my-scan ./helm/charts/cave-scan \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-scan` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-scan ./helm/charts/cave-scan \
...
```bash
helm test my-scan --namespace cave-system
```
