...
Helm chart for **cave-apiserver** — port of Kubernetes API server.

Workload kind: `deployment`. Default service port: `6443`.

## Install

```bash
helm install my-apiserver ./helm/charts/cave-apiserver \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-apiserver` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `6443` | upstream-aligned default |
...
```bash
helm upgrade my-apiserver ./helm/charts/cave-apiserver \
...
```bash
helm test my-apiserver --namespace cave-system
```
