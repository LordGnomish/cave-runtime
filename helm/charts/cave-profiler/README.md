...
Helm chart for **cave-profiler** — port of Pyroscope continuous profiling.

Workload kind: `deployment`. Default service port: `4040`.

## Install

```bash
helm install my-profiler ./helm/charts/cave-profiler \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-profiler` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `4040` | upstream-aligned default |
...
```bash
helm upgrade my-profiler ./helm/charts/cave-profiler \
...
```bash
helm test my-profiler --namespace cave-system
```
