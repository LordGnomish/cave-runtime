...
Helm chart for **cave-mesh** — port of Istio/Linkerd mesh control plane.

Workload kind: `deployment`. Default service port: `15010`.

## Install

```bash
helm install my-mesh ./helm/charts/cave-mesh \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-mesh` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `15010` | upstream-aligned default |
...
```bash
helm upgrade my-mesh ./helm/charts/cave-mesh \
...
```bash
helm test my-mesh --namespace cave-system
```
