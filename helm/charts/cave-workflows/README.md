...
Helm chart for **cave-workflows** — port of Argo Workflows v4.0.5.

Workload kind: `deployment`. Default service port: `2746`.

## Install

```bash
helm install my-workflows ./helm/charts/cave-workflows \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-workflows` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `2746` | upstream-aligned default |
...
```bash
helm upgrade my-workflows ./helm/charts/cave-workflows \
...
```bash
helm test my-workflows --namespace cave-system
```
