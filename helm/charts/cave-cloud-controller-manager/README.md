...
Helm chart for **cave-cloud-controller-manager** — port of K8s cloud controller manager.

Workload kind: `deployment`. Default service port: `10258`.

## Install

```bash
helm install my-cloud-controller-manager ./helm/charts/cave-cloud-controller-manager \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cloud-controller-manager` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `10258` | upstream-aligned default |
...
```bash
helm upgrade my-cloud-controller-manager ./helm/charts/cave-cloud-controller-manager \
...
```bash
helm test my-cloud-controller-manager --namespace cave-system
```
