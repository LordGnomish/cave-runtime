...
Helm chart for **cave-dns** — port of CoreDNS v1.14.3.

Workload kind: `deployment`. Default service port: `53`.

## Install

```bash
helm install my-dns ./helm/charts/cave-dns \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-dns` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `53` | upstream-aligned default |
...
```bash
helm upgrade my-dns ./helm/charts/cave-dns \
...
```bash
helm test my-dns --namespace cave-system
```
