...
Helm chart for **cave-uptime** — port of Uptime monitor.
...
```bash
helm install my-uptime ./helm/charts/cave-uptime \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-uptime` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-uptime ./helm/charts/cave-uptime \
...
```bash
helm test my-uptime --namespace cave-system
```
