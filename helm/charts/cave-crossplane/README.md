...
Helm chart for **cave-crossplane** — port of Crossplane v2.3.1.
...
```bash
helm install my-crossplane ./helm/charts/cave-crossplane \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-crossplane` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-crossplane ./helm/charts/cave-crossplane \
...
```bash
helm test my-crossplane --namespace cave-system
```
