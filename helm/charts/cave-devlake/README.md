...
Helm chart for **cave-devlake** — port of Apache DevLake.
...
```bash
helm install my-devlake ./helm/charts/cave-devlake \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-devlake` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-devlake ./helm/charts/cave-devlake \
...
```bash
helm test my-devlake --namespace cave-system
```
