...
Helm chart for **cave-metrics** — port of Prometheus metrics.

Workload kind: `statefulset`. Default service port: `9090`.

## Install

```bash
helm install my-metrics ./helm/charts/cave-metrics \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-metrics` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-metrics ./helm/charts/cave-metrics \
...
```bash
helm test my-metrics --namespace cave-system
```
