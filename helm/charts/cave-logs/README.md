...
Helm chart for **cave-logs** — port of Loki/ES log aggregation.

Workload kind: `statefulset`. Default service port: `3100`.

## Install

```bash
helm install my-logs ./helm/charts/cave-logs \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-logs` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `3100` | upstream-aligned default |
...
```bash
helm upgrade my-logs ./helm/charts/cave-logs \
...
```bash
helm test my-logs --namespace cave-system
```
