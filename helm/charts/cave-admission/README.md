...
Helm chart for **cave-admission** — port of K8s admission webhook.
...
```bash
helm install my-admission ./helm/charts/cave-admission \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-admission` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-admission ./helm/charts/cave-admission \
...
```bash
helm test my-admission --namespace cave-system
```
