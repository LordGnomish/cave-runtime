...
Helm chart for **cave-etcd** — port of etcd v3.5.

Workload kind: `statefulset`. Default service port: `2379`.

## Install

```bash
helm install my-etcd ./helm/charts/cave-etcd \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-etcd` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `2379` | upstream-aligned default |
...
```bash
helm upgrade my-etcd ./helm/charts/cave-etcd \
...
```bash
helm test my-etcd --namespace cave-system
```
