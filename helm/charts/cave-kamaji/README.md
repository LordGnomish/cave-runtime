...
Helm chart for **cave-kamaji** — port of Kamaji multi-tenant control plane.
...
```bash
helm install my-kamaji ./helm/charts/cave-kamaji \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-kamaji` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-kamaji ./helm/charts/cave-kamaji \
...
```bash
helm test my-kamaji --namespace cave-system
```
