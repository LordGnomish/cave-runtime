...
Helm chart for **cave-cdc** — port of Debezium CDC connector.

Workload kind: `deployment`. Default service port: `8083`.

## Install

```bash
helm install my-cdc ./helm/charts/cave-cdc \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-cdc` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8083` | upstream-aligned default |
...
```bash
helm upgrade my-cdc ./helm/charts/cave-cdc \
...
```bash
helm test my-cdc --namespace cave-system
```
