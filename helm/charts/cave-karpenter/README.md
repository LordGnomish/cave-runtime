...
Helm chart for **cave-karpenter** — port of Karpenter cluster autoscaler.
...
```bash
helm install my-karpenter ./helm/charts/cave-karpenter \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-karpenter` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-karpenter ./helm/charts/cave-karpenter \
...
```bash
helm test my-karpenter --namespace cave-system
```
