...
Helm chart for **cave-trace** — port of Jaeger v1.52.0.

Workload kind: `deployment`. Default service port: `16686`.

## Install

```bash
helm install my-trace ./helm/charts/cave-trace \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-trace` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `16686` | upstream-aligned default |
...
```bash
helm upgrade my-trace ./helm/charts/cave-trace \
...
```bash
helm test my-trace --namespace cave-system
```
