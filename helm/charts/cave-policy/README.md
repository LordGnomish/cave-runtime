...
Helm chart for **cave-policy** — port of OPA + Gatekeeper + Kyverno.
...
```bash
helm install my-policy ./helm/charts/cave-policy \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-policy` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-policy ./helm/charts/cave-policy \
...
```bash
helm test my-policy --namespace cave-system
```
