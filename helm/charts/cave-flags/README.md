...
Helm chart for **cave-flags** — port of OpenFeature feature flags.
...
```bash
helm install my-flags ./helm/charts/cave-flags \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-flags` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-flags ./helm/charts/cave-flags \
...
```bash
helm test my-flags --namespace cave-system
```
