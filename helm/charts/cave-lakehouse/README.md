...
Helm chart for **cave-lakehouse** — port of Lakehouse query coordinator.
...
```bash
helm install my-lakehouse ./helm/charts/cave-lakehouse \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-lakehouse` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-lakehouse ./helm/charts/cave-lakehouse \
...
```bash
helm test my-lakehouse --namespace cave-system
```
