...
Helm chart for **cave-keda** — port of KEDA event-driven autoscaler.

Workload kind: `deployment`. Default service port: `9022`.

## Install

```bash
helm install my-keda ./helm/charts/cave-keda \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-keda` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9022` | upstream-aligned default |
...
```bash
helm upgrade my-keda ./helm/charts/cave-keda \
...
```bash
helm test my-keda --namespace cave-system
```
