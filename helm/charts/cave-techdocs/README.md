...
Helm chart for **cave-techdocs** — port of TechDocs documentation service.
...
```bash
helm install my-techdocs ./helm/charts/cave-techdocs \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-techdocs` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-techdocs ./helm/charts/cave-techdocs \
...
```bash
helm test my-techdocs --namespace cave-system
```
