...
Helm chart for **cave-sandbox** — port of gVisor/Kata/Firecracker runtimes.

Workload kind: `daemonset`. Default service port: `8080`.

## Install

```bash
helm install my-sandbox ./helm/charts/cave-sandbox \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-sandbox` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-sandbox ./helm/charts/cave-sandbox \
...
```bash
helm test my-sandbox --namespace cave-system
```
