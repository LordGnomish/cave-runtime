...
Helm chart for **cave-portal** — port of Cave admin portal (Backstage-like).

Workload kind: `deployment`. Default service port: `7007`.

## Install

```bash
helm install my-portal ./helm/charts/cave-portal \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-portal` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `7007` | upstream-aligned default |
...
```bash
helm upgrade my-portal ./helm/charts/cave-portal \
...
```bash
helm test my-portal --namespace cave-system
```
