...
Helm chart for **cave-iceberg** — port of Apache Iceberg catalog.

Workload kind: `deployment`. Default service port: `8181`.

## Install

```bash
helm install my-iceberg ./helm/charts/cave-iceberg \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-iceberg` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8181` | upstream-aligned default |
...
```bash
helm upgrade my-iceberg ./helm/charts/cave-iceberg \
...
```bash
helm test my-iceberg --namespace cave-system
```
