...
Helm chart for **cave-slo** — port of SLO controller (sloth).
...
```bash
helm install my-slo ./helm/charts/cave-slo \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-slo` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-slo ./helm/charts/cave-slo \
...
```bash
helm test my-slo --namespace cave-system
```
