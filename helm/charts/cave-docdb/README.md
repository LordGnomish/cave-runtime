...
Helm chart for **cave-docdb** — port of MongoDB-compatible doc store.

Workload kind: `statefulset`. Default service port: `27017`.

## Install

```bash
helm install my-docdb ./helm/charts/cave-docdb \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-docdb` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `27017` | upstream-aligned default |
...
```bash
helm upgrade my-docdb ./helm/charts/cave-docdb \
...
```bash
helm test my-docdb --namespace cave-system
```
