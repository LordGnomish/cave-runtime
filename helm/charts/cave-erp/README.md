...
Helm chart for **cave-erp** — port of ERP integration service.
...
```bash
helm install my-erp ./helm/charts/cave-erp \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-erp` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-erp ./helm/charts/cave-erp \
...
```bash
helm test my-erp --namespace cave-system
```
