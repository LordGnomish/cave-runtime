...
Helm chart for **cave-chat** — port of ChatOps webhook handler.
...
```bash
helm install my-chat ./helm/charts/cave-chat \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-chat` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-chat ./helm/charts/cave-chat \
...
```bash
helm test my-chat --namespace cave-system
```
