...
Helm chart for **cave-pipelines** — port of Tekton pipelines controller.

Workload kind: `deployment`. Default service port: `9090`.

## Install

```bash
helm install my-pipelines ./helm/charts/cave-pipelines \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-pipelines` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `9090` | upstream-aligned default |
...
```bash
helm upgrade my-pipelines ./helm/charts/cave-pipelines \
...
```bash
helm test my-pipelines --namespace cave-system
```
