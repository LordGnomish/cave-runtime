...
Helm chart for **cave-forensics** — port of Tetragon v1.7.0.

Workload kind: `daemonset`. Default service port: `2112`.

## Install

```bash
helm install my-forensics ./helm/charts/cave-forensics \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-forensics` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `2112` | upstream-aligned default |
...
```bash
helm upgrade my-forensics ./helm/charts/cave-forensics \
...
```bash
helm test my-forensics --namespace cave-system
```
