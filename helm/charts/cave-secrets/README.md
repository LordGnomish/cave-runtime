...
Helm chart for **cave-secrets** — port of External Secrets Operator v2.5.0.
...
```bash
helm install my-secrets ./helm/charts/cave-secrets \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-secrets` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-secrets ./helm/charts/cave-secrets \
...
```bash
helm test my-secrets --namespace cave-system
```
