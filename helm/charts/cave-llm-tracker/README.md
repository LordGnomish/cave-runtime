...
Helm chart for **cave-llm-tracker** — port of LLM tracker daemon.
...
```bash
helm install my-llm-tracker ./helm/charts/cave-llm-tracker \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-llm-tracker` | OCI image (placeholder until release pipeline lands) |
...
```bash
helm upgrade my-llm-tracker ./helm/charts/cave-llm-tracker \
...
```bash
helm test my-llm-tracker --namespace cave-system
```
