...
Helm chart for **cave-identity** — port of SPIRE v1.15.0.

Workload kind: `statefulset`. Default service port: `8081`.

## Install

```bash
helm install my-identity ./helm/charts/cave-identity \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-identity` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `8081` | upstream-aligned default |
...
```bash
helm upgrade my-identity ./helm/charts/cave-identity \
...
```bash
helm test my-identity --namespace cave-system
```
