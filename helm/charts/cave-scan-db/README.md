...
Helm chart for **cave-scan-db** — port of Vulnerability DB store.

Workload kind: `statefulset`. Default service port: `5432`.

## Install

```bash
helm install my-scan-db ./helm/charts/cave-scan-db \
...
| Key | Default | Notes |
| --- | --- | --- |
| `image.repository` | `ghcr.io/lordgnomish/cave-scan-db` | OCI image (placeholder until release pipeline lands) |
| `image.tag` | `0.1.0` | matches chart `appVersion` |
| `replicaCount` | `1` | ignored for DaemonSet |
| `service.port` | `5432` | upstream-aligned default |
...
```bash
helm upgrade my-scan-db ./helm/charts/cave-scan-db \
...
```bash
helm test my-scan-db --namespace cave-system
```
